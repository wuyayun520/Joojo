...
class JoojoCoinProduct {
  final String amountStr;
  final String productId;
  final double price;
  int get amount => int.tryParse(amountStr) ?? 0;
  const JoojoCoinProduct(this.amountStr, this.productId, this.price);
}


class JoojoCoinProducts {
  static const List<JoojoCoinProduct> all = [
    JoojoCoinProduct('32', 'Joojo', 0.99),
    JoojoCoinProduct('60', 'Joojo1', 1.99),
    JoojoCoinProduct('96', 'Joojo2', 2.99),
    JoojoCoinProduct('155', 'Joojo4', 4.99),
    JoojoCoinProduct('189', 'Joojo5', 5.99),
    JoojoCoinProduct('249', 'Joojo6', 6.99),
    JoojoCoinProduct('359', 'Joojo9', 9.99),
    JoojoCoinProduct('729', 'Joojo19', 19.99),
    JoojoCoinProduct('1869', 'Joojo49', 49.99),
    JoojoCoinProduct('3799', 'Joojo99', 99.99),
  ];
}
...
class _WalletScreenState extends State<WalletScreen> {
  ...
  List<JoojoCoinProduct> get _coinProducts => JoojoCoinProducts.all;
  ...
  Future<void> _loadCoins() async {
    ...
    int coins = prefs.getInt('joojoCoins') ?? 0;
    debugPrint('Loading Joojo coins: $coins');
      setState(() {
        _coins = coins;
        if (!_isInitialized) {
          _isInitialized = true;
  ...
  Future<void> _saveCoins(int amount) async {
    final prefs = await SharedPreferences.getInstance();
    debugPrint('Saving Joojo coins: current=$_coins, adding=$amount, new=${_coins + amount}');
    debugPrint('Call stack: ${StackTrace.current}');
    setState(() {
      _coins += amount;
    });
    await prefs.setInt('joojoCoins', _coins);
  }
        ...
        child: Row(
          children: [
            ...
            Container(
              ...
              child: Center(
                child: Image.asset(
                  'assets/joojo_wallet_diamond.webp',
                  width: 55,
                  height: 55,
                  fit: BoxFit.contain,
                  errorBuilder: (context, error, stackTrace) {
  ...
  Widget _buildProductCard(JoojoCoinProduct item, String priceStr, bool isSelected) {
