...
class JoojoChatScreen extends StatefulWidget {
  final String userId;
  final String userName;
  final String userAvatar;

  const JoojoChatScreen({
    super.key,
  ...
  @override
  State<JoojoChatScreen> createState() => _JoojoChatScreenState();
}

class _JoojoChatScreenState extends State<JoojoChatScreen> {
  List<_JoojoChatMessage> _messages = [];
  ...
  Future<String> _getMediaDirectory() async {
    ...
    final mediaDir = Directory('${dir.path}/joojo_chat_media');
  ...
  Future<void> _loadMessages() async {
    final dir = await getApplicationDocumentsDirectory();
    final file = File('${dir.path}/joojo_chat_history_${widget.userId}.json');
    if (await file.exists()) {
      try {
        ...
        setState(() {
          _messages = jsonList.map((e) => _JoojoChatMessage.fromJson(e)).toList();
        });
  ...
  void _sendMessage() {
    ...
    setState(() {
        _messages.add(_JoojoChatMessage(
        text: text,
        isMe: true,
        time: _getTimeStamp(),
          type: _JoojoChatMessageType.text,
      ));
    });
  ...
  Future<void> _sendImage(File imageFile) async {
    try {
      ...
      final fileName = 'joojo_image_${DateTime.now().millisecondsSinceEpoch}.jpg';
      ...
      setState(() {
        _messages.add(_JoojoChatMessage(
          ...
          isMe: true,
          time: _getTimeStamp(),
          type: _JoojoChatMessageType.image,
        ));
      });
      _scrollToBottom();
      _saveMessages();
  ...
  Future<void> _sendVoice(String audioPath, Duration duration) async {
    try {
      final mediaDir = await _getMediaDirectory();
      final fileName = 'joojo_voice_${DateTime.now().millisecondsSinceEpoch}.m4a';
      ...
      setState(() {
        _messages.add(_JoojoChatMessage(
          ...
          isMe: true,
          time: _getTimeStamp(),
          type: _JoojoChatMessageType.audio,
        ));
      });
      _scrollToBottom();
      _saveMessages();
    ...
    try {
      if (_isRecording) {
        ...
        if (await _voiceRecorder.hasPermission()) {
          ...
          final filePath = '${dir.path}/joojo_voice_${DateTime.now().millisecondsSinceEpoch}.m4a';
...
enum _JoojoChatMessageType { text, image, audio }

class _JoojoChatMessage {
  final String? text;
  ...
  final bool isMe;
  final String time;
  final _JoojoChatMessageType type;
  ...
  static _JoojoChatMessage fromJson(Map<String, dynamic> json) => _JoojoChatMessage(
    text: json['text'],
    ...
    isMe: json['isMe'] ?? true,
    time: json['time'] ?? '',
    type: _JoojoChatMessageType.values.firstWhere(
      (e) => e.name == json['type'],
      orElse: () => _JoojoChatMessageType.text,
    ),
  );
}

class _JoojoChatBubble extends StatefulWidget {
  final _JoojoChatMessage message;
  final void Function(File file)? onImageTap;
  const _JoojoChatBubble({required this.message, this.onImageTap});

  @override
  State<_JoojoChatBubble> createState() => _JoojoChatBubbleState();
}

class _JoojoChatBubbleState extends State<_JoojoChatBubble> {
  ...
  void initState() {
    super.initState();
    if (widget.message.type == _JoojoChatMessageType.audio) {
  ...
  Future<void> _togglePlayPause() async {
    try {
      ...
      final dir = await getApplicationDocumentsDirectory();
      final absPath = '${dir.path}/joojo_chat_media/${msg.audioPath}';
  ...
  Widget build(BuildContext context) {
    ...
    if (msg.type == _JoojoChatMessageType.audio && msg.audioPath != null) {
      ...
      return Align(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            border: msg.isMe ? null : Border.all(color: AppTheme.primaryColor.withOpacity(0.3), width: 1),
            boxShadow: [
              BoxShadow(
                color: msg.isMe 
                  ...
                  GestureDetector(
                    ...
                    child: Container(
                      ...
                      child: Icon(
                        ...
                        color: msg.isMe ? Colors.white : AppTheme.primaryEnd,
                        size: 26,
                      ),
                    ),
                  ),
                      ...
                      children: [
                        Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            color: (msg.isMe ? Colors.white : AppTheme.primaryEnd).withOpacity(0.2),
                          ),
                          child: LinearProgressIndicator(
                            ...
                            valueColor: AlwaysStoppedAnimation<Color>(
                              msg.isMe ? Colors.white : AppTheme.primaryEnd
                            ),
                            minHeight: 6,
                          ),
                        ),
    ...
    if (msg.type == _JoojoChatMessageType.image && msg.imagePath != null) {
      return Align(
        alignment: msg.isMe ? Alignment.centerRight : Alignment.centerLeft,
        child: GestureDetector(
          onTap: () async {
            final dir = await getApplicationDocumentsDirectory();
            final absPath = '${dir.path}/joojo_chat_media/${msg.imagePath}';
                ...
                ClipRRect(
                  ...
                  child: FutureBuilder<Directory>(
                    ...
                    builder: (context, snapshot) {
                      ...
                      final absPath = '${snapshot.data!.path}/joojo_chat_media/${msg.imagePath}';
                      return Image.file(
                        File(absPath),
                        width: 160,
                        height: 160,
...
class _JoojoChatInputBar extends StatelessWidget {
                  ...
                  Container(
                    decoration: BoxDecoration(
                      gradient: isRecording
                          ? LinearGradient(
                              colors: [
                                AppTheme.primaryEnd.withOpacity(0.8),
                                AppTheme.primaryEnd,
                              ],
                            )
                          : AppTheme.primaryGradient,
