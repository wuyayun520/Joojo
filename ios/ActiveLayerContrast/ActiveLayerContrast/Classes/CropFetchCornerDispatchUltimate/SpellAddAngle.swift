...
protocol ClipEnhance: AnyObject {
...
class SpellAddAngle: UIViewController {
    ...
    private lazy var controllerScrollView: UIScrollView = {
        ...
        var height = CharacteristicSine.doBeau() - (topDistance + kTitle_H)
        //: if self.hasBttomTabBar {
        if self.hasBttomTabBar {
            //: height -= TabBarViewHeight
            height -= noti_makeFormat
        }
        ...
        let rect = CGRect(x: 0, y: topDistance + kTitle_H, width: CharacteristicSine.observeData(), height: height)
        //: let scrollView = UIScrollView(frame: rect)
        let scrollView = UIScrollView(frame: rect)
        //: scrollView.contentSize = CGSize(width: count * Int(MacroDefine.getScreenWidth()), height: 0)
        scrollView.contentSize = CGSize(width: count * Int(CharacteristicSine.observeData()), height: 0)
        ...
        scrollView.semanticContentAttribute = TitlePer.shared.direction == .leftToRight ? .forceLeftToRight:.forceRightToLeft
        //: return scrollView
        return scrollView
    //: }()
    }()
    ...
    private lazy var titleScrollView: UIScrollView = {
        ...
        var rect = CGRect(x: 15, y: topDistance, width: CharacteristicSine.observeData()-rightSpace, height: kTitle_H)
        //: if isBisect {
        if isBisect {
            //: rect = CGRect(x: 0, y: topDistance, width: MacroDefine.getScreenWidth()-rightSpace, height: kTitle_H)
            rect = CGRect(x: 0, y: topDistance, width: CharacteristicSine.observeData()-rightSpace, height: kTitle_H)
        }
        //: let scrollView = UIScrollView(frame: rect)
        let scrollView = UIScrollView(frame: rect)
        ...
        var originX: CGFloat = TitlePer.shared.direction == .leftToRight ? 0:25
        ...
        self.titleItems.enumerated().forEach { [weak self] index, obj in
            ...
            titleBtn.addTarget(self, action: #selector(paraTrigger), for: .touchDown)
            //: var titleWidth = 0.0
            var titleWidth = 0.0
            //: if isBisect {
            if isBisect {
                //: titleWidth = MacroDefine.getScreenWidth() / Double(self.titleItems.count)
                titleWidth = CharacteristicSine.observeData() / Double(self.titleItems.count)
            //: } else {
            } else {
            ...
            let x = TitlePer.shared.direction == .leftToRight ? originX:(CharacteristicSine.observeData()-originX-titleWidth)
...
extension SpellAddAngle {
    ...
    private func startDrop(withIndex index: Int) {
        ...
        let origin_x = CGFloat(index) * CharacteristicSine.observeData()
        ...
        vc.view.frame = CGRect(x: origin_x, y: 0, width: CharacteristicSine.observeData(), height: controllerScrollView.frame.size.height)
        //: controllerScrollView.addSubview(vc.view)
        controllerScrollView.addSubview(vc.view)
    }
    
    //: private func currentSelectedTitleButton(_ button: UIButton) {
    private func tagConvert(_ button: UIButton) {
    ...
    func rhoPressure(_ sender: UIButton) {
        //: if titleScrollView.contentSize.width <= MacroDefine.getScreenWidth() {
        if titleScrollView.contentSize.width <= CharacteristicSine.observeData() {
            //: return
            return
        }
        //: var offset_x = sender.center.x - MacroDefine.getScreenWidth() * 0.5
        var offset_x = sender.center.x - CharacteristicSine.observeData() * 0.5
        //: if offset_x < 0 {
        if offset_x < 0 {
            //: offset_x = 0
            offset_x = 0
        ...
        let maxOffset = titleScrollView.contentSize.width - CharacteristicSine.observeData()
...
extension SpellAddAngle: UIScrollViewDelegate {
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        let index = Int(scrollView.contentOffset.x / CharacteristicSine.observeData())
        ...
        let scaleR = scrollView.contentOffset.x / CharacteristicSine.observeData() - CGFloat(index)
        //: let scaleL = 1 - scaleR
        let scaleL = 1 - scaleR
