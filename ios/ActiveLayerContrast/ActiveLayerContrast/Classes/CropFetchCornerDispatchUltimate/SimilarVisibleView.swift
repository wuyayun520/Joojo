...
/*: "#ffffff" :*/
fileprivate let main_bubbleFormat:String = "#ffffffstrip edge"

/*: "earned" :*/
fileprivate let appParticleTitle:[Character] = ["e","a","r","n","e","d"]

/*: "Coins in" :*/
fileprivate let kAgainstTitle:String = "Coins grin access its control"
fileprivate let user_tauUrl:[Character] = ["i","n"]

/*: "Error:  :*/
fileprivate let show_evaluateValue:String = "today operate modelEr"
fileprivate let noti_ionPath:String = "momentum channel injury ported storageror: "

/*: "#C946D2" :*/
fileprivate let showEffectFormat:String = "#C946D2report visible available"

/*: "btn_chat_floatS_userBK_nor" :*/
fileprivate let main_teamData:[Character] = ["b","t","n","_","c","h","a","t","_","f","l","o","a","t","S","_","u","s","e","r","B","K","_"]
fileprivate let kScriptCondemnData:[Character] = ["n","o","r"]
...
class SimilarVisibleView: UIView {
    ...
    private let TextColor = (String(main_bubbleFormat.prefix(7)))

    //: var clickBlock: (() -> Void)?
    var clickBlock: (() -> Void)?

    //: convenience init(model: TalkingGameFloatingScreenModel?) {
    convenience init(model: PropertyModel?) {
        //: self.init()
        self.init()
    ...
    private func enableceEnable() {
        ...
        if danumuMsgModel.bgPic.count > 0 {
            ...
            partyLength(withUrl: danumuMsgModel.bgPic)
        }
        ...
        if danumuMsgModel.content.count > 0 {
            ...
            do {
                ...
                if let range = attrStr.string.range(of: (String(appParticleTitle))) {
                ...
                if let range = attrStr.string.range(of: (String(kAgainstTitle.prefix(6)) + String(user_tauUrl))) {
                    //: attrStr.addAttributes([.foregroundColor: UIColor.white], range: NSRange(range, in: attrStr.string))
                    attrStr.addAttributes([.foregroundColor: UIColor.white], range: NSRange(range, in: attrStr.string))
                }
                //: attrStr.addAttributes([.font: UIFont.pingfangFont(type: .Medium, fontSize: 12)], range: NSRange(location: 0, length: attrStr.length))
                attrStr.addAttributes([.font: UIFont.bagUniform(type: .Medium, fontSize: 12)], range: NSRange(location: 0, length: attrStr.length))
                ...
                printLog(message: (String(show_evaluateValue.suffix(2)) + String(noti_ionPath.suffix(5))) + "\(error)")
            }
        }
    }

    //: func updateBackground(withUrl url: String) {
    func partyLength(withUrl url: String) {
        ...
        self.backgroundImageView.moreThen(urlStr: url) { [weak self] result, image in
            ...
            if let image {
                ...
                if image.size.height > self.HeightOfBackgroundImageView {
                    ...
                    newImage = image.giftImplementContainerComprehensive(with: CGSize(width: targetWidth, height: self.HeightOfBackgroundImageView))
                }
            }
    ...
    var danumuMsgModel: PropertyModel? {
        //: didSet {
        didSet {
            //: configModel()
            enableceEnable()
    ...
    var totalTime: CGFloat {
        ...
        return (show_licenseTitle + widthValue) * SpendTime / show_licenseTitle
    }

    //: var totalTimeOfLine: CGFloat {
    var totalTimeOfLine: CGFloat {
        //: return widthValue * LineSpendTime / ScreenWidth
        return widthValue * LineSpendTime / show_licenseTitle
    }

    //: var hide2ShowTime: CGFloat {
    var hide2ShowTime: CGFloat {
        //: return widthValue * SpendTime / ScreenWidth
        return widthValue * SpendTime / show_licenseTitle
    }
    ...
    lazy var headAnimatedImageView: UIImageView = {
        ...
        h.layer.borderColor = UIColor(hex: (String(showEffectFormat.prefix(7))))?.cgColor
        //: return h
        return h
        //: }()
    }()
    ...
    lazy var headBackImageView: UIImageView = {
        ...
        h.image = UIImage.road(name: (String(main_teamData) + String(kScriptCondemnData)))
        //: return h
        return h
        //: }()
    }()
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(player(_:)))
        //: return t
        return t
        //: }()
    }()
...
extension SimilarVisibleView {
    /// 点击跳转事件
    //: @objc func tapAction(_ tapGesture: UITapGestureRecognizer) {
    @objc func player(_ tapGesture: UITapGestureRecognizer) {
        //: guard let gameUrl = self.danumuMsgModel?.gameUrl else { return }
        guard let gameUrl = self.danumuMsgModel?.gameUrl else { return }
        //: let httpsUrl = ChartShowSceneScientific.share.appConfigMode.urlH5Domain+gameUrl
        let httpsUrl = ChartShowSceneScientific.share.appConfigMode.urlH5Domain + gameUrl
        ...
        config.clearBgColor = true
        //: BeforeAssociate.share.func__pushToWebVC(urlStr: httpsUrl, webConfig: config)
        BeforeAssociate.share.magnitudeer(urlStr: httpsUrl, webConfig: config)
    }
}
