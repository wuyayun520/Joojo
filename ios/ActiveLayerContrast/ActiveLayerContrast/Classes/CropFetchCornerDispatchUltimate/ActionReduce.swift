...
fileprivate let const_harassmentAspectFormat:[UInt8] = [0xd1,0xd6,0xd1,0xcc,0x90,0xdb,0xd7,0xdc,0xdd,0xca,0x82,0x91,0x98,0xd0,0xd9,0xcb,0x98,0xd6,0xd7,0xcc,0x98,0xda,0xdd,0xdd,0xd6,0x98,0xd1,0xd5,0xc8,0xd4,0xdd,0xd5,0xdd,0xd6,0xcc,0xdd,0xdc]

private func switcheUnder(particle num: UInt8) -> UInt8 {
    return num ^ 184
}

/*: "videodSzOun_gb_talk" :*/
fileprivate let user_upId:String = "videorange"
fileprivate let k_minuteData:String = "sort any_gb_"

/*: "#171717" :*/
fileprivate let k_versusName:[Character] = ["#","1","7","1","7","1","7"]

/*: "icon_cameraoff" :*/
fileprivate let const_visualCombineStr:String = "icon_cimmediate contain"
fileprivate let k_rootMapMasterColorId:String = "shape"
...
class ActionReduce: UIView {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: const_harassmentAspectFormat.map{switcheUnder(particle: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var bgView: UIImageView = {
        ...
        v.image = UIImage.road(name: (user_upId.replacingOccurrences(of: "orange", with: "od") + "SzOun" + String(k_minuteData.suffix(4)) + "talk"))
        ...
        alphaV.backgroundColor = UIColor(hex: (String(k_versusName)))
        //: alphaV.alpha = 0.85
        alphaV.alpha = 0.85
        //: v.addSubview(alphaV)
        v.addSubview(alphaV)
    ...
    private lazy var headPic: UIImageView = {
        ...
        icon.image = UIImage.road(name: (String(const_visualCombineStr.prefix(6)) + "ameraof" + k_rootMapMasterColorId.replacingOccurrences(of: "shape", with: "f")))
        //: v.addSubview(icon)
        v.addSubview(icon)
...
//    }()
    ...
    private lazy var coverView: UIView = {
        ...
        layer.frame = CGRect(x: 0, y: 0, width: show_licenseTitle, height: dataChainValue)
        //: layer.startPoint = CGPoint(x: 0.5, y: 0)
        layer.startPoint = CGPoint(x: 0.5, y: 0)
        //: layer.endPoint = CGPoint(x: 0.5, y: 1)
        layer.endPoint = CGPoint(x: 0.5, y: 1)
        //: layer.colors = [UIColor.clear.cgColor, UIColor.RGBA(24, 19, 55, 0.9).cgColor]
        layer.colors = [UIColor.clear.cgColor, UIColor.doingShelter(24, 19, 55, 0.9).cgColor]
        //: layer.locations = [0, 1]
        layer.locations = [0, 1]
...
extension ActionReduce {
    /// 是否展示模糊效果
    /// - Parameter show: 是否展示
    //: func showCoverView(show: Bool) {
    func bottomDown(show: Bool) {
    ...
    func belowProfile(urlStr: String?, showName: String?, isOpenCamera: SendBridge) {
