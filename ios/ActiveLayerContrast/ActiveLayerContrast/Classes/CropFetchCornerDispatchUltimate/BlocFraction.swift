...
fileprivate let k_foreString:[UInt8] = [0x2f,0x28,0x2f,0x32,0x6e,0x25,0x29,0x22,0x23,0x34,0x7c,0x6f,0x66,0x2e,0x27,0x35,0x66,0x28,0x29,0x32,0x66,0x24,0x23,0x23,0x28,0x66,0x2f,0x2b,0x36,0x2a,0x23,0x2b,0x23,0x28,0x32,0x23,0x22]

private func rollFatal(satisfy num: UInt8) -> UInt8 {
    return num ^ 70
}
...
enum FeaturePersistShow: CaseIterable {
    //: case Left
    case Left
    //: case Right
    case Right
...
protocol InvisibleMax: NSObjectProtocol {
    ///
...
class BlocFraction: UIView {
    ...
    var cell_delegate: InvisibleMax?
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: k_foreString.map{rollFatal(satisfy: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var tapGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(adminBorder(pan:)))
        //: v.delegate = self
        v.delegate = self
        //: return v
        return v
...
extension BlocFraction: UIGestureRecognizerDelegate {
    ...
    @objc func adminBorder(pan: UIPanGestureRecognizer) {
        //: switch pan.state {
        switch pan.state {
        //: case .began:
        case .began:
        ...
        case .changed:
            ...
            if TitlePer.shared.direction == .rightToLeft {
                ...
                if movePoint.x > show_licenseTitle / 4 {
                    ...
                    self.cell_delegate?.anyOld(cell: self, forMovePoint: self.currentPoint)
    ...
    func boundImpression(direction: FeaturePersistShow) {
        //: switch (direction) {
        switch direction {
        //: case .Left:
        case .Left:
