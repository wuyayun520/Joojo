...
/*: "detection" :*/
fileprivate let show_replaceId:String = "detectpublishn"

/*: "aiff" :*/
fileprivate let app_artKey:[Character] = ["a","i","f","f"]
...
class EagerOdd: NSObject {
    // singleton
    //: static let shared = RBDMuteSwitch()
    static let shared = EagerOdd()
    ...
    func master() {
        ...
        let soundFilePath = DocumentWhite.default.stopAdd(name: (show_replaceId.replacingOccurrences(of: "publish", with: "io")), type: (String(app_artKey)))
