...
/*: "Search" :*/
fileprivate let mainFrequencyString:String = "Searchimply significant plate additional detect"

/*: "#F5F5F5" :*/
fileprivate let user_failureStr:[Character] = ["#","F","5","F","5","F","5"]

/*: "Search by UID" :*/
fileprivate let show_theCustomerUrl:[Character] = ["S","e","a","r","c","h"]
fileprivate let notiThemeString:String = " by UIDbag interrupt retain popular increase"

/*: "icon_search_sea" :*/
fileprivate let k_pureFormat:[Character] = ["i","c","o","n"]
fileprivate let appLocateTactStr:String = "_seaair significant"

/*: "Can’t find~" :*/
fileprivate let userFitId:String = "Can’t finpatter transfer village"
fileprivate let show_canStr:[Character] = ["d","~"]

/*: "icon_kong_kong_default" :*/
fileprivate let user_implementationName:[Character] = ["i","c","o","n","_","k","o","n","g","_"]
fileprivate let main_extentMsg:String = "sourceong"
fileprivate let kAboveWeeklyDownString:String = "aallowt"

/*: "Please enter the query UID" :*/
fileprivate let user_portedHiddenFormat:String = "magnitude blank row special presentPleas"
fileprivate let mainFlowLatBottomName:String = "accuracy end delivery a divee en"
fileprivate let const_budgeTitle:[Character] = ["t","h","e"," ","q","u","e","r","y"," ","U","I","D"]
...
class CleanOwner: TrainBrush {
    ...
    private lazy var searchBtn: UIButton = {
        ...
        btn.setTitle((String(mainFrequencyString.prefix(6))).localized, for: .normal)
        //: btn.setTitleColor(.white, for: .normal)
        btn.setTitleColor(.white, for: .normal)
        //: btn.titleLabel?.font = UIFont.pingfangRugularFont(fontSize: 15)
        btn.titleLabel?.font = UIFont.belly(fontSize: 15)
        ...
        btn.setBackgroundImage(UIImage.managerMouse(colors: UIColor.raceGold(), size: CGSize(width: 72, height: 32)), for: .normal)
        //: btn.layer.cornerRadius = 16
        btn.layer.cornerRadius = 16
        //: btn.clipsToBounds = true
        btn.clipsToBounds = true
        //: btn.addTarget(self, action: #selector(searchButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(containerRandom), for: .touchUpInside)
        //: btn.alpha = 0.5
        btn.alpha = 0.5
    ...
    private lazy var searchTF: UITextField = {
        ...
        tf.backgroundColor = UIColor(hex: (String(user_failureStr)))
        ...
        tf.font = UIFont.belly(fontSize: 16)
        ...
        tf.placeholder = (String(show_theCustomerUrl) + String(notiThemeString.prefix(7))).localized
    ...
    private lazy var unlessEditingView: UIView = {
        ...
        let image = UIImage.road(name: (String(k_pureFormat) + String(appLocateTactStr.prefix(4)) + "rch_sea"))
    ...
    private lazy var tableView: UITableView = {
        ...
        tabV.register(LevelTop.self, forCellReuseIdentifier: LevelTop.className())
    ...
    private lazy var emptyV: EmptyView = {
        ...
        style.TipsTitle = (String(userFitId.prefix(9)) + String(show_canStr)).localized
        //: style.TipsIcon = "icon_kong_kong_default"
        style.TipsIcon = (String(user_implementationName) + main_extentMsg.replacingOccurrences(of: "source", with: "k") + "_def" + kAboveWeeklyDownString.replacingOccurrences(of: "allow", with: "ul"))
        ...
        let v = EmptyView(frame: CGRect(x: 0, y: 14 + appPurpleVersusMessage, width: show_licenseTitle, height: dataChainValue - (14 + appPurpleVersusMessage + dataSuccessResponseMessage)), style: style)
    ...
    private lazy var reqManager: AngleTotalerval = //: return AngleTotalerval()
        .init()
    //: }()
}
...
extension CleanOwner {
    // 根据uid查询用户
    //: private func req_searchUid() {
    private func noDocument() {
        //: self.view.endEditing(true)
        self.view.endEditing(true)
        //: guard searchTF.text?.count ?? 0 > 0 else {
        guard searchTF.text?.count ?? 0 > 0 else {
            //: self.func__showStatusBarErrorMsg(showMsg: "Please enter the query UID".localized)
            self.bury(showMsg: (String(user_portedHiddenFormat.suffix(5)) + String(mainFlowLatBottomName.suffix(4)) + "ter " + String(const_budgeTitle)).localized)
            //: return
            return
        }
        ...
        reqManager.factoryCounteraction(uid: searchTF.text ?? "") { [weak self] succeed, result, errorModel in
            //: ProgressHUD.dismiss()
            GrayView.at()
...
extension CleanOwner {
    ...
    @objc private func containerRandom() {
        //: req_searchUid()
        noDocument()
    }
...
extension CleanOwner: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: LevelTop.className(), for: indexPath) as! LevelTop
        //: let model = reqManager.searchListDataArr[indexPath.row]
        let model = reqManager.searchListDataArr[indexPath.row]
        //: cell.refreshCell(model: model)
        cell.sumerval(model: model)
...
extension CleanOwner: UITextFieldDelegate {
...
extension CleanOwner: JXPagingViewListViewDelegate {
...
extension CleanOwner {
    ...
    private func variationAppointedCharacter() {
