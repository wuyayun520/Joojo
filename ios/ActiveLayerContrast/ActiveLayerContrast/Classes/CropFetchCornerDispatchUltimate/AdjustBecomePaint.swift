...
fileprivate let noti_pinMsg:[UInt8] = [0xbe,0xb9,0xbe,0xa3,0xff,0xb4,0xb8,0xb3,0xb2,0xa5,0xed,0xfe,0xf7,0xbf,0xb6,0xa4,0xf7,0xb9,0xb8,0xa3,0xf7,0xb5,0xb2,0xb2,0xb9,0xf7,0xbe,0xba,0xa7,0xbb,0xb2,0xba,0xb2,0xb9,0xa3,0xb2,0xb3]

private func mediaForm(fail num: UInt8) -> UInt8 {
    return num ^ 215
}

/*: "icon_chat_miss" :*/
fileprivate let constDismissFormat:String = "sternon"
fileprivate let data_interestedMessage:String = "interest heli_miss"

/*: "icon_chat_yes" :*/
fileprivate let dataCriticalUrl:String = "icon_activity scope begin healthy"
fileprivate let data_duringStr:String = "emission"
...
class AdjustBecomePaint: GestureLast {
//    var transBlock: (() -> Void)?

    //: var textData: VariableChain?
    var textData: VariableChain?
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        fatalError(String(bytes: noti_pinMsg.map{mediaForm(fail: $0)}, encoding: .utf8)!)
    }
...
//                self.messageData?.translateStatus != .MsgTranslate_succeed && self.messageData?.msgModel.msgInfo.callCmd.count ?? 0 <= 0 && temTextData.msgModel.msgInfo.messageType != .MessageTypeWelcome && "\(temTextData.msgModel.user.uid)" != CharacteristicSine.getXiaoMiID()) { // 对方发的 && 开启了翻译功能 && 消息未翻译成功
...
//            }
            ...
            if temTextData.msgModel.quoteMsgInfo.yield() {
            ...
            if self.textData?.translateStatus == .MsgTranslate_translating {
                ...
                self.transStatusImagV.image = UIImage.road(name: (constDismissFormat.replacingOccurrences(of: "stern", with: "ic") + "_chat" + String(data_interestedMessage.suffix(5))))
                ...
                self.transStatusImagV.image = UIImage.road(name: (String(dataCriticalUrl.prefix(5)) + "chat_y" + data_duringStr.replacingOccurrences(of: "mission", with: "s")))
            }
    ...
    func afterPer() {
        ...
        let containerTap = UITapGestureRecognizer(target: self, action: #selector(note(tap:)))
        ...
        self.container.addGestureRecognizer(containerTap)
        //: let isOfficial = (String(self.textData?.msgModel.user.uid ?? 0) == MacroDefine.getXiaoMiID())
        let isOfficial = (String(self.textData?.msgModel.user.uid ?? 0) == CharacteristicSine.beforeVent())
        //: if isOfficial == false {
        if isOfficial == false {
            //: addLongTagGes()
            untilDimension()
...
//            }
    ...
    func untilDimension() {
        ...
        let longPressGr = UILongPressGestureRecognizer(target: self, action: #selector(beside(tap:)))
        //: self.container.addGestureRecognizer(longPressGr)
        self.container.addGestureRecognizer(longPressGr)
    }

    //: @objc func TouchCellMessageAction(tap: UITapGestureRecognizer?) {
    @objc func note(tap: UITapGestureRecognizer?) {
    ...
    @objc func beside(tap: UILongPressGestureRecognizer) {
        //: if tap.state == .ended {
        if tap.state == .ended {
            //: return
            return
...
//    }
    ...
    @objc func rankDown() {
        ...
        self.delegate?.pinAnswer(cell: self)
    }
}
