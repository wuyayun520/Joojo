...
fileprivate let k_meetingMsg:[UInt8] = [0x1d,0x22,0x1d,0x28,0xdc,0x17,0x23,0x18,0x19,0x26,0xee,0xdd,0xd4,0x1c,0x15,0x27,0xd4,0x22,0x23,0x28,0xd4,0x16,0x19,0x19,0x22,0xd4,0x1d,0x21,0x24,0x20,0x19,0x21,0x19,0x22,0x28,0x19,0x18]

fileprivate func shadeMagnitudeNear(floor num: UInt8) -> UInt8 {
...
/*: "bg_medals_podium" :*/
fileprivate let app_occasionKey:String = "kind prosecution step columnbg_m"
fileprivate let constUnlikelyUrl:String = "destination blind crush pet shelters_podium"

/*: "icon_rank_first_bg" :*/
fileprivate let kPartnerStr:[Character] = ["i","c","o","n","_","r","a","n","k","_","f","i","r","s","t","_","b"]
fileprivate let data_partMessage:String = "send"

/*: "icon_rank_first" :*/
fileprivate let dataFailureMsg:[Character] = ["i","c","o","n","_","r","a"]
fileprivate let dataResolveStr:[Character] = ["n","k","_","f","i","r","s","t"]

/*: "icon_ammount_points" :*/
fileprivate let noti_starKeepPath:[Character] = ["i","c","o","n","_","a","m","m","o","u","n","t","_"]
fileprivate let user_helloAbsName:[Character] = ["p","o","i","n","t","s"]

/*: "icon_lounge" :*/
fileprivate let main_remainingId:String = "reward tic agent rawicon_lo"
fileprivate let app_toolBedTitle:String = "unmediae"

/*: "icon_rank_coin" :*/
fileprivate let kSlideName:[Character] = ["i","c","o","n","_","r","a","n","k","_","c","o","i","n"]

/*: "icon_rank_coin2" :*/
fileprivate let showConstantId:[Character] = ["i","c","o","n","_","r","a","n","k","_","c","o","i","n"]
fileprivate let data_monsterUrl:String = "2"

/*: "icon_rank_second" :*/
fileprivate let dataAssociatedString:String = "flex peaicon_r"
fileprivate let app_activeStr:String = "teen"
fileprivate let kTooMessage:[Character] = ["k","_","s","e","c","o","n","d"]

/*: "icon_rank_third" :*/
fileprivate let appSaleValue:[Character] = ["i","c","o","n"]
fileprivate let data_animaVidTitle:[Character] = ["_","r","a","n","k","_","t","h","i","r","d"]
...
class GroupView: UIView {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: k_meetingMsg.map{shadeMagnitudeNear(floor: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var medalsView = UIImageView().then {
        ...
        $0.image = UIImage.road(name: (String(app_occasionKey.suffix(4)) + "edal" + String(constUnlikelyUrl.suffix(8))))
    }

    //: lazy var firstView = SocialRankHeaderItemView.init(frame: .zero, rankType: .firstRankType).then {
    lazy var firstView = SizeBind(frame: .zero, rankType: .firstRankType).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.tag = 0
        $0.tag = 0
        //: let tap = UITapGestureRecognizer.init(target: self, action: #selector(registTapAction))
        let tap = UITapGestureRecognizer(target: self, action: #selector(variation))
        //: $0.addGestureRecognizer(tap)
        $0.addGestureRecognizer(tap)
    }

    //: lazy var secondView = SocialRankHeaderItemView.init(frame: .zero, rankType: .secondRankType).then {
    lazy var secondView = SizeBind(frame: .zero, rankType: .secondRankType).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.tag = 1
        $0.tag = 1
    ...
    lazy var thirdView = SizeBind(frame: .zero, rankType: .thirdRankType).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.tag = 2
        $0.tag = 2
...
extension GroupView {
    ...
    private func enableTarget() {
        ...
        firstView.drink(model: dataArray[0], type: self.index)
        //: secondView.updateItemWithModel(model: dataArray.count>1 ? dataArray[1]: nil, type: self.index)
        secondView.drink(model: dataArray.count > 1 ? dataArray[1] : nil, type: self.index)
        //: thirdView.updateItemWithModel(model: dataArray.count>2 ? dataArray[2]: nil, type: self.index)
        thirdView.drink(model: dataArray.count > 2 ? dataArray[2] : nil, type: self.index)
    }

    //: @objc func registTapAction(tapGesture: UITapGestureRecognizer) {
    @objc func variation(tapGesture: UITapGestureRecognizer) {
...
extension GroupView {
    ...
    private func destroyVillageNumberma() {
        //: let firstWidth = (ScreenWidth - actualWidth(w: 68)) / 3 + actualWidth(w: 10)
        let firstWidth = (show_licenseTitle - actualWidth(w: 68)) / 3 + actualWidth(w: 10)
        //: let itemWidth = (ScreenWidth - actualWidth(w: 68)) / 3 - actualWidth(w: 10)
        let itemWidth = (show_licenseTitle - actualWidth(w: 68)) / 3 - actualWidth(w: 10)
...
class SizeBind: UIView {
    ...
    public init(frame: CGRect, rankType: NearSince) {
        //: super.init(frame: frame)
        super.init(frame: frame)
        //: self.setupItemViews(type: rankType)
        self.plain(type: rankType)
    }
    ...
    lazy var headPicBgView = UIImageView().then {
        ...
        $0.image = UIImage.road(name: (String(kPartnerStr) + data_partMessage.replacingOccurrences(of: "send", with: "g")))
    }
    ...
    lazy var crownImgView = UIImageView().then {
        ...
        $0.image = UIImage.road(name: (String(dataFailureMsg) + String(dataResolveStr)))
    }
    ...
    lazy var nickLabel = UILabel().then {
        ...
        $0.font = .bagUniform(type: .Medium, fontSize: 18)
        //: $0.textAlignment = .center
        $0.textAlignment = .center
    }

    //: lazy var coinButton = TalkingButton().then {
    lazy var coinButton = KeepPurple().then {
        //: $0.isUserInteractionEnabled = false
        $0.isUserInteractionEnabled = false
        //: $0.setImage(UIImage.BundleImageNamed(name: "icon_ammount_points"), for: .normal)
        $0.setImage(UIImage.road(name: (String(noti_starKeepPath) + String(user_helloAbsName))), for: .normal)
        //: $0.titleLabel?.font = .pingfangFont(type: .Regular, fontSize: 13)
        $0.titleLabel?.font = .bagUniform(type: .Regular, fontSize: 13)
    ...
    lazy var loungeImgV = UIImageView().then {
        ...
        $0.image = UIImage.road(name: (String(main_remainingId.suffix(7)) + app_toolBedTitle.replacingOccurrences(of: "media", with: "g")))
    }
}
...
extension SizeBind {
    ...
    func drink(model: FormatBecome?, type: Int) {
        ...
        headPic.moreThen(urlStr: model!.headPic, placeImg: UIImage.holiday(sex: model!.sex))
        ...
        if type == 0 {
            ...
            coinButton.setImage(UIImage.road(name: (String(kSlideName))), for: .normal)
            //: } else {
        } else {
            //: coinButton.setImage(UIImage.BundleImageNamed(name: "icon_rank_coin2"), for: .normal)
            coinButton.setImage(UIImage.road(name: (String(showConstantId) + data_monsterUrl.capitalized)), for: .normal)
        }
    }
}
...
//        var topCrownMargin = actualHeight(h: 4)
        ...
        case .firstRankType:
            ...
            crownImgView.image = UIImage.road(name: (String(dataFailureMsg) + String(dataResolveStr)))
            //: headPic.layer.borderColor = UIColor.RGBA(255, 224, 58, 1).cgColor
            headPic.layer.borderColor = UIColor.doingShelter(255, 224, 58, 1).cgColor
...
//            topCrownMargin = actualHeight(h: 10)
        ...
        case .secondRankType:
            ...
            crownImgView.image = UIImage.road(name: (String(dataAssociatedString.suffix(6)) + app_activeStr.replacingOccurrences(of: "tee", with: "a") + String(kTooMessage)))
            //: headPic.layer.borderColor = UIColor.RGBA(182, 199, 254, 1).cgColor
            headPic.layer.borderColor = UIColor.doingShelter(182, 199, 254, 1).cgColor
        ...
        case .thirdRankType:
            ...
            crownImgView.image = UIImage.road(name: (String(appSaleValue) + String(data_animaVidTitle)))
            //: headPic.layer.borderColor = UIColor.RGBA(251, 166, 109, 1).cgColor
            headPic.layer.borderColor = UIColor.doingShelter(251, 166, 109, 1).cgColor
            //: headPic.layer.cornerRadius = actualWidth(w: 31)
            headPic.layer.cornerRadius = actualWidth(w: 31)
            //: break
        }
