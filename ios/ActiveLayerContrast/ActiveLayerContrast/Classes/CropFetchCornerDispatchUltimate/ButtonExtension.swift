...
/*: "recharge" :*/
fileprivate let constSpecKey:String = "recharmobilee"

/*: "recharge_ :*/
fileprivate let dataTillGenderMsg:[Character] = ["r","e","c","h","a","r","g","e"]
fileprivate let const_agreeMessage:String = "rem"

/*: ".png" :*/
fileprivate let k_boardString:[Character] = [".","p","n","g"]

/*: .png" :*/
fileprivate let noti_evenMsg:[Character] = [".","p","n","g"]

/*: ".jpg" :*/
fileprivate let data_stopMessage:String = ".jpgagain limit something chemist stretch"

/*: .jpg" :*/
fileprivate let data_installMessage:String = "load cookie.jpg"
...
class KeepPurple: UIButton {
    ...
    var imageAlignment: ContainScale = .left
...
extension UIButton {
    ...
    func locateGroup(_ interval: CGFloat) {
        //: verticalAlignImageAndTitle(interval: interval, topOffset: 0)
        corruptionFuture(interval: interval, topOffset: 0)
    }
    ...
    func corruptionFuture(interval: CGFloat, topOffset: CGFloat) {
    ...
    func inputTally(interval: CGFloat, imageSize: CGSize, topOffset: CGFloat) {
...
extension UIButton {
    ...
    func asTimingPlain(color: UIColor, forState: UIControl.State) {
    ...
    func withinText(urlStr: String,
                    //: placeImg: UIImage? = UIImage.placeImgSquare(),
                    placeImg: UIImage? = UIImage.franklyBounce(),
                    ...
                    loadFinish: ((_ isSucceed: Bool) -> Void)? = nil)
    {
        ...
        let imgUrl = urlStr.replacingOccurrences(of: notiSaltFinishId.customForWrite(), with: data_graphicUrl)
    ...
    func lightByMar(urlStr: String) {
        //: guard !urlStr.isEmptyString else { return }
        guard !urlStr.isEmptyString else { return }
        //: var newUrl = ""
        var newUrl = ""
        //: if urlStr.contains("recharge") {
        if urlStr.contains((constSpecKey.replacingOccurrences(of: "mobile", with: "g"))) {
            //: newUrl = urlStr.replacingOccurrences(of: "recharge", with: "recharge_\(PackageID)")
            newUrl = urlStr.replacingOccurrences(of: (constSpecKey.replacingOccurrences(of: "mobile", with: "g")), with: (String(dataTillGenderMsg) + const_agreeMessage.replacingOccurrences(of: "rem", with: "_")) + "\(noti_messageFormat)")
            //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
            self.withinText(urlStr: newUrl, placeImg: nil)
            //: return
            return
                //: } else if urlStr.contains(".png") {
        } else if urlStr.contains((String(k_boardString))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".png", with: "_\(PackageID).png")
            newUrl = urlStr.replacingOccurrences(of: (String(k_boardString)), with: "_\(noti_messageFormat)" + (String(noti_evenMsg)))

            //: } else if urlStr.contains(".jpg") {
        } else if urlStr.contains((String(data_stopMessage.prefix(4)))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".jpg", with: "_\(PackageID).jpg")
            newUrl = urlStr.replacingOccurrences(of: (String(data_stopMessage.prefix(4))), with: "_\(noti_messageFormat)" + (String(data_installMessage.suffix(4))))
        }
        //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
        self.withinText(urlStr: newUrl, placeImg: nil)
    }
