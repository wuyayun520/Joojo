...
fileprivate let kRecognizeLanguageMessage:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "bth_live_follow_pre" :*/
fileprivate let kNomineeConversationStr:String = "third global triggerbth_"
fileprivate let notiPerformString:String = "FOLLOW"
fileprivate let mainScopeStr:[Character] = ["_","p","r","e"]

/*: "btn_live_mini_nor" :*/
fileprivate let data_splitKey:String = "btn_livepresent tip fail"
fileprivate let user_agreeValue:String = "next definitely_min"

/*: "#2DF2FF" :*/
fileprivate let k_orangId:[Character] = ["#","2","D","F","2"]
fileprivate let data_eggMessage:String = "slideslide"

/*: "Live" :*/
fileprivate let showHalfMethodMessage:[Character] = ["L","i","v","e"]

/*: "removeAttentionUid" :*/
fileprivate let constTranslateItsFormat:[Character] = ["r","e","m","o","v","e","A","t","t","e","n"]
fileprivate let show_youKey:String = "tionUidroman unlikely"

/*: "attentionUid" :*/
fileprivate let app_selectionMsg:[Character] = ["a","t","t","e","n","t","i","o","n","U","i","d"]

/*: "source" :*/
fileprivate let userPopularMessage:[Character] = ["s","o","u","r","c","e"]

/*: "num" :*/
fileprivate let app_manageMortalName:[Character] = ["n","u","m"]

/*: "UID: :*/
fileprivate let mainProSmilePath:String = "UID:vid make"

/*: "btn_live_closed_nor" :*/
fileprivate let constLeaveMorePath:[Character] = ["b","t","n","_","l","i","v","e","_","c","l","o","s","e","d","_","n"]
fileprivate let appBottomData:String = "ostage"
...
class RestoreProcessAttach: UIView {
    //: weak var weakVC: TalkingBaseViewController?
    weak var weakVC: TrainBrush?
    // 右侧按钮点击Block
    ...
    private var liveModel = AlwaysGreat()
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: kRecognizeLanguageMessage.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var iconBgBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(push), for: .touchUpInside)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var nickNameLab: UILabel = {
        ...
        lab.font = UIFont.pass(fontSize: 11)
        //: return lab
        return lab
        //: }()
    }()
    ...
    private lazy var followBtn: UIButton = {
        ...
        btn.setImage(UIImage.road(name: (String(kNomineeConversationStr.suffix(4)) + "live_" + notiPerformString.lowercased() + String(mainScopeStr))), for: .normal)
    ...
    private lazy var miniBtn: UIButton = {
        ...
        btn.setImage(UIImage.road(name: (String(data_splitKey.prefix(8)) + String(user_agreeValue.suffix(4)) + "i_nor")), for: .normal)
        ...
        //: btn.isHidden = (ChartShowSceneScientific.share.appStatus == AppSkinStatus.special.rawValue && ChartShowSceneScientific.share.loginUserMode.sex == Gender.female.rawValue)
        btn.isHidden = (ChartShowSceneScientific.share.appStatus == SophisticatedMultiple.special.rawValue && ChartShowSceneScientific.share.loginUserMode.sex == SharedOdd.female.rawValue)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var liveStatusImgV: KeepPurple = {
        ...
        let image = UIImage.fellowProduct(color: UIColor(hex: (String(k_orangId) + data_eggMessage.replacingOccurrences(of: "slide", with: "F")))!, size: CGSize(width: 8, height: 8))
        //: img.setImage(image.all(), for: .normal)
        img.setImage(image.all(), for: .normal)
        //: img.setTitle("Live".localized, for: .normal)
        img.setTitle((String(showHalfMethodMessage)).localized, for: .normal)
        //: img.isHidden = true
        img.isHidden = true
        //: return img
        return img
    ...
    private lazy var onlineNumBtn: UIButton = {
        ...
        btn.asTimingPlain(color: UIColor.black.withAlphaComponent(0.25), forState: .normal)
        //: btn.addTarget(self, action: #selector(onlineNumBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(putPrimary), for: .touchUpInside)
        //: btn.layer.cornerRadius = actualWidth(w: 32)/2
        btn.layer.cornerRadius = actualWidth(w: 32) / 2
        //: btn.setTitleColor(.white, for: .normal)
        btn.setTitleColor(.white, for: .normal)
        //: btn.titleLabel?.font = UIFont.pingfangMediumFont(fontSize: 12)
        btn.titleLabel?.font = UIFont.pass(fontSize: 12)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var onlineListView: CrucialPost = {
...
extension RestoreProcessAttach {
    ...
    @objc func validSpend(uid: Int, isAttention: Bool, completion: ((_ succeed: Bool) -> Void)? = nil) {
        ...
        if isAttention {
            ...
            dict[(String(constTranslateItsFormat) + String(show_youKey.prefix(7)))] = uid
            //: } else {
        } else {
            //: dict["attentionUid"] = uid
            dict[(String(app_selectionMsg))] = uid
            //: dict["source"] = "1"
            dict[(String(userPopularMessage))] = "1"
        }
        //: TalkingChatRequestTool.req_atationTool(isAttention: !isAttention, params: dict) { succeed, result, errorModel in
        NameProjection.clipAcrossManual(isAttention: !isAttention, params: dict) { succeed, result, errorModel in
            //: completion?(succeed)
            completion?(succeed)
        }
    }
...
extension RestoreProcessAttach {
    ...
    @objc private func putPrimary() {
        //: self.onlineListView.showView(chatGroupId: liveModel.chatGroupId)
        self.onlineListView.pendingBy(chatGroupId: liveModel.chatGroupId)
    }
    ...
    @objc private func onChart() {
        ...
        CancelTable().densityOwner(chatGroupId: liveModel.chatGroupId, completion: { succeed, result, errorModel in
            //: guard succeed else { return }
            guard succeed else { return }
            //: let json = JSON(result ?? [String: Any]())
            let json = JSON(result ?? [String: Any]())
            //: let num = json["num"].string
            let num = json[(String(app_manageMortalName))].string
    ...
    private func atCompareRecognize() {
        //: stopTimer()
        brushOf()
        ...
        onlineNunTimer = Timer.scheduledTimer(timeInterval: 10, target: self, selector: #selector(onChart), userInfo: nil, repeats: true)
    }
...
extension RestoreProcessAttach {
    ...
    func aboveConsume(_ liveModel: AlwaysGreat) {
        ...
        uidLab.text = (String(mainProSmilePath.prefix(4))) + "\(liveModel.streamerInfo.uid)"
        ...
        icon.moreThen(urlStr: liveModel.streamerInfo.headPic, placeImg: UIImage.holiday(sex: liveModel.streamerInfo.sex), options: optionsInfo)
        // 是否是主播
        //: let isAnchor = (String(liveModel.streamerInfo.uid) == ChartShowSceneScientific.share.loginUid)
        let isAnchor = (String(liveModel.streamerInfo.uid) == ChartShowSceneScientific.share.loginUid)
        //: if isAnchor {
        if isAnchor {
            //: } else {
            ...
            miniBtn.setImage(UIImage.road(name: (String(constLeaveMorePath) + appBottomData.replacingOccurrences(of: "stage", with: "r"))), for: .normal)
            //: refreshFollow(isAttention: self.liveModel.streamerInfo.isAttention)
            originalShared(isAttention: self.liveModel.streamerInfo.isAttention)
        }
    }
    ...
    @objc private func push() {
        ...
        property(uid: String(CaptureUsed.mutualAcross().liveRoomModel.streamerInfo.uid))
    }

    /// 展示个人资料卡
    //: private func showUserCard(uid: String) {
    private func property(uid: String) {
        //: let userCard = TalkingLiveUserCardView(frame: .zero, uid: uid)
        let userCard = AccelerateCurrency(frame: .zero, uid: uid)
        //: userCard.delegate = weakVC as? any TalkingLiveUserCardViewDelegate
        userCard.delegate = weakVC as? any SendCapacity
        //: userCard.show()
        userCard.under()
    }
    ...
    private func originalShared(isAttention: Bool) {
    ...
    @objc private func count() {
        ...
        validSpend(uid: liveModel.streamerInfo.uid, isAttention: liveModel.streamerInfo.isAttention) { [weak self] succeed in
            //: guard let self = self else { return }
            guard let self = self else { return }
            //: guard succeed else { return }
            guard succeed else { return }
            ...
            self.originalShared(isAttention: self.liveModel.streamerInfo.isAttention)
        }
    }
}
...
extension RestoreProcessAttach {
    ...
    private func messageSpring() {
        //: self.frame = CGRect(x: 0, y: 0, width: ScreenWidth, height: StatusBarHeight+actualWidth(w: 68))
        self.frame = CGRect(x: 0, y: 0, width: show_licenseTitle, height: show_orientationKey + actualWidth(w: 68))
