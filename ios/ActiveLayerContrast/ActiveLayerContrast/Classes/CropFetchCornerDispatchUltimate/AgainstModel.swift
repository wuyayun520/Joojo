...
/*: "reply" :*/
fileprivate let showCollectTodayMessage:[Character] = ["r","e","p","l","y"]
...
extension AgainstModel {
    //: mutating func caculateCommentItemHeight() {
    mutating func extraStructure() {
        //: var tempContent = ""
        var tempContent = ""
        ...
        if replyUser?.count ?? 0 > 0 {
            ...
            if TitlePer.shared.direction == .leftToRight {
                ...
                tempContent = (String(showCollectTodayMessage)).localized + "\(replyUser!): \(content!)"
                //: } else {
            } else {
                //: tempContent = "\(content!) :\(replyUser!)" + "reply".localized
                tempContent = "\(content!) :\(replyUser!)" + (String(showCollectTodayMessage)).localized
            }
        ...
        let sizeheight = Int(ScientificView.everyNative(text: tempContent, size: CGSize(width: show_licenseTitle - 70, height: CGFloat(MAXFLOAT)), font: UIFont.bagUniform(type: .Regular, fontSize: 16), lineNumber: 0).height)
