...
/*: "num" :*/
fileprivate let userRecoverKey:String = "nuniversal"
...
@objc public enum SinkContent: Int {
    //: case normal  = 1
    case normal = 1 // 普通礼物
    //: case myStery = 2
    case myStery = 2 // 盲盒礼物
...
struct IndexInput: HandyJSON {
    ...
    mutating func featureDoingto(giftNum: Int) {
        //: self.num = giftNum
        self.num = giftNum
    }
}
...
struct AboutMedium: HandyJSON {
    ...
    mutating func mapping(mapper: HelpingMapper) {
        ...
        mapper <<<
            //: self.giftNum <-- "num"
            self.giftNum <-- (userRecoverKey.replacingOccurrences(of: "universal", with: "um"))
    }
}
