...
fileprivate let mainColorTitle:[UInt8] = [0xb6,0xb1,0xb6,0xab,0xf7,0xbc,0xb0,0xbb,0xba,0xad,0xe5,0xf6,0xff,0xb7,0xbe,0xac,0xff,0xb1,0xb0,0xab,0xff,0xbd,0xba,0xba,0xb1,0xff,0xb6,0xb2,0xaf,0xb3,0xba,0xb2,0xba,0xb1,0xab,0xba,0xbb]

/*: "Select mute duration" :*/
fileprivate let app_informationPath:[Character] = ["S","e","l","e","c","t"," ","m","u","t","e"," ","d","u"]
fileprivate let kAreaId:String = "holdtion"

/*: "Cirform" :*/
fileprivate let main_coalitionUrl:String = "cancel"
fileprivate let const_donEmergeFormat:[Character] = ["i","r","f","o","r","m"]

/*: "Mute success" :*/
fileprivate let kPastId:String = "connect below another signMute"

/*: "day" :*/
fileprivate let main_minimumId:String = "classicay"

/*: "days" :*/
fileprivate let mainCustomerValue:[Character] = ["d","a","y","s"]
...
//: enum MuteViewType {
enum InsertImplement {
    //: case live
    case live
}

//: class TalkingLiveRoomMuteView: UIView {
class TechniqueView: UIView {
    ...
    var viewType: InsertImplement = .live
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: mainColorTitle.map{$0^223}, encoding: .utf8)!)
    }
    ...
    private lazy var tipsLb: UILabel = {
        ...
        lb.text = (String(app_informationPath) + kAreaId.replacingOccurrences(of: "hold", with: "ra")).localized
        //: lb.numberOfLines = 0
        lb.numberOfLines = 0
        //: return lb
        return lb
    ...
    lazy var commitBtn: UIButton = {
        ...
        btn.setTitle((main_coalitionUrl.replacingOccurrences(of: "cancel", with: "C") + String(const_donEmergeFormat)).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(clip), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension TechniqueView {
    ...
    func response() {
        ...
        self.contentView.frame = CGRect(x: (show_licenseTitle - contentV_Size.width) / 2, y: dataChainValue, width: contentV_Size.width, height: contentV_Size.height)

        //: UIView.animate(withDuration: 0.3) {
        UIView.animate(withDuration: 0.3) {
            //: self.contentView.frame.origin.y = (ScreenHeight-self.contentV_Size.height)/2
            self.contentView.frame.origin.y = (dataChainValue - self.contentV_Size.height) / 2
        }
    }
    ...
    func dark() {
        ...
        UIView.animate(withDuration: 0.3) {
            ...
            self.contentView.frame.origin.y = dataChainValue
...
extension TechniqueView {
    ...
    @objc private func criticalAcross(btn: UIButton) {
    ...
    @objc private func clip() {
        ...
        if self.viewType == .live {
            ...
            CancelTable().resumeEnable(targetUid: targetUid, duration: index) { succeed, result, errorModel in
                //: ProgressHUD.dismiss()
                GrayView.at()
                //: if succeed {
                if succeed {
                    //: self.func__showStatusBarSuccessMsg(showMsg: "Mute success".localized)
                    self.allInsidePosition(showMsg: (String(kPastId.suffix(4)) + " success").localized)
                }
                //: self.dismiss()
                self.dark()
            }
...
extension TechniqueView {
    ...
    private func iconCurveButt() {
        ...
        for i in 0 ..< 3 {
            ...
            btn.setTitleColor(UIColor.depth(), for: .normal)
            //: btn.setTitleColor(.white, for: .selected)
            btn.setTitleColor(.white, for: .selected)
            //: btn.setBackgroundColor(color: .white, forState: .normal)
            btn.asTimingPlain(color: .white, forState: .normal)
            //: btn.setBackgroundColor(color: UIColor.appThemeColor(), forState: .selected)
            btn.asTimingPlain(color: UIColor.special(), forState: .selected)
            ...
            let dayStr = (title == "1") ? (main_minimumId.replacingOccurrences(of: "classic", with: "d")).localized : (String(mainCustomerValue)).localized
            //: btn.setTitle("\(title) " + dayStr, for: .normal)
            btn.setTitle("\(title) " + dayStr, for: .normal)
            //: btn.tag = 100+i
            btn.tag = 100 + i
            //: btn.addTarget(self, action: #selector(muteBtnClick), for: .touchUpInside)
            btn.addTarget(self, action: #selector(criticalAcross), for: .touchUpInside)
