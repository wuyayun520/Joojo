...
fileprivate let appDisplayValue:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "icon_male_default" :*/
fileprivate let dataAlbumName:[UInt8] = [0x74,0x6c,0x75,0x61,0x66,0x65,0x64,0x5f,0x65,0x6c,0x61,0x6d,0x5f,0x6e,0x6f,0x63,0x69]

/*: "icon_female_default" :*/
fileprivate let dataOptionUrl:String = "global"
fileprivate let noti_aboutMessage:[Character] = ["c","o","n","_","f","e","m","a","l","e","_","d","e","f","a","u","l","t"]

/*: "level_%@" :*/
fileprivate let notiMeetHappyString:[Character] = ["l","e","v","e","l","_"]
fileprivate let app_executeTitle:String = "%@"
...
class BoldConstraint: UIView {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: appDisplayValue.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var nickLabel = UILabel().then {
        ...
        $0.font = .bagUniform(type: .Medium, fontSize: 17)
        //: $0.textColor = .white
        $0.textColor = .white
    }
    ...
    lazy var ageLabel = UILabel().then {
        ...
        $0.font = .bagUniform(type: .Regular, fontSize: 10)
        //: $0.textColor = .white
        $0.textColor = .white
    }
...
extension BoldConstraint {
    //: func updateContentWithModel(model: TPreviewInfoModel) {
    func downAttribute(model: HeroSink) {
        //: headerPic.kf.setImage(with: URL(string: model.headPic), placeholder: UIImage.placeImgSquare())
        headerPic.kf.setImage(with: URL(string: model.headPic), placeholder: UIImage.franklyBounce())
        ...
        let imgStr = model.sex == SharedOdd.male.rawValue ? String(bytes: dataAlbumName.reversed(), encoding: .utf8)! : (dataOptionUrl.replacingOccurrences(of: "global", with: "i") + String(noti_aboutMessage))
        //: sexImgView.image = UIImage(named: imgStr)
        sexImgView.image = UIImage(named: imgStr)
        //: let levelStr = String(format: "level_%@", model.level)
        let levelStr = String(format: (String(notiMeetHappyString) + app_executeTitle.capitalized), model.level)
