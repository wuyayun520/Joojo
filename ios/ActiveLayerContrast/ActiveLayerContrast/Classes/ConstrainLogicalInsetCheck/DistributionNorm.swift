...
fileprivate let app_distinctionPublishName:[UInt8] = [0xa2,0xa5,0xa2,0xbf,0xe3,0xa8,0xa4,0xaf,0xae,0xb9,0xf1,0xe2,0xeb,0xa3,0xaa,0xb8,0xeb,0xa5,0xa4,0xbf,0xeb,0xa9,0xae,0xae,0xa5,0xeb,0xa2,0xa6,0xbb,0xa7,0xae,0xa6,0xae,0xa5,0xbf,0xae,0xaf]

private func dentedEdit(month num: UInt8) -> UInt8 {
    return num ^ 203
}

/*: "Save successfully" :*/
fileprivate let dataFenderWholeId:String = "Savshelter must republic bling"
fileprivate let noti_oxygenName:[Character] = ["e"," ","s","u","c","c","e","s","s","f","u","l","l","y"]
...
class DistributionNorm: LocationCurve {
    ...
    init(momentModel: FinishEdge, index: Int, type: SaveForm) {
        ...
        if TitlePer.shared.direction == .rightToLeft {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.3) {
                ...
                self.collecView.contentOffset = CGPoint(x: curren * Int(show_licenseTitle), y: 0)
    ...
    init(images: [UIImage], index: Int, type: SaveForm) {
        //: super.init(nibName: nil, bundle: nil)
        super.init(nibName: nil, bundle: nil)
        //: myType = type
        myType = type
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: app_distinctionPublishName.map{dentedEdit(month: $0)}, encoding: .utf8)!)
    }
    ...
    override func viewDidLoad() {
        ...
        withinMoveAbove(modle: currenModel)
//        if myType == .normal {
//            setDownloadView()
//        }
    }
    ...
    lazy var collecView: UICollectionView = {
        ...
        flayout.itemSize = CGSize(width: show_licenseTitle, height: dataChainValue)
        ...
        view.frame = CGRect(x: 0, y: 0, width: show_licenseTitle, height: dataChainValue)
        ...
        view.contentOffset = CGPoint(x: pageIndex * Int(show_licenseTitle), y: 0)
        //: if myType == .normal {
        if myType == .normal {
            //: view.contentSize = CGSize(width: (currenModel.pic.count )*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (currenModel.pic.count) * Int(show_licenseTitle), height: 0)
            //: } else {
        } else {
            //: view.contentSize = CGSize(width: (photos.count)*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (photos.count) * Int(show_licenseTitle), height: 0)
        }
        //: view.register(TalkingMomentPhotosPreviewCell.self, forCellWithReuseIdentifier: TalkingMomentPhotosPreviewCell.className())
        view.register(GrainUser.self, forCellWithReuseIdentifier: GrainUser.className())
...
//    lazy var downloadBtn: KeepPurple = {
//        let btn = KeepPurple.init()
...
//        ContainPlate.func__openAlbumServiceWithBlock(true) { [weak self] (isOpen: Bool) in
//            if isOpen {
//                guard let self = self else {return}
//                guard  self.pageIndex >= 0 && self.pageIndex < self.currenModel.pic.count  else { return }
//                let model: SendSmall = self.currenModel.pic[self.pageIndex]
        ...
        PHPhotoLibrary.shared().performChanges({
            ...
            if isSuccess {
                ...
                DispatchQueue.main.async {
                    ...
                    self.allInsidePosition(showMsg: (String(dataFenderWholeId.prefix(3)) + String(noti_oxygenName)).localized)
                }
                //: } else {
            } else {
...
extension DistributionNorm: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    //: func hideTopView() {
    func concreteStart() {
...
//        downloadBtn.isHidden = isHideTopView
    ...
    func impact() {
        //: if  currenModel.sex==ChartShowSceneScientific.share.loginUserMode.sex {
        if currenModel.sex == ChartShowSceneScientific.share.loginUserMode.sex {
            //: return
            return
        }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell: GrainUser = collectionView.dequeueReusableCell(withReuseIdentifier: GrainUser.className(), for: indexPath) as! GrainUser
        //: if myType == .normal {
        if myType == .normal {
            //: let model: TalkingMomentPhotoModel = currenModel.pic[indexPath.row]
            let model: SendSmall = currenModel.pic[indexPath.row]
            //: if model.imgUrl?.count ?? 0 > 1 {
            if model.imgUrl?.count ?? 0 > 1 {
                //: cell.configUrl(url: model.imgUrl! )
                cell.contrastAcross(url: model.imgUrl!)
                //: }else {
            } else {
                //: cell.configImage(image: model.imagePic ?? UIImage.init())
                cell.secure(image: model.imagePic ?? UIImage())
            }

            //: } else {
        } else {
            //: cell.configImage(image: photos[indexPath.row])
            cell.secure(image: photos[indexPath.row])
        }
    ...
    func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        ...
        let tcell: GrainUser = cell as! GrainUser
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        let currentIndex = Int(offSetWidth / show_licenseTitle)
        //: if myType == .normal {
        if myType == .normal {
