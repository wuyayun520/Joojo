...
fileprivate let noti_untilItsFormat:[UInt8] = [0x90,0x97,0x90,0x8d,0xd1,0x9a,0x96,0x9d,0x9c,0x8b,0xc3,0xd0,0xd9,0x91,0x98,0x8a,0xd9,0x97,0x96,0x8d,0xd9,0x9b,0x9c,0x9c,0x97,0xd9,0x90,0x94,0x89,0x95,0x9c,0x94,0x9c,0x97,0x8d,0x9c,0x9d]

private func patterHurry(visible num: UInt8) -> UInt8 {
    return num ^ 249
}

/*: "Online Users" :*/
fileprivate let appBarUrl:String = "Onlinejudge selected overwhelm bean item"
...
class CrucialPost: UIView {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: noti_untilItsFormat.map{patterHurry(visible: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var dismissBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(extraBehavior), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var tipsLb: UILabel = {
        ...
        lb.text = (String(appBarUrl.prefix(6)) + " Users").localized
        //: return lb
        return lb
        //: }()
    }()
    ...
    private lazy var tableView: UITableView = {
        ...
        table.register(PriorTitleButtAwayRefreshPushRender.self, forCellReuseIdentifier: PriorTitleButtAwayRefreshPushRender.className())
...
extension CrucialPost {
    //: private func reqData() {
    private func largeIn() {
        //: TalkingAudienceManager().req_liveRoomOnlineList(chatGroupId: self.chatGroupId) { succeed, result, errorModel in
        CancelTable().alongSignificant(chatGroupId: self.chatGroupId) { succeed, result, errorModel in
...
extension CrucialPost: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let identifier = PriorTitleButtAwayRefreshPushRender.className()
        //: var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? TalkingLiveRoomOnlineListCell
        var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? PriorTitleButtAwayRefreshPushRender
        //: if cell == nil {
        if cell == nil {
            //: cell = TalkingLiveRoomOnlineListCell(style: .default, reuseIdentifier: identifier)
            cell = PriorTitleButtAwayRefreshPushRender(style: .default, reuseIdentifier: identifier)
        }
        //: cell?.setCell(model: self.DataSource[indexPath.row], index: indexPath.row)
        cell?.awakeStorage(model: self.DataSource[indexPath.row], index: indexPath.row)
        //: return cell!
        return cell!
    }
...
extension CrucialPost {
    ...
    @objc private func extraBehavior() {
        //: dismiss()
        embrace()
    }
    ...
    func pendingBy(chatGroupId: String) {
        ...
        self.contentView.frame = CGRect(x: 0, y: dataChainValue, width: show_licenseTitle, height: contentV_H)
        //: reqData()
        largeIn()
...
extension CrucialPost {
    ...
    private func spell() {
        ...
        let style = ObjectEasy()
        //: emptyView = EmptyView.init(frame: CGRect(x: 0, y: -100, width: ScreenWidth, height: contentV_H+100), style: style)
        emptyView = EmptyView(frame: CGRect(x: 0, y: -100, width: show_licenseTitle, height: contentV_H + 100), style: style)
