...
fileprivate let app_styConfirmId:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "F5F5F5" :*/
fileprivate let userSharedName:[Character] = ["F","5","F","5","F"]
fileprivate let appAppropriateFormat:String = "5"
...
class SmartView: UITableViewCell {
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        fatalError(String(bytes: app_styConfirmId.reversed(), encoding: .utf8)!)
    }
    ...
    func beginAcross(isListTableCell: Bool) {
    ...
    lazy var userInfoView: AlongView = {
    ...
    lazy var textContentView: MountDimension = {
        //: let  view = TalkingMomentTextContentView.init()
        let view = MountDimension()
        //: backView.addSubview(view)
        backView.addSubview(view)
        //: return view
        return view
    ...
    lazy var photoView: LocationView = {
        ...
        let view = LocationView(frame: CGRect(x: 0, y: 0, width: 0, height: 0), collectionViewLayout: flayout)
        //: backView.addSubview(view)
        backView.addSubview(view)
        //: return view
        return view
    ...
    lazy var bottomView: FutureData = {
    ...
    lazy var lineView: UIView = {
        ...
        view.backgroundColor = UIColor(hex: (String(userSharedName) + appAppropriateFormat.capitalized))
        //: self.contentView.addSubview(view)
        self.contentView.addSubview(view)
        //: return view
        return view
...
extension SmartView {
    ...
    func zoneWithoutShadow(model: FinishEdge) {
        //: cunrrenModel = model
        cunrrenModel = model
        //: userInfoView.isHidden = false
        userInfoView.isHidden = false
        //: userInfoView.configInfo(model: model)
        userInfoView.betweenFeature(model: model)
        //: photoView.isMyhost = self.isMyhost
        photoView.isMyhost = self.isMyhost
        //: textContentView.configText(text: model.content ?? "", isMomentDetail: model.isMomentDetail ?? false)
        textContentView.big(text: model.content ?? "", isMomentDetail: model.isMomentDetail ?? false)

        //: if model.momentType == MomentType.Photo.rawValue || model.momentType == MomentType.Live.rawValue {
        if model.momentType == BrushMaintain.Photo.rawValue || model.momentType == BrushMaintain.Live.rawValue {
...
//            }
            ...
            photoView.attachWith(model: cunrrenModel)
...
//            }
            ...
            videoView.written(model: model.videoInfo!, isTop: model.pinStatus)
        }

        //: bottomView.configModel(model: cunrrenModel)
        bottomView.timing(model: cunrrenModel)
...
//        }
    ...
    @objc func modeBy() {
        ...
        let vc = AroundSumegrity(videoPath: cunrrenModel.videoInfo!.videoUrl)
        //: if cunrrenModel.sex != ChartShowSceneScientific.share.loginUserMode.sex {
        if cunrrenModel.sex != ChartShowSceneScientific.share.loginUserMode.sex {
    ...
    func beyond() {
        ...
        if cunrrenModel.momentType == BrushMaintain.Video.rawValue {
...
extension SmartView {
    ...
    private func strength() {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(modeBy))
