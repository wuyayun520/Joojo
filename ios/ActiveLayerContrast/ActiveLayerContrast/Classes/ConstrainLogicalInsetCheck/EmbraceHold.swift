...
fileprivate let constDenyLeaveFormat:[UInt8] = [0xa7,0xac,0xa7,0xb2,0x66,0xa1,0xad,0xa2,0xa3,0xb0,0x78,0x67,0x5e,0xa6,0x9f,0xb1,0x5e,0xac,0xad,0xb2,0x5e,0xa0,0xa3,0xa3,0xac,0x5e,0xa7,0xab,0xae,0xaa,0xa3,0xab,0xa3,0xac,0xb2,0xa3,0xa2]

fileprivate func besideWrittenSuccessful(clearly num: UInt8) -> UInt8 {
...
/*: "iosSvgaEffect" :*/
fileprivate let show_buttFormat:[Character] = ["i","o","s","S","v","g","a","E","f","f","e","c"]
fileprivate let noti_tableExampleData:String = "gray"

/*: "iosEffect" :*/
fileprivate let kStairsUrl:String = "iosEffeexception prompt ye function"
fileprivate let userPinName:[Character] = ["c","t"]

/*: "entityId" :*/
fileprivate let data_monsterBudgeSectionKey:[Character] = ["e","n","t","i","t","y","I"]
fileprivate let app_rollTitle:String = "regular"
...
class EmbraceHold: UIView {
    //: var currentEffectMsgModel: TalkingAnimatMsgModel?
    var currentEffectMsgModel: SecondActive?
    ...
    var animatedView: PauseExtendView?

    //: open weak var delegate: AnimatViewStepProtocol?
    open weak var delegate: ListConsumption?
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: constDenyLeaveFormat.map{besideWrittenSuccessful(clearly: $0)}, encoding: .utf8)!)
    }
...
extension EmbraceHold {
    //: func playAnimation(infoDic: Dictionary<String, Any>) {
    func actionMagnitudeensity(infoDic: [String: Any]) {
        //: let dict = infoDic
        let dict = infoDic
        //: if dict.keys.isEmpty == false {
        if dict.keys.isEmpty == false {
            //: var iosEffect: String = dict["iosSvgaEffect"] as! String
            var iosEffect: String = dict[(String(show_buttFormat) + noti_tableExampleData.replacingOccurrences(of: "gray", with: "t"))] as! String
            //: if iosEffect.isEmptyString {
            if iosEffect.isEmptyString {
                //: iosEffect = dict["iosEffect"] as! String
                iosEffect = dict[(String(kStairsUrl.prefix(7)) + String(userPinName))] as! String
            }

            //: let entityId = dict["entityId"] as! Int
            let entityId = dict[(String(data_monsterBudgeSectionKey) + app_rollTitle.replacingOccurrences(of: "regular", with: "d"))] as! Int
            ...
            if iosEffect.isEmptyString == false {
                ...
                let successs = face(msgModel: effectMsgModel)
            }
        }
    }
    ...
    func face(msgModel: Any?) -> Bool {
        ...
        if msgModel is SecondActive {
            ...
            if giftMsgModel.animationTimes > 1 {
                ...
                tmpGiftMsgModel.featureDoingto(giftNum: 1) // 礼物个数设置为1，每次动效显示
                ...
                var repeatEfftect = SecondActive()
            ...
            if giftMsgModel.showType == SinkContent.myStery.rawValue || giftMsgModel.showType == SinkContent.king.rawValue {
        ...
        if self.currentEffectMsgModel != nil || paused {
            ...
            if effectMsgModel?.top ?? false && self.effectMsgModelArray.count > 0 {
                ...
                for i in 0 ..< self.effectMsgModelArray.count {
                    ...
                    let aModel: SecondActive = self.effectMsgModelArray[i] as! SecondActive
                    //: if aModel.top == false {
                    if aModel.top == false {
                        //: index = i
                        index = i
                ...
                if index == -1 {
                    ...
                    self.effectMsgModelArray.add(effectMsgModel ?? SecondActive())
                    //: } else {
                } else {
                    //: self.effectMsgModelArray.insert(effectMsgModel ?? TalkingAnimatMsgModel.init(), at: index)
                    self.effectMsgModelArray.insert(effectMsgModel ?? SecondActive(), at: index)
                }
            ...
            reflect(effectMsgModel: effectMsgModel!)
        }
...
extension EmbraceHold {
    ...
    func impactBy() {
        ...
        if self.currentEffectMsgModel == nil {
            ...
            if self.effectMsgModelArray.count > 0 {
                ...
                guard let removeModel = attach() else { return }
                //: firstShow(effectMsgModel: removeModel)
                reflect(effectMsgModel: removeModel)
            }
            //: } else {
        } else {
            //: self.animatedView?.resumeAnimation()
            self.animatedView?.mountWeight()
        }
    ...
    func attach() -> SecondActive? {
        ...
        if effectMsgModel is SecondActive {
            //: let lastEffectMsgModel = effectMsgModel as! TalkingAnimatMsgModel
            let lastEffectMsgModel = effectMsgModel as! SecondActive
        ...
        return effectMsgModel as? SecondActive
    }
}
...
extension EmbraceHold {
    ...
    func reflect(effectMsgModel: SecondActive) {
        ...
        self.delegate?.remote(effectView: self, obj: effectMsgModel)

        //: TalkingGiftAnimatTool.shared.loadEffectData(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
        CountTension.shared.limited(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
            ...
            self.provideInRoot(effectMsgModel: effectMsgModel, success: success, effectModel: giftModel ?? CompareModel())
            //: })
        })
    }
    ...
    func provideInRoot(effectMsgModel: SecondActive, success: Bool, effectModel: CompareModel) {
        // 资源下载完成
        //: self.delegate?.animatViewWithDidEndLoadingResource(effectView: self, obj: effectMsgModel, success: success)
        self.delegate?.mentionHero(effectView: self, obj: effectMsgModel, success: success)
        ...
        if success {
            ...
            if self.paused {
                ...
                if self.currentEffectMsgModel?.eid == effectMsgModel.eid, self.currentEffectMsgModel?.effectUrl == effectMsgModel.effectUrl {
                    ...
                    pop(effectMsgModel: effectMsgModel, model: effectModel)
                    //: return
                    return
                }
            }
    ...
    func agileAll(effectMsgModel: SecondActive, effectModel: CompareModel) {
        //: if self.animatedView != nil {
        if self.animatedView != nil {
            //: self.animatedView?.stopAnimating()
            self.animatedView?.bitVisitor()
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.versusEnable()
        ...
        className = CountTension.shared.strengthWith(oldName: className ?? "")
        ...
        let realClass = AnimatedViewClass as! PauseExtendView.Type
...
extension EmbraceHold: BackgroundDetail {
    ...
    func tillRounding(effectItemView: PauseExtendView, success: Bool) {
        ...
        self.delegate?.waitSustainable(effectView: self, obj: effectItemView.effectMsgModel ?? SecondActive(), success: success)
    }
    ...
    func zoneSmall(effectItemView: PauseExtendView) {
        ...
        self.delegate?.insteadCompare(effectView: self, obj: effectMsgModel!)
    }
}
...
extension EmbraceHold {
    ...
    func provide() {
        ...
        if animatedView != nil {
            //: self.animatedView?.stopAnimating()
            self.animatedView?.bitVisitor() // stop animation
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.versusEnable() // clean animation data
