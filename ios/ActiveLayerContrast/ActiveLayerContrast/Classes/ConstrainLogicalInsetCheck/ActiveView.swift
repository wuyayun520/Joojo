...
fileprivate let user_expressionTunPath:[UInt8] = [0x6,0x1,0x6,0x1b,0x47,0xc,0x0,0xb,0xa,0x1d,0x55,0x46,0x4f,0x7,0xe,0x1c,0x4f,0x1,0x0,0x1b,0x4f,0xd,0xa,0xa,0x1,0x4f,0x6,0x2,0x1f,0x3,0xa,0x2,0xa,0x1,0x1b,0xa,0xb]
...
class ActiveView: UIView {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: user_expressionTunPath.map{$0^111}, encoding: .utf8)!)
    }
    ...
    private lazy var progressView: PathView = {
        ...
        let v = PathView(frame: self.frame, progressBgColor: self.progressBgColor, progressPlayedColor: self.progressPlayedColor, progressCachedColor: self.progressCachedColor)
    ...
    private lazy var panGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(without(gesture:)))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var tapGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(cycleInSequence(gesture:)))
        //: return v
        return v
        //: }()
    }()
...
extension ActiveView {
    ...
    @objc func without(gesture: UIPanGestureRecognizer) {
        // 移动的距离
    ...
    @objc func cycleInSequence(gesture: UIPanGestureRecognizer) {
