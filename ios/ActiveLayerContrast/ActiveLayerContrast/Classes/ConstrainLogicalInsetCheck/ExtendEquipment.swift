...
/*: "call_bgm" :*/
fileprivate let k_cycleValue:[Character] = ["c","a","l","l","_","b"]
fileprivate let app_existName:[Character] = ["g","m"]

/*: "call_response_bgm" :*/
fileprivate let noti_availableData:[Character] = ["c","a","l","l","_","r","e","s","p","o"]
fileprivate let mainSpecifyCountValue:String = "origin convent against event viewnse_bgm"

/*: "invitePop_call_bgm" :*/
fileprivate let constSearchTellName:String = "imayvi"
fileprivate let k_muttFormat:[Character] = ["t","e","P","o","p","_","c","a","l","l","_","b","g","m"]
...
class ExtendEquipment: NSObject {
    //: var type = CallBGMPlayerType.Call
    var type = StopIndex.Call
    //: var player = AVPlayer()
    var player = AVPlayer()
    //: var filePath = ""
    var filePath = ""
    ...
    class func brushWrite(type: StopIndex) -> ExtendEquipment {
        //: let player = TalkingCallBGMPlayer()
        let player = ExtendEquipment()
        //: player.type = type
        player.type = type
        //: player.getFilePath()
        player.subtle()
...
extension ExtendEquipment {
    ...
    private func creation(notify: Notification) {
    ...
    private func isPendingReliability(notify: Notification) {
    ...
    private func distributionControl() -> Bool {
    ...
    private func subtle() {
        ...
        var fileName = (String(k_cycleValue) + String(app_existName))
        //: if self.type == .Response {
        if self.type == .Response {
            //: fileName = "call_response_bgm"
            fileName = (String(noti_availableData) + String(mainSpecifyCountValue.suffix(7)))
            //: } else if self.type == .invitePop {
        } else if self.type == .invitePop {
            //: fileName = "invitePop_call_bgm"
            fileName = (constSearchTellName.replacingOccurrences(of: "may", with: "n") + String(k_muttFormat))
        }
        // 收到通话推送要播放mp3声音，不能放到.zip中
        //: let path = SVGAEffectTool.default.getMp3Path(name: fileName)
        let path = DocumentWhite.default.stopAdd(name: fileName)
        //: self.filePath = path
        self.filePath = path
    }
}
...
extension ExtendEquipment: TXVodPlayListener {
