...
fileprivate let noti_blankContactTitle:[UInt8] = [0x20,0x73,0x69,0x20,0x73,0x75,0x74,0x61,0x74,0x53,0x6e,0x6f,0x69,0x74,0x61,0x7a,0x69,0x72,0x6f,0x68,0x74,0x75,0x61,0x20,0x6e,0x6f,0x69,0x74,0x61,0x63,0x6f,0x6c]

/*: "get location failed. error: :*/
fileprivate let k_weeklyName:String = "get linspect framework"
fileprivate let app_pageName:String = "n fwithout team never"
fileprivate let userYetSecondUrl:[Character] = ["a","i","l","e","d","."," ","e","r","r","o","r",":"]
...
class TargetContainer: NSObject {
    ...
    var locationType: ToShadow?

    // singleton
    //: private static var _instance: TalkingLocationManager?
    private static var _instance: TargetContainer?
    //: class func shared() -> TalkingLocationManager {
    class func layerDesign() -> TargetContainer {
        //: guard _instance != nil else {
        guard _instance != nil else {
            //: _instance = TalkingLocationManager()
            _instance = TargetContainer()
            //: return _instance!
            return _instance!
        }
        //: return _instance!
    ...
    override init() {
        ...
        locationManager.delegate = self
        //: requestLocation()
        afterTransformConcrete()
    }

    //: class func destroy() {
    class func messagePast() {
    ...
    func overwhelmCount(block: @escaping Locationblock) {
    ...
    func towardManual(block: @escaping Authblock) {
    ...
    func eastern() -> CLAuthorizationStatus {
        ...
        if #available(iOS 14.0, *) {
            ...
            printLog(message: String(bytes: noti_blankContactTitle.reversed(), encoding: .utf8)! + "\(status.rawValue)")
            //: return status
            return status
            //: } else {
        } else {
...
extension TargetContainer: CLLocationManagerDelegate {
...
//            guard abs(loction.timestamp.timeIntervalSinceNow) > 5 else { return }
    ...
    func locationManager(_ manager: CLLocationManager, didFailWithError error: Error) {
        ...
        printLog(message: (String(k_weeklyName.prefix(5)) + "ocatio" + String(app_pageName.prefix(3)) + String(userYetSecondUrl)) + "\(error.localizedDescription)")
    }
}
