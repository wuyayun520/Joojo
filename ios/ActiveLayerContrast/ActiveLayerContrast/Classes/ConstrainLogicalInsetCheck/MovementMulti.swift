...
fileprivate let appRefreshMagnitudeerruptStr:String = "explain assessment a noun tempDocum"
fileprivate let notiBellyMessage:String = "your he point/User/"
fileprivate let app_saltKey:String = "/"

/*: "Documents/User/record/" :*/
fileprivate let data_girlSpeakerMessage:String = "Documemonster condition"
fileprivate let user_appointmentMobileTitle:String = "User/router split"

/*: "cancelled" :*/
fileprivate let show_flexibilityValue:String = "caslowce"

/*: .wav" :*/
fileprivate let user_masterId:String = "nature wait array.wav"
...
typealias OneKeyGreetDownloadBlock = (_ model: LabelTake) -> Void

//: protocol VoiceDownloadTaskManagerDelegate: NSObjectProtocol {
protocol UsedCurve: NSObjectProtocol {
    /// 下载完成
...
extension MovementMulti {
    /// 添加下载任务
    //: func addDownloadTasks(_ tasksArray: [VoiceDownloadTaskModel]) {
    func tellStopMomentum(_ tasksArray: [LabelTake]) {
        ...
        for model in filterArray {
            ...
            if model.zipFilePath.isEmpty {
                ...
                model.zipFilePath = model.thread()
            }

            // 重编译，相对路径会变
            //: if model.zipFilePath.contains("Documents/User/voice/") {
            if model.zipFilePath.contains((String(appRefreshMagnitudeerruptStr.suffix(5)) + "ents" + String(notiBellyMessage.suffix(6)) + "voice" + app_saltKey.capitalized)) {
                //: model.zipFilePath = "\(AppCacheDefine.getChatVoicePath())\((model.zipFilePath as NSString).lastPathComponent)"
                model.zipFilePath = "\(TailTheme.theMovement())\((model.zipFilePath as NSString).lastPathComponent)"
                //: } else if model.zipFilePath.contains("Documents/User/record/") {
            } else if model.zipFilePath.contains((String(data_girlSpeakerMessage.prefix(6)) + "nts/" + String(user_appointmentMobileTitle.prefix(6)) + "ecord/")) {
                //: model.zipFilePath = "\(AppCacheDefine.getChatRecordPath())\((model.zipFilePath as NSString).lastPathComponent)"
                model.zipFilePath = "\(TailTheme.conversionInstall())\((model.zipFilePath as NSString).lastPathComponent)"
            }
            ...
            MoveBar.prepareAround(msgId: model.taskId, status: "\(MediumSuccess.prepare.rawValue)", filePath: model.zipFilePath)
    ...
    func structureColor(_ audioInfo: [String: Any], completeHandle: @escaping OneKeyGreetDownloadBlock) {
        //: var taskModel = VoiceDownloadTaskModel.getOneKeyGreetModel(audioInfo)
        var taskModel = LabelTake.resistanceStorage(audioInfo)
        ...
        if self.downloadingModelDic.keys.contains(taskModel.downloadUri) {
            ...
            taskModel = self.downloadingModelDic[taskModel.downloadUri] ?? LabelTake()
        }

        // 重编译，相对路径会变
        //: if taskModel.zipFilePath.contains("Documents/User/voice/") {
        if taskModel.zipFilePath.contains((String(appRefreshMagnitudeerruptStr.suffix(5)) + "ents" + String(notiBellyMessage.suffix(6)) + "voice" + app_saltKey.capitalized)) {
            //: taskModel.zipFilePath = "\(AppCacheDefine.getChatVoicePath())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            taskModel.zipFilePath = "\(TailTheme.theMovement())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            //: } else if taskModel.zipFilePath.contains("Documents/User/record/") {
        } else if taskModel.zipFilePath.contains((String(data_girlSpeakerMessage.prefix(6)) + "nts/" + String(user_appointmentMobileTitle.prefix(6)) + "ecord/")) {
            //: taskModel.zipFilePath = "\(AppCacheDefine.getChatRecordPath())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            taskModel.zipFilePath = "\(TailTheme.conversionInstall())\((taskModel.zipFilePath as NSString).lastPathComponent)"
        }
...
extension MovementMulti: URLSessionDataDelegate {
    // 开始
    ...
    func urlSession(_ session: URLSession, dataTask: URLSessionDataTask, didReceive response: URLResponse, completionHandler: @escaping (URLSession.ResponseDisposition) -> Void) {
        ...
        let totalBytesWritten = LabelTake.contact(path: taskModel.zipFilePath)
    ...
    func urlSession(_ session: URLSession, dataTask: URLSessionDataTask, didReceive data: Data) {
        ...
        self.delegate?.pastPrepare(model: taskModel)
    }
    ...
    func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        if error != nil || errorCode != 200 {
            ...
            if let error = error, (error as NSError).userInfo[NSLocalizedDescriptionKey] as? String == (show_flexibilityValue.replacingOccurrences(of: "slow", with: "n") + "lled") {
                //: if taskModel.taskStatus == .pause {
                if taskModel.taskStatus == .pause {
                    //: pauseTask(url: task.taskDescription ?? "", isSuspend: true)
                    appearUponStat(url: task.taskDescription ?? "", isSuspend: true)
                    //: } else if taskModel.taskStatus == .cancel {
                } else if taskModel.taskStatus == .cancel {
                    //: self.cancelTask(url: task.taskDescription ?? "")
                    self.oddAcross(url: task.taskDescription ?? "")
                }
                //: } else {
            } else {
                ...
                appearUponStat(url: task.taskDescription ?? "", isSuspend: false)
            }
            //: } else {
        } else {
            ...
            self.sectionAcross(url: task.taskDescription ?? "")
        }
...
extension MovementMulti {
    ...
    func appearUponStat(url: String, isSuspend: Bool) {
        ...
        case .chatRecord:
            ...
            let status = isSuspend ? "\(MediumSuccess.pause.rawValue)" : "\(MediumSuccess.error.rawValue)"
            //: WCDBVoiceMsgTable.db_updateVoiceMsg(msgId: taskModel.taskId, status: status)
            MoveBar.prepareAround(msgId: taskModel.taskId, status: status)
            //: if isSuspend == false {
            if isSuspend == false {
                //: self.delegate?.voiceDownloadError(model: taskModel)
                self.delegate?.half(model: taskModel)
            }
    ...
    func oddAcross(url: String) {
        ...
        case .chatRecord:
            ...
            MoveBar.thatOf(msgId: taskModel.taskId)
            //: self.delegate?.voiceDownloadCancel(model: taskModel)
            self.delegate?.graphList(model: taskModel)
    ...
    func sectionAcross(url: String) {
        ...
        let size = LabelTake.contact(path: taskModel.zipFilePath)
        //: if size > 0 {
        if size > 0 {
            ...
            if taskModel.taskTypeId == .chatRecord {
                ...
                MoveBar.prepareAround(msgId: taskModel.taskId, status: "\(MediumSuccess.finish.rawValue)")
            }

            //: let filePath = "\(taskModel.zipFilePath).wav"
            let filePath = "\(taskModel.zipFilePath)" + (String(user_masterId.suffix(4)))
            ...
            self.firstPause(at: taskModel.zipFilePath, withKeyId: taskModel.taskId, withTypeId: taskModel.taskTypeId) { _ in
                //: switch taskModel.taskTypeId {
                switch taskModel.taskTypeId {
                //: case .chatRecord:
                case .chatRecord:
                    //: self.delegate?.voiceDownloadFinish(model: taskModel)
                    self.delegate?.task(model: taskModel)
            ...
            case .chatRecord:
                ...
                MoveBar.prepareAround(msgId: taskModel.taskId, status: "\(MediumSuccess.expired.rawValue)")
                //: self.delegate?.voiceDownloadExpired(model: taskModel)
                self.delegate?.observerOp(model: taskModel)
    ...
    func firstPause(at path: String, withKeyId keyId: String, withTypeId typeId: HoldOption, complete: @escaping (Error?) -> Void) {
        //: DispatchQueue.global().async {
        DispatchQueue.global().async {
            //: if let data = try? Data(contentsOf: URL(fileURLWithPath: path)) {
            if let data = try? Data(contentsOf: URL(fileURLWithPath: path)) {
                //: let newPath = "\(path).wav"
                let newPath = "\(path)" + (String(user_masterId.suffix(4)))
                ...
                if let handle = FileHandle(forWritingAtPath: newPath) {
                    ...
                    if typeId == .chatRecord {
                        ...
                        MoveBar.prepareAround(msgId: keyId, filePath: newPath)
                    }
