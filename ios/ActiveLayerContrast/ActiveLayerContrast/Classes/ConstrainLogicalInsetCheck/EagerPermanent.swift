...
/*: "icon_randow_user_bg" :*/
fileprivate let userAdditionName:String = "icon_rancover success someone during"
fileprivate let notiClubName:[Character] = ["d","o","w","_","u","s","e","r","_","b","g"]
...
class EagerPermanent: UIView {
    // MARK: - UI Components

    //: private let leftAvatarView = AvatarView()
    private let leftAvatarView = MechanismView()
    //: private let centerAvatarView = AvatarView()
    private let centerAvatarView = MechanismView()
    //: private let rightAvatarView = AvatarView()
    private let rightAvatarView = MechanismView()
    ...
    private func roundAnnouncement() {
    ...
    private func endNo() {
        ...
        centerBgView.image = UIImage.road(name: (String(userAdditionName.prefix(8)) + String(notiClubName)))
    }
...
extension EagerPermanent {
    // MARK: - Public Methods

    //: func configure(avatars: [String]) {
    func sameClick(avatars: [String]) {
...
class MechanismView: UIView {
    ...
    private func pause() {
        ...
        imageView.fireIn(urlStr: self.imageurl ?? "")
    }
}
