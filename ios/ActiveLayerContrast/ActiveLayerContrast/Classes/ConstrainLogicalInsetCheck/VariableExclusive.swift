...
fileprivate let constShallThumbStr:[UInt8] = [0x63,0x64,0x63,0x7e,0x22,0x69,0x65,0x6e,0x6f,0x78,0x30,0x23,0x2a,0x62,0x6b,0x79,0x2a,0x64,0x65,0x7e,0x2a,0x68,0x6f,0x6f,0x64,0x2a,0x63,0x67,0x7a,0x66,0x6f,0x67,0x6f,0x64,0x7e,0x6f,0x6e]

private func conditionAlongSure(help num: UInt8) -> UInt8 {
    return num ^ 10
}

/*: "#4FAAFF" :*/
fileprivate let dataFollowData:String = "#4FAAFFstyle possible advertising"

/*: "reply" :*/
fileprivate let constTowardId:String = "resentencely"

/*: "F5F5F5" :*/
fileprivate let constMediaName:String = "Fmaymay5"
...
class VariableExclusive: UITableViewCell {
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        fatalError(String(bytes: constShallThumbStr.map{conditionAlongSure(help: $0)}, encoding: .utf8)!)
    }
...
extension VariableExclusive {
    //: @objc func onLongTapCell(sender: UIGestureRecognizer) {
    @objc func beforeFactor(sender: UIGestureRecognizer) {}
}

//: extension TalkingCommentReplyItemCell {
extension VariableExclusive {
    //: func configCell(model: TalkingCommentReplyModel) {
    func accessible(model: OldPlayBecome) {
        //: if model.allCountContent.count>0 {
        if model.allCountContent.count > 0 {
            //: contentLB.textColor = UIColor.init(hex: "#4FAAFF")
            contentLB.textColor = UIColor(hex: (String(dataFollowData.prefix(7))))
            ...
            let font = UIFont.belly(fontSize: 14)
            //: if model.replyUser.count>0 {
            if model.replyUser.count > 0 {
                //: strA = [model.nickname, " "+"reply".localized+" ", model.replyUser, "：\(model.content)"]
                strA = [model.nickname, " " + (constTowardId.replacingOccurrences(of: "sentence", with: "p")).localized + " ", model.replyUser, "：\(model.content)"]
                //: colorA = [UIColor.appValueDetailColor(), UIColor.appTitleColor(), UIColor.appValueDetailColor(), UIColor.appTitleColor()]
                colorA = [UIColor.beforePainter(), UIColor.sharkHighlightMaintain(), UIColor.beforePainter(), UIColor.sharkHighlightMaintain()]
                ...
                colorA = [UIColor.beforePainter(), UIColor.sharkHighlightMaintain()]
                //: fontA = [font, font]
                fontA = [font, font]
            }
    ...
    func spotObject(cell: UITableViewCell, tableView: UITableView, indexPath: NSIndexPath, radius: CGFloat, height: CGFloat) {
        ...
        let bounds = CGRect(x: 51, y: 0, width: show_licenseTitle - 51 - 15, height: cell.bounds.size.height)
        ...
        layer.fillColor = UIColor(hex: (constMediaName.replacingOccurrences(of: "may", with: "5F")))?.cgColor
...
extension VariableExclusive {
    ...
    private func lock() {
        ...
        let longtap = UILongPressGestureRecognizer(target: self, action: #selector(beforeFactor(sender:)))
