...
fileprivate let showAloneWrapMessage:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
class AboutView: UIView {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: showAloneWrapMessage.reversed(), encoding: .utf8)!)
    }
    ...
    override func willMove(toSuperview newSuperview: UIView?) {
        ...
        ToPresent.shared.containerView?.isHidden = false
    }
    ...
    func before() {
        ...
        UIView.animate(withDuration: 0.3, delay: 2, options: .curveEaseOut) {
            ...
            let x = (self.center.x <= show_licenseTitle * 0.5) ? -self.width : show_licenseTitle + self.width
            ...
            ToPresent.shared.containerView?.isHidden = true
        }
    }
}
...
extension AboutView {
    ...
    func appCompleteAgentHide() -> UITapGestureRecognizer {
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(self.tapGestureEvent(sender:)))
        let tap = UITapGestureRecognizer(target: self, action: #selector(self.untilM(sender:)))
    ...
    func beforeLine() {
        ...
        let pan = UIPanGestureRecognizer(target: self, action: #selector(self.refuse(sender:)))
        //: let tap = self.addTapGesture()
        let tap = self.appCompleteAgentHide()
    ...
    @objc func refuse(sender: UIPanGestureRecognizer) {
        ...
        if sender.state == .ended {
            ...
            ToPresent.shared.videoMiniPosition = CGPoint(x: recordX, y: recordY)
        }
    }

    /// 点击手势
    //: @objc private func tapGestureEvent(sender: UIPanGestureRecognizer) {
    @objc private func untilM(sender: UIPanGestureRecognizer) {
