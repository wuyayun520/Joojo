...
struct ControlRetain: HandyJSON {
    //: var videoUrl = ""
    var videoUrl = ""
    //: var imgUrl = ""
    var imgUrl = ""
    ...
    var status: JoinTitle?
}

///
//: enum MomentVideoStatus: String {
enum JoinTitle: String {
    //: case NoPass = "-1"   // 未通过
    case NoPass = "-1" // 未通过
    //: case Audit = "0"     // 审核中
    case Audit = "0" // 审核中
...
extension FinishEdge {
    //: mutating func caculateItemHeight(isPost: Bool = false) {
    mutating func aboveAddDensity(isPost: Bool = false) {
        ...
        if content!.count > 0 {
            ...
            textHeight = Int(ScientificView.everyNative(text: content!, size: CGSize(width: show_licenseTitle - 54, height: CGFloat(MAXFLOAT)), font: UIFont.bagUniform(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber == 0 ? 20 : 0))
        }

        //: if momentType == MomentType.Photo.rawValue || momentType == MomentType.Live.rawValue {
        if momentType == BrushMaintain.Photo.rawValue || momentType == BrushMaintain.Live.rawValue {
            //: if pic.count==1 {
            if pic.count == 1 {
                //: photesHeight = 212
                photesHeight = 212
