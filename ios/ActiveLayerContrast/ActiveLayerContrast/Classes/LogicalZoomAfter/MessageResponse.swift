...
fileprivate let k_thirdPath:[UInt8] = [0x34,0x33,0x34,0x29,0x75,0x3e,0x32,0x39,0x38,0x2f,0x67,0x74,0x7d,0x35,0x3c,0x2e,0x7d,0x33,0x32,0x29,0x7d,0x3f,0x38,0x38,0x33,0x7d,0x34,0x30,0x2d,0x31,0x38,0x30,0x38,0x33,0x29,0x38,0x39]

private func canvasPrimary(window num: UInt8) -> UInt8 {
    return num ^ 93
}

/*: "icon_chat_translate_nor" :*/
fileprivate let userBoaEdgeTitle:[Character] = ["i","c","o","n","_","c","h","a","t"]
fileprivate let showMiniName:String = "_transcomposition associated fade"
fileprivate let app_totalStr:String = "_norbar for"

/*: "icon_chat_translate_pre" :*/
fileprivate let appRomanName:[Character] = ["i","c","o","n","_","c","h","a","t","_","t","r","a"]
fileprivate let showAriaData:String = "significant"
fileprivate let noti_activityName:String = "slawoo"

/*: "icon_chat_miss" :*/
fileprivate let const_regionId:String = "ifellowon"
fileprivate let const_giftUrl:String = "access fender want_chat"

/*: "icon_chat_yes" :*/
fileprivate let mainMpId:[Character] = ["i","c"]
fileprivate let show_drownYetStr:[Character] = ["o","n","_","c","h","a","t","_","y","e","s"]
...
class MessageResponse: GestureLast {
    //: var transBlock: (() -> Void)?
    var transBlock: (() -> Void)?

    //: var textData: CoordinatorBasicOff?
    var textData: CoordinatorBasicOff?
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        fatalError(String(bytes: k_thirdPath.map{canvasPrimary(window: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var transLB: YYLabel = {
        ...
        label.font = UIFont.belly(fontSize: 16)
        //: return label
        return label
        //: }()
    }()
    ...
    lazy var transBtn: UIButton = {
        ...
        btn.setImage(UIImage.road(name: (String(userBoaEdgeTitle) + String(showMiniName.prefix(6)) + "late" + String(app_totalStr.prefix(4)))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_chat_translate_pre"), for: .highlighted)
        btn.setImage(UIImage.road(name: (String(appRomanName) + showAriaData.replacingOccurrences(of: "significant", with: "n") + noti_activityName.replacingOccurrences(of: "woo", with: "t") + "e_pre")), for: .highlighted)
        //: btn.addTarget(self, action: #selector(transBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(anStyleLimited), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension MessageResponse {
    ...
    override func grain(with data: ChartEmptyPop) {
        ...
        //: textData = data as? CoordinatorBasicOff
        textData = data as? CoordinatorBasicOff
        ...
        if imgPreview.count > 0, textData.msgModel.msgInfo.messageType == .MessageTypeWelcome {
            ...
            giftImagV.fireIn(urlStr: imgPreview)
        }
        ...
        //: let maxWidth = MacroDefine.getScreenWidth() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        let maxWidth = CharacteristicSine.observeData() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        // 翻译赋值相关
        ...
        if textData.translateStatus == .MsgTranslate_translating {
            ...
            self.transStatusImagV.image = UIImage.road(name: (const_regionId.replacingOccurrences(of: "fellow", with: "c") + String(const_giftUrl.suffix(5)) + "_miss"))
            ...
            self.transStatusImagV.image = UIImage.road(name: (String(mainMpId) + String(show_drownYetStr)))
        }
    ...
    func visitor() {
        ...
        let longPressGr = UILongPressGestureRecognizer(target: self, action: #selector(quickStage(tap:)))
        //: self.container.addGestureRecognizer(longPressGr)
        self.container.addGestureRecognizer(longPressGr)
    }

    //: @objc func LognCellMessageAction(tap: UILongPressGestureRecognizer) {
    @objc func quickStage(tap: UILongPressGestureRecognizer) {
        //: if tap.state == .ended {
        if tap.state == .ended {
            //: return
            return
