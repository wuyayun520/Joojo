...
fileprivate let constExclusiveMessage:[UInt8] = [0xf0,0xf7,0xf0,0xed,0xb1,0xfa,0xf6,0xfd,0xfc,0xeb,0xa3,0xb0,0xb9,0xf1,0xf8,0xea,0xb9,0xf7,0xf6,0xed,0xb9,0xfb,0xfc,0xfc,0xf7,0xb9,0xf0,0xf4,0xe9,0xf5,0xfc,0xf4,0xfc,0xf7,0xed,0xfc,0xfd]

private func shakeVisual(automat num: UInt8) -> UInt8 {
    return num ^ 153
}

/*: "FFD855" :*/
fileprivate let k_oldApprovePath:String = "FFcommon85"
fileprivate let main_helpValue:String = "5"
...
class IconFeatureDimension: UIView {
    ...
    var defaultWidth: CGFloat = 0.0 {
        ...
        didSet {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.2) {
                ...
                let minX = self.leftSilder.frame.origin.x + self.defaultWidth - k_petReduceData
                //: self.rightSlider.frame = CGRect(x: minX,
                self.rightSlider.frame = CGRect(x: minX,
                                                //: y: self.rightSlider.frame.origin.y,
                                                y: self.rightSlider.frame.origin.y,
                                                //: width: kSliderWidth,
                                                width: k_petReduceData,
                ...
                self.grassList(startX: self.videoStartX)
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: constExclusiveMessage.map{shakeVisual(automat: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var indicatorView: UIView = {
        ...
        indicator.backgroundColor = UIColor(hex: (k_oldApprovePath.replacingOccurrences(of: "common", with: "D") + main_helpValue.capitalized))
        //    indicator.alpha = 0.5
    ...
    private lazy var leftSilder: UIView = {
        ...
        leftSilder.frame = CGRect(x: main_purchaseInformationString, y: 0, width: k_petReduceData, height: self.frame.size.height)
    ...
    private lazy var rightSlider: UIView = {
        ...
        rightSlider.frame = CGRect(x: self.frame.size.width - k_petReduceData, y: 0, width: k_petReduceData, height: self.frame.size.height)
    ...
    private lazy var leftCoverView: UIView = {
        ...
        leftCoverView.backgroundColor = UIColor.doingShelter(0, 0, 0, 0.6)
        //: leftCoverView.frame = CGRect(x: 0, y: 0, width: LeftMargin, height: self.frame.size.height)
        leftCoverView.frame = CGRect(x: 0, y: 0, width: main_purchaseInformationString, height: self.frame.size.height)
    ...
    private lazy var rightCoverView: UIView = {
        ...
        rightCoverView.backgroundColor = UIColor.doingShelter(0, 0, 0, 0.6)
...
extension IconFeatureDimension {
    ...
    func grassList(startX: CGFloat) {
        ...
        rightScope = CGVector(dx: leftSilder.frame.minX + minDurationGap - k_petReduceData,
                              //: dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - kSliderWidth)
                              dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - k_petReduceData)
    }
}
...
extension IconFeatureDimension {
    ...
    @objc func countegrity(gesture: UIPanGestureRecognizer) {
        ...
        case .ended:
            ...
            self.grassList(startX: videoStartX)
    ...
    @objc func composeInstance(gesture: UIPanGestureRecognizer) {
        //: let thisTimePoint = gesture.location(in: self)
        let thisTimePoint = gesture.location(in: self)
        //: let state = gesture.state
        let state = gesture.state
...
extension IconFeatureDimension {
    ...
    private func cancelParent() {
        ...
        let leftPanGesture = UIPanGestureRecognizer(target: self, action: #selector(countegrity(gesture:)))
        ...
        let rightPanGesture = UIPanGestureRecognizer(target: self, action: #selector(composeInstance(gesture:)))
    ...
    func socialSty(_ xposition: CGFloat) {
