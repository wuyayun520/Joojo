...
fileprivate let kSharkFormat:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Full" :*/
fileprivate let user_nobodyMsg:[Character] = ["F","u","l","l"]

/*: "%@ Online" :*/
fileprivate let dataNothingPath:[Character] = ["%","@"," ","O","n","l","i","n"]
fileprivate let appPromptFormat:[Character] = ["e"]
...
class SessionOutside: UITableViewCell {
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        fatalError(String(bytes: kSharkFormat.reversed(), encoding: .utf8)!)
    }
...
extension SessionOutside {
    //: func setChatRoomListCell(model: TalkingChatRoomListModel) {
    func calendar(model: SearchionVideo) {
        //: IconBtn.setUrlImage(urlStr: model.icon)
        IconBtn.withinText(urlStr: model.icon)
        ...
        if model.isFull {
            ...
            numberLB.text = (String(user_nobodyMsg)).localized
            //: numberLB.textColor = UIColor.msgTipsColor()
            numberLB.textColor = UIColor.bubbleStrategy()
            //: }else {
        } else {
            //: numberLB.text = "%@ Online".localizedArguments(model.num)
            numberLB.text = (String(dataNothingPath) + String(appPromptFormat)).skipLive(model.num)
            //: numberLB.textColor = .appValueDetailColor()
            numberLB.textColor = .beforePainter()
        }
    }
}
