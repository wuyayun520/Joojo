...
fileprivate let noti_uponId:[UInt8] = [0x11,0x16,0x11,0xc,0x50,0x1b,0x17,0x1c,0x1d,0xa,0x42,0x51,0x58,0x10,0x19,0xb,0x58,0x16,0x17,0xc,0x58,0x1a,0x1d,0x1d,0x16,0x58,0x11,0x15,0x8,0x14,0x1d,0x15,0x1d,0x16,0xc,0x1d,0x1c]

/*: "icon_chat_start" :*/
fileprivate let appFeatureData:[Character] = ["i","c","o","n","_","c","h","a","t","_","s","t","a","r","t"]

/*: "#EBEBEB" :*/
fileprivate let show_successfullyId:[Character] = ["#","E","B","E","B","E","B"]

/*: "icon_talk_reply_voive" :*/
fileprivate let show_maleProfileValue:String = "merge always hold drawericon_ta"
fileprivate let noti_mendPaintKey:String = "production enjoy displaylk_re"
fileprivate let appAdjustValue:String = "voivexactly"

/*: "#DCDCDC" :*/
fileprivate let data_billFormat:[Character] = ["#","D","C","D","C","D","C"]

/*: "text" :*/
fileprivate let data_effectFormat:[Character] = ["t","e","x","t"]

/*: "gift" :*/
fileprivate let appAllName:String = "changeift"

/*: "Sent " :*/
fileprivate let showInstallName:[Character] = ["S","e","n","t"," "]

/*: "audio" :*/
fileprivate let mainExposeRidName:[Character] = ["a","u","d","i","o"]

/*: "img" :*/
fileprivate let app_spellUrl:String = "IMG"

/*: "video" :*/
fileprivate let user_bitBroadcastData:String = "vipartnero"
...
class AngleAboutView: UIButton {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: noti_uponId.map{$0^120}, encoding: .utf8)!)
    }
    ...
    lazy var playView: UIImageView = {
        ...
        img.image = UIImage.road(name: (String(appFeatureData)))
        //: img.isHidden = true
        img.isHidden = true
        //: return img
        return img
    ...
    lazy var audioView: UIView = {
        ...
        v.backgroundColor = UIColor(hex: (String(show_successfullyId)))
    ...
    lazy var audioImg: UIImageView = {
        ...
        img.image = UIImage.road(name: (String(show_maleProfileValue.suffix(7)) + String(noti_mendPaintKey.suffix(5)) + "ply_" + appAdjustValue.replacingOccurrences(of: "exactly", with: "e")))
        //: return img
        return img
        //: }()
    }()
    ...
    var direction: ThresholdCorner = .MsgDirectionIncoming {
        //: didSet {
        didSet {
            //: if direction == .MsgDirectionIncoming {
            if direction == .MsgDirectionIncoming {
                //: self.lineView.backgroundColor = UIColor.appValueDetailColor()
                self.lineView.backgroundColor = UIColor.beforePainter()
                //: self.nameLab.textColor = UIColor.appValueDetailColor()
                self.nameLab.textColor = UIColor.beforePainter()
                //: }else{
            } else {
                //: self.lineView.backgroundColor = UIColor(hex: "#DCDCDC")
                self.lineView.backgroundColor = UIColor(hex: (String(data_billFormat)))
                //: self.nameLab.textColor = UIColor(hex: "#DCDCDC")
                self.nameLab.textColor = UIColor(hex: (String(data_billFormat)))
            }
        }
    }
    ...
    var quoteModel: FrameworkTimePlainModel? {
        ...
        didSet {
            ...
            if quoteModel.renderType == (String(data_effectFormat)) {
                //: self.nameLab.text = quoteModel.sendName + ": " +  quoteModel.renderData.content
                self.nameLab.text = quoteModel.sendName + ": " + quoteModel.renderData.content
                //: }else if quoteModel.renderType == "gift" {
            } else if quoteModel.renderType == (appAllName.replacingOccurrences(of: "change", with: "g")) {
                //: self.nameLab.text = quoteModel.sendName + ": " + "Sent ".localized + quoteModel.renderData.giftNameLocal() + " x\(quoteModel.renderData.giftNum)"
                self.nameLab.text = quoteModel.sendName + ": " + (String(showInstallName)).localized + quoteModel.renderData.lightLength() + " x\(quoteModel.renderData.giftNum)"
                //: }else if quoteModel.renderType == "audio" {
            } else if quoteModel.renderType == (String(mainExposeRidName)) {
            ...
            } else if quoteModel.renderType == (app_spellUrl.lowercased()) {
                //: self.nameLab.text = quoteModel.sendName + ": "
                self.nameLab.text = quoteModel.sendName + ": "
                ...
                if quoteModel.renderData.previewUrl.count > 0 && isExists {
                    ...
                    self.imgView.fireIn(urlStr: quoteModel.renderData.previewUrl)
                }
                //: spaceRight = 34
                spaceRight = 34
                //: }else if quoteModel.renderType == "video" {
            } else if quoteModel.renderType == (user_bitBroadcastData.replacingOccurrences(of: "partner", with: "de")) {
                //: self.nameLab.text = quoteModel.sendName + ": "
                self.nameLab.text = quoteModel.sendName + ": "
                //: self.imgView.isHidden = false
                self.imgView.isHidden = false
                ...
                self.imgView.fireIn(urlStr: quoteModel.renderData.coverUrl)
                //: spaceRight = 34
                spaceRight = 34
            }
