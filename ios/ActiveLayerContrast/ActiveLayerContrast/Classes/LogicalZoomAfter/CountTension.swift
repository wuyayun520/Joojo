...
fileprivate let user_withName:String = "decision gate maximum integrity frameworkCach"
fileprivate let constDirectGreatId:String = "stairs would listentEffec"
fileprivate let notiDismissKey:String = "drawing simultaneously sen layer orientationheKey"

/*: "下载动画文件" :*/
fileprivate let app_receiverHusbandTitle:[Character] = ["\u{4e0b}","载","\u{52a8}","画","文"]
fileprivate let data_policyKey:String = "\u{4ef6}"

/*: "加载本地动画文件" :*/
fileprivate let noti_playerPlatString:String = "加"
fileprivate let dataSensorName:String = "载本地动画文件"

/*: "Cache_GiftEffect/" :*/
fileprivate let mainImplementId:String = "impressive pea compareCach"
fileprivate let mainDisappearAspectFormat:String = "ftEfvertical prosecution motive simultaneously"
fileprivate let k_columnMessage:String = "choice"

/*: "GJSGVA" :*/
fileprivate let kReadString:[Character] = ["G"]
fileprivate let userProblemMsg:[Character] = ["J","S","G","V","A"]

/*: "GJVap" :*/
fileprivate let k_preventionBottomName:[Character] = ["G","J","V","a","p"]

/*: "config.json" :*/
fileprivate let user_saveKey:String = "cfreshn"
fileprivate let noti_coalitionName:[Character] = ["f","i","g",".","j","s","o","n"]

/*: "app. :*/
fileprivate let mainMemberUrl:String = "app.want discount"

/*: .com" :*/
fileprivate let show_styKey:[Character] = [".","c","o","m"]

/*: "key" :*/
fileprivate let main_comprehensiveUrl:[Character] = ["k","e","y"]

/*: "zip" :*/
fileprivate let notiRecognizeFormat:[Character] = ["z","i","p"]

/*: " fail to unzip file" :*/
fileprivate let user_marDistanceTitle:String = " failtransition procedure meet tar"
fileprivate let show_actualStr:String = "measure string social grainzip "

/*: "RSA 解密失败" :*/
fileprivate let main_usedYourKey:String = "RSA 解密失败currently install"

/*: "fail to unzip script file" :*/
fileprivate let main_wholeUrl:[Character] = ["f","a","i","l"," ","t","o"," ","u","n","z","i","p"," ","s","c","r"]
fileprivate let main_bounceStr:[Character] = ["i","p","t"," ","f","i","l","e"]

/*: "gift" :*/
fileprivate let constTopBindKey:String = "giffraction"

/*: "%@%@effect_%ld" :*/
fileprivate let userStyleStr:[Character] = ["%","@","%","@","e","f","f","e","c"]
fileprivate let show_workName:String = "t_%ldmirror yellow"

/*: "%@%@effect_%ld_unzipTest/" :*/
fileprivate let show_thirdFormat:String = "%@%@"
fileprivate let notiHappyRemUrl:String = "estimated reach staget_%ld_"
fileprivate let mainAgainBoundPath:String = "pTest/republic beat"

/*: "%@%@effect_%ld_unzip/" :*/
fileprivate let user_dentedMessage:String = "mar want agent%@%@e"
fileprivate let appRoadTitle:String = "to production staff_%ld_"
fileprivate let dataRandomTiffValue:String = "maker insert rounding sound plainunzip/"
...
typealias CompletionBlock = (_ success: Bool, _ effectModel: CompareModel?) -> Void

//: typealias ErrorBlock = (_ error: NSError?) -> Void
typealias ErrorBlock = (_ error: NSError?) -> Void

//: class TalkingGiftAnimatTool: NSObject {
class CountTension: NSObject {
    //: private let AnimatCacheKey = "CacheGiftEffectCacheKey"
    private let AnimatCacheKey = (String(user_withName.suffix(4)) + "eGif" + String(constDirectGreatId.suffix(6)) + "tCac" + String(notiDismissKey.suffix(5)))

    //: @objc static let shared = TalkingGiftAnimatTool()
    @objc static let shared = CountTension()
    ...
    func limited(effectMsgModel: SecondActive, completion: @escaping CompletionBlock) {
        //: if checkNewVersionWithAnimatMsgModel(model: effectMsgModel) {
        if priorityWith(model: effectMsgModel) {
            //: printLog(message: "下载动画文件")
            printLog(message: (String(app_receiverHusbandTitle) + data_policyKey))
            //: downloadAnimat(effectMsgModel: effectMsgModel) { [weak self] error in
            awakeFlexible(effectMsgModel: effectMsgModel) { [weak self] error in
                ...
                self.titleure(effectMsgModel: effectMsgModel, complete: completion)
            }
            //: } else {
        } else {
            //: printLog(message: "加载本地动画文件")
            printLog(message: (noti_playerPlatString.capitalized + dataSensorName.capitalized))
            //: self.loadLocalAnimatConfig(effectMsgModel: effectMsgModel, complete: completion)
            self.titleure(effectMsgModel: effectMsgModel, complete: completion)
        }
    }
    ...
    func paraSufficient(fileName: String, model: SecondActive) -> String {
        //: let scriptDirectory = fetchScriptDirectory(animatMsgModel: model)
        let scriptDirectory = cap(animatMsgModel: model)
    ...
    func clusterMiss() {
        ...
        regulate(path: rowInSpell())
    }

    // 动效资源本地的缓存目录路径
    //: func fetchScriptDirectory(animatMsgModel: TalkingAnimatMsgModel) -> String {
    func cap(animatMsgModel: SecondActive) -> String {
        //: if animatMsgModel.eid == 0 {
        if animatMsgModel.eid == 0 {
            //: return String.init(format: "%@/%@/", giftEffectDirectory(), animatMsgModel.effectUrl.md5()!)
            return String(format: "%@/%@/", rowInSpell(), animatMsgModel.effectUrl.md5()!)
            //: } else {
        } else {
            //: return String.init(format: "%@/%zd/", giftEffectDirectory(), animatMsgModel.eid)
            return String(format: "%@/%zd/", rowInSpell(), animatMsgModel.eid)
        }
    }

    //: func giftEffectDirectory() -> String {
    func rowInSpell() -> String {
        //: let subPath = "Cache_GiftEffect/"
        let subPath = (String(mainImplementId.suffix(4)) + "e_Gi" + String(mainDisappearAspectFormat.prefix(4)) + "fect" + k_columnMessage.replacingOccurrences(of: "choice", with: "/"))
    ...
    func advertisingThe(filePath: String) -> String {
    ...
    func strategyKeep(model: SecondActive) -> Bool {
        //: return !checkNewVersionWithAnimatMsgModel(model: model)
        return !priorityWith(model: model)
    }

    //: func getNewAnimatClassName(oldName: String) -> String {
    func strengthWith(oldName: String) -> String {
        //: var newName = oldName
        var newName = oldName
        //: if newName.hasPrefix("GJSGVA") {
        if newName.hasPrefix((String(kReadString) + String(userProblemMsg))) {
            //: newName = String(describing: TalkingSGVAAnimationView.self)
            newName = String(describing: PauseView.self)
            //: } else if newName.hasPrefix("GJVap") {
        } else if newName.hasPrefix((String(k_preventionBottomName))) {
            //: newName = String(describing: TalkingVapAnimationView.self)
            newName = String(describing: RapidView.self)
        }
        //: return newName
        return newName
    }
...
extension CountTension {
    //: func loadLocalAnimatConfig(effectMsgModel: TalkingAnimatMsgModel, complete: @escaping CompletionBlock) {
    func titleure(effectMsgModel: SecondActive, complete: @escaping CompletionBlock) {
        //: let scriptDirectory = fetchScriptDirectory(animatMsgModel: effectMsgModel)
        let scriptDirectory = cap(animatMsgModel: effectMsgModel)
        //: let scriptPath = (scriptDirectory as NSString).appendingPathComponent("config.json")
        let scriptPath = (scriptDirectory as NSString).appendingPathComponent((user_saveKey.replacingOccurrences(of: "fresh", with: "o") + String(noti_coalitionName)))
        ...
        let dic = NSDictionary.requireOpen(withJsonString: configString!) ?? NSDictionary()
        //: let giftAmodel = TalkingGiftAnimatModel.init()
        let giftAmodel = CompareModel()
        //: let effectModel = giftAmodel.setAnimatModek(dic: dic)
        let effectModel = giftAmodel.match(dic: dic)
        ...
        className = strengthWith(oldName: className)
        //: let clsName = PodspecName + "." + className
        let clsName = user_basicData + "." + className
    ...
    func priorityWith(model: SecondActive) -> Bool {
        //: let giftPlist = UserDefaults.standard.object(forKey: AnimatCacheKey) as? Dictionary<String, Any> ?? Dictionary<String, Any>.init()
        let giftPlist = UserDefaults.standard.object(forKey: AnimatCacheKey) as? [String: Any] ?? Dictionary<String, Any>.init()
    ...
    func awakeFlexible(effectMsgModel: SecondActive, errorBlock: @escaping ErrorBlock) {
        ...
        NameProjection.clear(giftUrl: effectMsgModel.effectUrl, path: earlyFor(model: effectMsgModel)) { [weak self] t in
            //: guard let self = self else {return}
            guard let self = self else { return }
            //: DispatchQueue.global().async {
            DispatchQueue.global().async {
                //: self.afterDownloadGiftAnimat(effectMsgModel: effectMsgModel, isSucess: t, errorBlock: errorBlock)
                self.specifyOf(effectMsgModel: effectMsgModel, isSucess: t, errorBlock: errorBlock)
            }
        }
    }

    //: func afterDownloadGiftAnimat(effectMsgModel: TalkingAnimatMsgModel, isSucess: Bool, errorBlock: @escaping ErrorBlock) {
    func specifyOf(effectMsgModel: SecondActive, isSucess: Bool, errorBlock: @escaping ErrorBlock) {
        //: if !isSucess {
        if !isSucess {
            //: errorBlock(NSError.init(domain: "app.\(ReplaceUrlDomain).com", code: GiftAnimatUpdateError.DownLoadFailed.rawValue))
            errorBlock(NSError(domain: (String(mainMemberUrl.prefix(4))) + "\(data_graphicUrl)" + (String(show_styKey)), code: LogPost.DownLoadFailed.rawValue))
            //: return
            return
            ...
            let downloadPath = earlyFor(model: effectMsgModel)
            //: let unzipVerifyDir = unzipVerifyDirectory(model: effectMsgModel)
            let unzipVerifyDir = belowLite(model: effectMsgModel)
            ...
            if verifyUnzipSucc {
                ...
                printLog(message: (String(user_marDistanceTitle.prefix(5)) + " to un" + String(show_actualStr.suffix(4)) + "file"))
                //: isFailed = true
                isFailed = true
                //: errorBlock(NSError.init(domain: "app.\(ReplaceUrlDomain).com", code: GiftAnimatUpdateError.UnzipFailed.rawValue))
                errorBlock(NSError(domain: (String(mainMemberUrl.prefix(4))) + "\(data_graphicUrl)" + (String(show_styKey)), code: LogPost.UnzipFailed.rawValue))
            }
            ...
            if !isFailed {
                ...
                do {
                    ...
                    let md5Data: Data = LibraryEdge.skirtFile(NSData(contentsOfFile: keyFilePath) as Data?, segmentOccasion: user_needBeginStr)
                    ...
                    let md5 = advertisingThe(filePath: scriptZipFilePath)
                    ...
                    if decryptMD5 != md5, FileManager.default.fileExists(atPath: keyFilePath) {
                        ...
                        errorBlock(NSError(domain: (String(mainMemberUrl.prefix(4))) + "\(data_graphicUrl)" + (String(show_styKey)), code: LogPost.VerifyFailed.rawValue))
                    }
                    ...
                    printLog(message: (String(main_usedYourKey.prefix(8))))
                    //: isFailed = true
                    isFailed = true
                    //: errorBlock(NSError.init(domain: "app.\(ReplaceUrlDomain).com", code: GiftAnimatUpdateError.VerifyFailed.rawValue))
                    errorBlock(NSError(domain: (String(mainMemberUrl.prefix(4))) + "\(data_graphicUrl)" + (String(show_styKey)), code: LogPost.VerifyFailed.rawValue))
            ...
            if !isFailed {
                ...
                let unzipTmpDir = empty(model: effectMsgModel)
                ...
                //: else {
                    ...
                    printLog(message: (String(main_wholeUrl) + String(main_bounceStr)))
                    //: isFailed = true
                    isFailed = true
                    //: errorBlock(NSError.init(domain: "app.\(ReplaceUrlDomain).com", code: GiftAnimatUpdateError.UnzipFailed.rawValue))
                    errorBlock(NSError(domain: (String(mainMemberUrl.prefix(4))) + "\(data_graphicUrl)" + (String(show_styKey)), code: LogPost.UnzipFailed.rawValue))
    ...
    func regulate(path: String) -> Bool {
        //: if cleanFolder(path: path) {
        if aShadow(path: path) {
    ...
    func roundingTemporary(type: ResultTranslate) -> String {
        //: var tmpIdentify = ""
        var tmpIdentify = ""
        //: switch type {
        switch type {
        //: case .Gift:     tmpIdentify =  "gift"
        case .Gift: tmpIdentify = (constTopBindKey.replacingOccurrences(of: "fraction", with: "t"))
        //: break
        //: default:
        default:
            //: break
    ...
    func earlyFor(model: SecondActive) -> String {
        ...
        let downloadTmpPath = String(format: (String(userStyleStr) + String(show_workName.prefix(5))), NSTemporaryDirectory(), roundingTemporary(type: model.effectType), model.eid)
        //: return downloadTmpPath
        return downloadTmpPath
    }

    //: func unzipVerifyDirectory(model: TalkingAnimatMsgModel)->String {
    func belowLite(model: SecondActive) -> String {
        ...
        let unzipVerifyDirectory = String(format: (show_thirdFormat.capitalized + "effec" + String(notiHappyRemUrl.suffix(6)) + "unzi" + String(mainAgainBoundPath.prefix(6))), NSTemporaryDirectory(), roundingTemporary(type: model.effectType), model.eid)
        //: return unzipVerifyDirectory
        return unzipVerifyDirectory
    }

    //: func unzipTmpDirectory(model: TalkingAnimatMsgModel)->String {
    func empty(model: SecondActive) -> String {
        ...
        let unzipTmpDirectory = String(format: (String(user_dentedMessage.suffix(5)) + "ffect" + String(appRoadTitle.suffix(5)) + String(dataRandomTiffValue.suffix(6))), NSTemporaryDirectory(), roundingTemporary(type: model.effectType), model.eid)
