...
fileprivate let kSelectStr:[UInt8] = [0xbb,0xbc,0xbb,0xa6,0xfa,0xb1,0xbd,0xb6,0xb7,0xa0,0xe8,0xfb,0xf2,0xba,0xb3,0xa1,0xf2,0xbc,0xbd,0xa6,0xf2,0xb0,0xb7,0xb7,0xbc,0xf2,0xbb,0xbf,0xa2,0xbe,0xb7,0xbf,0xb7,0xbc,0xa6,0xb7,0xb6]

private func standardSecond(contrast num: UInt8) -> UInt8 {
    return num ^ 210
}

/*: "move" :*/
fileprivate let dataInsteadId:String = "mshadowe"
...
class MustPathView: UIView {
    ...
    var danumuMsgArray: [EvaluateReduce] = []
    //: var danumuItemViewArray: [TalkingFloatScreenItemView] = []
    var danumuItemViewArray: [OfCommand] = []

    //: let TapAction: PublishSubject<TalkingFloatingScreenModel?> = PublishSubject()
    let TapAction: PublishSubject<EvaluateReduce?> = PublishSubject()
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: kSelectStr.map{standardSecond(contrast: $0)}, encoding: .utf8)!)
    }
    ...
    func managerPair(_ danumuMsgModel: EvaluateReduce?) {
    ...
    @objc func monsterProgram(_ tapGesture: UITapGestureRecognizer) {}
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(monsterProgram(_:)))
        //: t.delegate = self
        t.delegate = self
        //: return t
        return t
...
extension MustPathView: UIGestureRecognizerDelegate {
    ...
    func margin(for danumuMsgModel: EvaluateReduce?, isFirst: Bool) {
        //: let itemView = TalkingFloatScreenItemView(model: danumuMsgModel)
        let itemView = OfCommand(model: danumuMsgModel)
        ...
        if let anim = POPBasicAnimation(propertyNamed: kPOPViewFrame) {
            ...
            itemView.pop_add(anim, forKey: (dataInsteadId.replacingOccurrences(of: "shadow", with: "ov")))
    ...
    func rank(_ isFirst: Bool) {
        ...
        if danumuMsgArray.count > 0 {
            ...
            if let model {
                ...
                margin(for: model, isFirst: isFirst)
