...
fileprivate let mainDiscountThemeData:[UInt8] = [0x74,0x73,0x74,0x69,0x35,0x7e,0x72,0x79,0x78,0x6f,0x27,0x34,0x3d,0x75,0x7c,0x6e,0x3d,0x73,0x72,0x69,0x3d,0x7f,0x78,0x78,0x73,0x3d,0x74,0x70,0x6d,0x71,0x78,0x70,0x78,0x73,0x69,0x78,0x79]

private func boxBed(skirt num: UInt8) -> UInt8 {
    return num ^ 29
}

/*: "btnJQbEAB_etihw_kcab_shadow" :*/
fileprivate let app_poolPhaseStr:[Character] = ["b","t","n","J","Q","b","E","A","B","_","e","t","i","h","w","_","k"]
fileprivate let showShotName:[Character] = ["c","a","b","_","s"]
fileprivate let kProblemKey:String = "hadnote"

/*: "Edit" :*/
fileprivate let app_stepMsg:[Character] = ["E","d","i","t"]

/*: "Finish" :*/
fileprivate let dataAdditionalData:[Character] = ["F","i","n","i","s","h"]

/*: "btnYDo2AS_oediv_play" :*/
fileprivate let show_scheduleData:[Character] = ["b","t","n","Y","D","o","2","A","S","_"]
fileprivate let notiMarPath:[Character] = ["o","e","d","i"]
fileprivate let dataRemindValue:String = "among delicate any originv_play"

/*: "Cancel" :*/
fileprivate let mainCollectionUrl:[Character] = ["C","a","n","c","e","l"]

/*: "bgQkRTbF_oediv_toast" :*/
fileprivate let app_handleName:[Character] = ["b","g","Q","k","R","T","b","F"]
fileprivate let showReflectProblemMeetData:String = "_oedivinvisible remain video"

/*: "The video length should be %@-%@ seconds. Please intercept it" :*/
fileprivate let notiVillageName:[UInt8] = [0x87,0xbb,0xb6,0xf3,0xa5,0xba,0xb7,0xb6,0xbc,0xf3,0xbf,0xb6,0xbd,0xb4,0xa7,0xbb,0xf3,0xa0,0xbb,0xbc,0xa6,0xbf,0xb7,0xf3,0xb1,0xb6,0xf3,0xf6,0x93,0xfe,0xf6,0x93,0xf3,0xa0,0xb6,0xb0,0xbc,0xbd,0xb7,0xa0,0xfd,0xf3,0x83,0xbf,0xb6,0xb2,0xa0,0xb6,0xf3,0xba,0xbd,0xa7,0xb6,0xa1,0xb0,0xb6,0xa3,0xa7,0xf3,0xba,0xa7]
...
typealias VideoEditBlock = (_ videoInfo: TagProject?) -> Void

//: let kMinVideoDuration: CGFloat = 5.0
let const_promiseTitle: CGFloat = 5.0

//: class TalkingVideoEditorController: TalkingBaseViewController {
class HandleViewController: TrainBrush {
    ...
    private var slider: ChainSpace?
    //: private var videoInfo = TalkingVideoInfoModel()
    private var videoInfo = TagProject() // 编辑结果模型
    //: private var currStyle = TalkingVideoEditorStyle.normal
    private var currStyle = MaintainTheme.normal // 当前样式
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        //: if TalkingSocketManager.shared.byWork == true {
        if ToPresent.shared.byWork == true { // 正在音视频通话，静音播放
    ...
    init(style: MaintainTheme = .normal, maxDuration: CGFloat, videoURL: URL, completionHandler: @escaping VideoEditBlock) {
        ...
        imgWidth = (show_licenseTitle - 2 * padding) / (framesInScreen * 1.0)
        //: imgHeight = 50
        imgHeight = 50
        //: duration = 0
        duration = 0
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: mainDiscountThemeData.map{boxBed(skirt: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var backBtn: UIButton = {
        ...
        v.setImage(UIImage.road(name: (String(app_poolPhaseStr) + String(showShotName) + kProblemKey.replacingOccurrences(of: "note", with: "ow"))), for: .normal)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = TitlePer.shared.direction == .rightToLeft ? .right : .left
        //: v.addTarget(self, action: #selector(backAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(untilFlexibility), for: .touchUpInside)
    ...
    private lazy var editBtn: UIButton = {
        ...
        v.setTitle((String(app_stepMsg)).localized, for: .normal)
        //: v.setTitleColor(UIColor.appThemeColor(), for: .normal)
        v.setTitleColor(UIColor.special(), for: .normal)
        //: v.titleLabel?.font = UIFont.pingfangRugularFont(fontSize: 14)
        v.titleLabel?.font = UIFont.belly(fontSize: 14)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = TitlePer.shared.direction == .rightToLeft ? .right : .left
        //: v.isHidden = true
        v.isHidden = true
//        v.addTarget(self, action: #selector(editAction(sender:)), for: .touchUpInside)
    ...
    private lazy var playBtn: UIButton = {
        ...
        v.setImage(UIImage.road(name: (String(show_scheduleData) + String(notiMarPath) + String(dataRemindValue.suffix(6)))), for: .normal)
        //: v.addTarget(self, action: #selector(playAction(sender:)), for: .touchUpInside)
        v.addTarget(self, action: #selector(modelResult(sender:)), for: .touchUpInside)
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        //: return v
        return v
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        v.setTitle((String(mainCollectionUrl)).localized, for: .normal)
    ...
    private lazy var confirmBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(faceDown), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: v.snp.makeConstraints { make in
        v.snp.makeConstraints { make in
    ...
    private lazy var toastImageView: UIImageView = {
        ...
        let v = UIImageView(image: UIImage.road(name: (String(app_handleName) + String(showReflectProblemMeetData.prefix(6)) + "_toast")))
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        ...
        lab.text = String(bytes: notiVillageName.map{$0^211}, encoding: .utf8)!.skipLive(minDuration, Int(maxduration))
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.numberOfLines = 0
        lab.numberOfLines = 0
    ...
    lazy var useVideoView: PhaseTailTap = {
        //: let v = TalkingSendIntimateInfoView(frame: CGRect.zero, type: .intimateVideo)
        let v = PhaseTailTap(frame: CGRect.zero, type: .intimateVideo)
        //: v.isHidden = true
        v.isHidden = true
        //: self.view.addSubview(v)
        self.view.addSubview(v)
...
extension HandleViewController {
    /// 返回按钮事件
    //: @objc func backAction() {
    @objc func untilFlexibility() {
    ...
    private func remarkConnection(animated: Bool = true) {
    ...
    @objc func calendarDate() {
        ...
        self.previewLayer?.frame = CGRect(x: 26, y: 26, width: show_licenseTitle - 2 * 26, height: dataChainValue - 46 - 127 - const_controlData)
        //: imgWidth = ScreenWidth / framesInScreen
        imgWidth = show_licenseTitle / framesInScreen

        //: if slider == nil {
        if slider == nil {
            //: let frame = CGRect(x: 0, y: ScreenHeight, width: ScreenWidth, height: CGFloat(imgHeight))
            let frame = CGRect(x: 0, y: dataChainValue, width: show_licenseTitle, height: CGFloat(imgHeight))
            // 创建预览区域
            ...
            slider = ChainSpace(frame: frame, url: self.videoURL!, imgWidth: imgWidth, maxDuration: maxduration, minVideoDuration: const_promiseTitle)
            ...
            slider?.snp.makeConstraints { make in
                ...
                make.bottom.equalToSuperview().offset(-60 - const_controlData)
...
//                self.playing = false
            ...
            slider?.slidingBlock = { [weak self] (direction: CountEdge) in
                //: guard let self = self else { return }
                guard let self = self else { return }
...
//                        self.playBtn.isHidden = true
    ...
    @objc func practicalTact() {
        ...
        if self.videoEditHandler != nil {
            ...
            RateFactory.shared.postOf(videoInfo: videoInfo, containerView: self.view, saveDocuments: true) { [weak self] outputPath, coverPath in
                //: guard let self = self else { return }
                guard let self = self else { return }
                //: self.popCurrentViewController()
                self.remarkConnection()
    ...
    @objc func modelResult(sender: UIButton) {
...
extension HandleViewController {
    ...
    @objc private func aroundCrucial() {
        //: self.player?.isMuted = true
        self.player?.isMuted = true
    }
}
...
extension HandleViewController {
    ...
    private func everyRegulatePic() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(definitely), name: NSNotification.Name.AVPlayerItemDidPlayToEndTime, object: item)
        ...
        self.playerTimer = self.player?.addPeriodicTimeObserver(forInterval: step, queue: DispatchQueue.main, using: { [weak self] time in
            ...
            if self.slider != nil, self.slider!.isHidden == false {
                ...
                self.slider?.overlay.socialSty(xposition)
            }
            //: }) as AnyObject?
        }) as AnyObject?
    ...
    private func byTip() {
        ...
        self.botBar.snp.makeConstraints { make in
            ...
            make.height.equalTo(48 + const_controlData)
        }
    ...
    private func beanPan() {
        // 监听应用进入后台
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(didEnterBackground),
                                               selector: #selector(unlessAccount),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(aroundCrucial),
