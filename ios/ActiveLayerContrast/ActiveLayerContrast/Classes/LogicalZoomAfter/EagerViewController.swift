...
fileprivate let notiLocalMsg:[UInt8] = [0x6d,0x6a,0x6d,0x70,0x2c,0x67,0x6b,0x60,0x61,0x76,0x3e,0x2d,0x24,0x6c,0x65,0x77,0x24,0x6a,0x6b,0x70,0x24,0x66,0x61,0x61,0x6a,0x24,0x6d,0x69,0x74,0x68,0x61,0x69,0x61,0x6a,0x70,0x61,0x60]
...
class EagerViewController: TrainBrush {
    //: var tranConfig: BasePresentTransition!
    var tranConfig: HandleIsolate!
    //: init() {
    init() {
        //: super.init(nibName: nil, bundle: nil)
        super.init(nibName: nil, bundle: nil)
        //: self.tranConfig = BasePresentTransition.init(target: self)
        self.tranConfig = HandleIsolate(target: self)
    }

    //: required init?(coder: NSCoder) {
    required init?(coder: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: notiLocalMsg.map{$0^4}, encoding: .utf8)!)
    }
}

//: class BasePresentTransition: NSObject {
class HandleIsolate: NSObject {
    ...
    var frameOfPresentedView = CGRect(x: 0, y: 0, width: show_licenseTitle, height: dataChainValue * 0.6)
    ...
    func asGo(corners: UIRectCorner, radius: CGFloat) {
...
extension HandleIsolate: UIViewControllerTransitioningDelegate {
    ...
    func presentationController(forPresented presented: UIViewController, presenting: UIViewController?, source: UIViewController) -> UIPresentationController? {
        ...
        let res = ToController(transition: self, presentedViewController: presented, presentingViewController: presenting)
        //: return res
        return res
    }
}
...
extension HandleIsolate: UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        if toVC == targetVC {
            ...
            photoEditor(transitionContext: transitionContext)
            //: } else {
        } else {
            //: dismissTransition(transitionContext: transitionContext)
            undertake(transitionContext: transitionContext)
        }
    }

    //: private func presentTransition(transitionContext: UIViewControllerContextTransitioning) {
    private func photoEditor(transitionContext: UIViewControllerContextTransitioning) {
        //: let toVC = transitionContext.viewController(forKey: .to)
        let toVC = transitionContext.viewController(forKey: .to)
    ...
    private func undertake(transitionContext: UIViewControllerContextTransitioning) {
...
class ToController: UIPresentationController, UIGestureRecognizerDelegate {
    //: var transition: BasePresentTransition!
    var transition: HandleIsolate!

    //: init(transition: BasePresentTransition, presentedViewController: UIViewController, presentingViewController: UIViewController?) {
    init(transition: HandleIsolate, presentedViewController: UIViewController, presentingViewController: UIViewController?) {
    ...
    func pastFadeTell() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(againHistory))
        //: tap.delegate = self
        tap.delegate = self
    ...
    lazy var visualView: UIView = {
        ...
        view.frame = containerView?.bounds ?? CGRect(x: 0, y: 0, width: show_licenseTitle, height: dataChainValue)
