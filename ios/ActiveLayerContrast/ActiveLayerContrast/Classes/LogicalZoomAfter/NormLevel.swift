...
fileprivate let mainOrangeId:[UInt8] = [0x1b,0x1c,0x1b,0x6,0x5a,0x11,0x1d,0x16,0x17,0x0,0x48,0x5b,0x52,0x1a,0x13,0x1,0x52,0x1c,0x1d,0x6,0x52,0x10,0x17,0x17,0x1c,0x52,0x1b,0x1f,0x2,0x1e,0x17,0x1f,0x17,0x1c,0x6,0x17,0x16]

private func progressSend(symbol num: UInt8) -> UInt8 {
    return num ^ 114
}

/*: "icon_photo_giftbg" :*/
fileprivate let showCriticalValue:String = "recon"
fileprivate let const_backgroundStr:String = "o_gifwrit share to"
fileprivate let constWooTitle:String = "tbeasy"

/*: "Send" :*/
fileprivate let appScenarioKey:[Character] = ["S","e","n","d"]

/*: "nav_back_black_nor" :*/
fileprivate let kAdAngleName:String = "bass suspend original sty cancelnav_"
fileprivate let notiDeployFormat:String = "alive here some_bla"
fileprivate let userVisibleData:String = "board"

/*: "toUid" :*/
fileprivate let showWriteSequenceId:[Character] = ["t","o","U","i","d"]

/*: "giftId" :*/
fileprivate let data_amBeforeName:String = "enhance versus insteadgiftId"

/*: "giftNum" :*/
fileprivate let k_showData:String = "G"
fileprivate let userSinceName:[Character] = ["i","f","t","N","u","m"]

/*: "unlockMsgId" :*/
fileprivate let userActivityFormat:String = "usupportlo"
fileprivate let showGlassScenarioPath:String = "pack create delay disclosegId"

/*: "Need a %@ (%@ Gold coins) to unlock" :*/
fileprivate let noti_lockPhaseMsg:[UInt8] = [0x29,0x40,0x40,0x3f,0xfb,0x3c,0xfb,0x0,0x1b,0xfb,0x3,0x0,0x1b,0xfb,0x22,0x4a,0x47,0x3f,0xfb,0x3e,0x4a,0x44,0x49,0x4e,0x4,0xfb,0x4f,0x4a,0xfb,0x50,0x49,0x47,0x4a,0x3e,0x46]

fileprivate func priceDensity(announcement num: UInt8) -> UInt8 {
...
/*: "icon_photo_lock" :*/
fileprivate let const_showUrl:[Character] = ["i","c","o","n","_","p","h","o","t","o","_","l","o","c"]
fileprivate let noti_eventCityTitle:[Character] = ["k"]

/*: "Click on the screen to view 15s" :*/
fileprivate let show_profileUrl:[UInt8] = [0xac,0x83,0x86,0x8c,0x84,0xcf,0x80,0x81,0xcf,0x9b,0x87,0x8a,0xcf,0x9c,0x8c,0x9d,0x8a,0x8a,0x81,0xcf,0x9b,0x80,0xcf,0x99,0x86,0x8a,0x98,0xcf,0xde,0xda,0x9c]

private func sisDaily(leave num: UInt8) -> UInt8 {
    return num ^ 239
}

/*: "icon_photo_burn" :*/
fileprivate let k_keepStr:String = "IC"
fileprivate let show_supporterName:[Character] = ["o","n","_","p","h","o","t","o","_","b","u","r","n"]

/*: "Already burned" :*/
fileprivate let kLoopName:String = "first suspend sty domain husbandAlre"
fileprivate let constMergeSmallUrl:String = "effectd"
...
typealias IntimatePhotoBlock = (_ model: CurrencyModel) -> Void

//: class TalkingIntimatePhotoPreviewVC: TalkingBaseViewController {
class NormLevel: TrainBrush {
    ...
    //: private var imageTotalData = WantText(direction: .MsgDirectionOutgoing)
    private var imageTotalData = WantText(direction: .MsgDirectionOutgoing) // 图片消息数据
    //: private var photoModel = AbTalkingChatMsgIntimatePhotoModel()
    private var photoModel = CurrencyModel() // 当前私密照片模型
    ...
    ///   - imageCellData: 图片模型
    ///   - toUid: 对方Id
    //: init(imageCellData: WantText, toUid: String) {
    init(imageCellData: WantText, toUid: String) {
        ...
        //: targetId = toUid
        targetId = toUid
        //: isMySelf = (imageTotalData.msgModel.user.uid == Int(ChartShowSceneScientific.share.loginUserMode.userID))
        isMySelf = (imageTotalData.msgModel.user.uid == Int(ChartShowSceneScientific.share.loginUserMode.userID))
    }
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: mainOrangeId.map{progressSend(symbol: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var giftBgView: UIImageView = {
        ...
        img.image = UIImage.road(name: (showCriticalValue.replacingOccurrences(of: "re", with: "i") + "_phot" + String(const_backgroundStr.prefix(5)) + constWooTitle.replacingOccurrences(of: "easy", with: "g")))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var giftIcon: UIImageView = {
        ...
        img.fireIn(urlStr: photoModel.giftIcon)
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var giftSendBtn: UIButton = {
        ...
        btn.setTitle((String(appScenarioKey)).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(foundCorner), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    lazy var backBtn: UIButton = {
        ...
        let img = UIImage.road(name: (String(kAdAngleName.suffix(4)) + "back" + String(notiDeployFormat.suffix(4)) + "ck_n" + userVisibleData.replacingOccurrences(of: "board", with: "or"))).withRenderingMode(.alwaysTemplate)
        //: btn.setImage(img, for: .normal)
        btn.setImage(img, for: .normal)
        //: btn.tintColor = .white
        btn.tintColor = .white
        //: btn.addTarget(self, action: #selector(backButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(medalIn), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension NormLevel {
    ...
    func springBy(giftId: String, giftNum: String) {
        ...
        params[(String(showWriteSequenceId))] = self.targetId
        //: params["giftId"] = giftId
        params[(String(data_amBeforeName.suffix(6)))] = giftId
        //: params["giftNum"] = giftNum
        params[(k_showData.lowercased() + String(userSinceName))] = giftNum
        //: params["unlockMsgId"] = self.imageTotalData.msgModel.msgInfo.msgId
        params[(userActivityFormat.replacingOccurrences(of: "support", with: "n") + "ckMs" + String(showGlassScenarioPath.suffix(3)))] = self.imageTotalData.msgModel.msgInfo.msgId
...
extension NormLevel {
    ...
    @objc private func foundCorner() {
        //: req_sendGiftMsg(giftId: photoModel.giftId, giftNum: "1")
        springBy(giftId: photoModel.giftId, giftNum: "1")
    }

    /// 阅后即焚15s倒计时开始
    //: @objc private func burnPhotoTapGesture() {
    @objc private func radioBy() {
        ...
        self.disappearPerform(urlStr: photoModel.originUrl, isMosaic: false) { [weak self] isSucceed in
            //: ProgressHUD.dismiss()
            GrayView.at()
            ...
            self.countdownTimer = Timer.scheduledTimer(withTimeInterval: 0.1, block: { [weak self] timer in
                ...
                self.progressView.jumpRemain(totalTime: 150, currTime: time)
                //: time -= 1
                time -= 1
                //: }, repeats: true)
            }, repeats: true)
...
extension NormLevel {
    ...
    private func gateTing() {
        ...
        if photoModel.lockStatus == 1 { // 锁定状态，需要送礼解锁
            ...
            self.desLab.text = String(bytes: noti_lockPhaseMsg.map{priceDensity(announcement: $0)}, encoding: .utf8)!.skipLive(photoModel.giftName, photoModel.giftPrice)
            ...
            if photoModel.burnStatus == 0 { // 未销毁
                ...
                self.intimateStatusImgV.image = UIImage.road(name: (String(const_showUrl) + String(noti_eventCityTitle)))
                //: self.desLab.text = "Click on the screen to view 15s".localized
                self.desLab.text = String(bytes: show_profileUrl.map{sisDaily(leave: $0)}, encoding: .utf8)!.localized
                // 添加手势
                //: let tap = UITapGestureRecognizer(target: self, action: #selector(burnPhotoTapGesture))
                let tap = UITapGestureRecognizer(target: self, action: #selector(radioBy))
                ...
                self.intimateStatusImgV.image = UIImage.road(name: (k_keepStr.lowercased() + String(show_supporterName)))
                //: self.desLab.text = "Already burned".localized
                self.desLab.text = (String(kLoopName.suffix(4)) + "ady burn" + constMergeSmallUrl.replacingOccurrences(of: "effect", with: "e")).localized
            }
        ...
        self.disappearPerform(urlStr: imgUrlStr, isMosaic: imgIsMosaic)
    }
    ...
    private func disappearPerform(urlStr: String, isMosaic: Bool, completionHandler: ((_ isSucceed: Bool) -> Void)? = nil) {
        ...
        guard isMySelf == false else { // 本人发的图片，展示本地缓存资源
            ...
            filePath = (filePath as NSString).appendingPathComponent(user_availableValue)
