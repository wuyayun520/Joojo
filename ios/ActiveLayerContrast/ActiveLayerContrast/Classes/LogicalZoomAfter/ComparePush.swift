...
fileprivate let const_managerProductStr:String = "\u{8bbe}\u{7f6e}普通状"
fileprivate let mainQuantityryString:String = "色时 favorite"
fileprivate let show_twoMessage:String = "\u{7a7a}间\u{7684}颜色值"

/*: "设置选中状态的文字颜色时 请使用RGB空间的颜色值" :*/
fileprivate let const_heavyLocateString:String = "\u{8bbe}置选\u{4e2d}\u{72b6}"
fileprivate let noti_mutualId:String = "颜色时 "
fileprivate let appCartId:String = "GB空\u{95f4}的颜\u{8272}值"

/*: "init(coder:) has not been implemented" :*/
fileprivate let dataSystemMsg:[UInt8] = [0xb5,0xb2,0xb5,0xa8,0xf4,0xbf,0xb3,0xb8,0xb9,0xae,0xe6,0xf5,0xfc,0xb4,0xbd,0xaf,0xfc,0xb2,0xb3,0xa8,0xfc,0xbe,0xb9,0xb9,0xb2,0xfc,0xb5,0xb1,0xac,0xb0,0xb9,0xb1,0xb9,0xb2,0xa8,0xb9,0xb8]

private func properlyCharm(hold num: UInt8) -> UInt8 {
    return num ^ 220
}

/*: "设置的下标不合法!!" :*/
fileprivate let showMonsterKey:[Character] = ["设","置","的","\u{4e0b}","\u{6807}"]
fileprivate let constRelationTitle:String = "不\u{5408}法!!"

/*: "Bags" :*/
fileprivate let userManualMsg:String = "Bagscustom allow ref"
...
open class ComparePush: UIView {
    //: open var segmentStyle: SegmentStyle
    open var segmentStyle: CellPlain
    /// 点击响应
    ...
    fileprivate lazy var normalColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let normalRgb = self.of(self.segmentStyle.normalTitleColor) {
            //: return normalRgb
            return normalRgb
            //: } else {
        } else {
            //: fatalError("设置普通状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((const_managerProductStr + "态的文字\u{989c}" + mainQuantityryString.replacingOccurrences(of: "favorite", with: "请使") + "用RGB" + show_twoMessage))
        }
        //: }()
    }()
    ...
    fileprivate lazy var selectedTitleColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let selectedRgb = self.of(self.segmentStyle.selectedTitleColor) {
            //: return selectedRgb
            return selectedRgb
            //: } else {
        } else {
            //: fatalError("设置选中状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((const_heavyLocateString + "\u{6001}的\u{6587}字" + noti_mutualId.capitalized + "请使用R" + appCartId))
        }
    ...
    fileprivate func of(_ color: UIColor) -> (r: CGFloat, g: CGFloat, b: CGFloat)? {
    ...
    public init(frame: CGRect, segmentStyle: CellPlain, titles: [String]) {
    ...
    @objc func pet(_ tapGes: UITapGestureRecognizer) {
        //: guard let currentLabel = tapGes.view as? CustomLabel else { return }
        guard let currentLabel = tapGes.view as? ShadeLabel else { return }
    ...
    public required init?(coder aDecoder: NSCoder) {
        ...
        fatalError(String(bytes: dataSystemMsg.map{properlyCharm(hold: $0)}, encoding: .utf8)!)
    }
}
...
public extension ComparePush {
    ///  对外界暴露设置选中的下标的方法 可以改变设置下标滚动后是否有动画切换效果
    //: public func selectedIndex(_ selectedIndex: Int, animated: Bool) {
    func betweenCountervalMomentum(_ selectedIndex: Int, animated: Bool) {
        //: assert(!(selectedIndex < 0 || selectedIndex >= titles.count), "设置的下标不合法!!")
        assert(!(selectedIndex < 0 || selectedIndex >= titles.count), (String(showMonsterKey) + constRelationTitle))
    ...
    func privacyFor(_ titles: [String]) {
        // 移除所有的scrollView子视图
        ...
        betweenCountervalMomentum(0, animated: true)
    }
}
...
//: extension ScrollSegmentView {
extension ComparePush {
    // 根据Titles添加相应的控件
    //: fileprivate func setupTitles() {
    fileprivate func sensorFromMaintainHave() {
        //: for (index, title) in titles.enumerated() {
        for (index, title) in titles.enumerated() {
            //: let label = CustomLabel(frame: CGRect.zero)
            let label = ShadeLabel(frame: CGRect.zero)
            //: label.tag = index
            label.tag = index
            //: label.text = title
            label.text = title
            ...
            let tapGes = UITapGestureRecognizer(target: self, action: #selector(self.pet(_:)))
            ...
            if title == (String(userManualMsg.prefix(4))).localized {
    ...
    fileprivate func robotRegulate() {
        // 设置Label位置
    ...
    fileprivate func followingWith() {
        ...
        if let firstLabel = labelsArray[0] as? ShadeLabel {
    ...
    public func read(_ currentIndex: Int) {
