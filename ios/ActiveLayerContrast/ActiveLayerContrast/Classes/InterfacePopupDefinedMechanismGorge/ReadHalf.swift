...
fileprivate let notiGradeData:[UInt8] = [0x81,0x86,0x81,0x9c,0xc0,0x8b,0x87,0x8c,0x8d,0x9a,0xd2,0xc1,0xc8,0x80,0x89,0x9b,0xc8,0x86,0x87,0x9c,0xc8,0x8a,0x8d,0x8d,0x86,0xc8,0x81,0x85,0x98,0x84,0x8d,0x85,0x8d,0x86,0x9c,0x8d,0x8c]

private func conditionRefuse(job num: UInt8) -> UInt8 {
    return num ^ 232
}

/*: "Gifts received" :*/
fileprivate let k_remindTitle:[Character] = ["G","i","f","t","s"," ","r","e","c","e","i","v"]
fileprivate let const_expectedMessage:String = "ED"
...
class ReadHalf: UITableViewCell {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: notiGradeData.map{conditionRefuse(job: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var titleLab: UILabel = {
        ...
        lb.text = (String(k_remindTitle) + const_expectedMessage.lowercased()).localized
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        collect.register(SystemView.self, forCellWithReuseIdentifier: SystemView.className())
        //: return collect
        return collect
        //: }()
    }()
...
extension ReadHalf: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: SystemView.className(), for: indexPath) as! SystemView
        //: let model: TalkingUserReceivedGiftModel = self.giftsDataArray[indexPath.row]
        let model: SymbolEnable = self.giftsDataArray[indexPath.row]
        //: cell.configGiftItemCellWithData(giftModel: model, isNewProfile)
        cell.productAir(giftModel: model, isNewProfile)
        //: return cell
        return cell
    }
