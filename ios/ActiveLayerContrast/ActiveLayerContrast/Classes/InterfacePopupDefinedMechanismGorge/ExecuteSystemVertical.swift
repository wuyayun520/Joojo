...
fileprivate let mainSatisfyName:[UInt8] = [0x65,0x62,0x65,0x78,0x24,0x6f,0x63,0x68,0x69,0x7e,0x36,0x25,0x2c,0x64,0x6d,0x7f,0x2c,0x62,0x63,0x78,0x2c,0x6e,0x69,0x69,0x62,0x2c,0x65,0x61,0x7c,0x60,0x69,0x61,0x69,0x62,0x78,0x69,0x68]

/*: "icon_chat_lock" :*/
fileprivate let user_snapMessage:String = "icprior"
fileprivate let main_presidencyId:[Character] = ["_","l","o","c","k"]

/*: "icon_chat_burned" :*/
fileprivate let appBoySignatureId:String = "pageon"
fileprivate let data_moreHideUrl:String = "document never less_burned"

/*: "icon_chat_burned_shan" :*/
fileprivate let kValueMainMsg:[Character] = ["i","c","o","n","_","c","h","a","t","_","b","u"]
fileprivate let mainMagnitudeeractionValue:String = "RNED"

/*: "Already burned" :*/
fileprivate let appUnknownValue:String = "lib"
fileprivate let user_processorStr:[Character] = ["l","r","e","a","d","y"," ","b","u","r","n","e","d"]
...
class ExecuteSystemVertical: GestureLast {
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        fatalError(String(bytes: mainSatisfyName.map{$0^12}, encoding: .utf8)!)
    }
    ...
    lazy var lockImagV: UIImageView = {
        ...
        imag.image = UIImage.road(name: (user_snapMessage.replacingOccurrences(of: "prior", with: "on") + "_chat" + String(main_presidencyId)))
    ...
    lazy var burnedImagV: KeepPurple = {
        ...
        imgV.setBackgroundImage(UIImage.road(name: (appBoySignatureId.replacingOccurrences(of: "page", with: "ic") + "_chat" + String(data_moreHideUrl.suffix(7)))), for: .normal)
        //: imgV.setImage(UIImage.BundleImageNamed(name: "icon_chat_burned_shan"), for: .normal)
        imgV.setImage(UIImage.road(name: (String(kValueMainMsg) + mainMagnitudeeractionValue.lowercased() + "_shan")), for: .normal)
        //: imgV.imageAlignment = .top
        imgV.imageAlignment = .top
        //: imgV.setTitle("Already burned".localized, for: .normal)
        imgV.setTitle((appUnknownValue.replacingOccurrences(of: "lib", with: "A") + String(user_processorStr)).localized, for: .normal)
        //: imgV.setTitleColor(.white, for: .normal)
        imgV.setTitleColor(.white, for: .normal)
        //: imgV.titleLabel?.font = UIFont.pingfangRugularFont(fontSize: 16)
        imgV.titleLabel?.font = UIFont.belly(fontSize: 16)
        //: return imgV
        return imgV
        //: }()
    }()
...
extension ExecuteSystemVertical {
    //: override func fill(with data: TCommonCellData) {
    override func grain(with data: ChartEmptyPop) {
        //: super.fill(with: data)
        super.grain(with: data)
        //: guard let imageCellData = data as? WantText else { return }
        guard let imageCellData = data as? WantText else { return }
        ...
        let intimatePhoto: CurrencyModel? = imageCellData.msgModel.msgInfo.intimatePhoto
        ...
        if intimatePhoto != nil || imageCellData.msgModel.msgInfo.customSend.isBurn == "1" || Int(imageCellData.msgModel.msgInfo.customSend.giftId) ?? 0 > 0 { // 私密照片
            ...
            statDown(data: imageCellData, urlStr: imgUrlStr, isMosaic: imgIsMosaic)

            //: } else {
        } else { // 普通图片
            //: showImageView(data: imageCellData, urlStr: nil, isMosaic: false)
            statDown(data: imageCellData, urlStr: nil, isMosaic: false)
        }
    }
    ...
    ///   - urlStr: 图片网络地址
    ///   - isMosaic: 是否需要马赛克
    //: func showImageView(data: WantText, urlStr: String?, isMosaic: Bool) {
    func statDown(data: WantText, urlStr: String?, isMosaic: Bool) {
        ...
        if data.imageUrl!.count > 0, isExists {
            ...
            DispatchQueue.global().async {
                ...
                DispatchQueue.main.async {
                    ...
                    //: if isMosaic {
                    if isMosaic {
                        //: self.photoView.image = UIImage.stop(image, logIn: 90)
                        self.photoView.image = UIImage.stop(image, logIn: 90)
                        //: } else {
                    } else {
            ...
            outputByVisit(urlStr: urlStr ?? "", isMosaic: isMosaic)
            //: } else {
        } else { // 本地图片请求网络
            ...
            if data.imageData != nil {
                ...
                self.photoView.fireIn(urlStr: data.imageUrl!)
            }
        }
    }
    ...
    func outputByVisit(urlStr: String, isMosaic: Bool) {
        //: self.photoView.setCookieToUrlImage(urlStr: urlStr) {[weak self] image in
        self.photoView.difficultDown(urlStr: urlStr) { [weak self] image in
