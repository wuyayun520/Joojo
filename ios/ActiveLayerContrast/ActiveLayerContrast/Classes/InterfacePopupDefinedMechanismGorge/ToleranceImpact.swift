...
fileprivate let data_enjoyStr:[UInt8] = [0xd6,0xd1,0xd6,0xcb,0x97,0xdc,0xd0,0xdb,0xda,0xcd,0x85,0x96,0x9f,0xd7,0xde,0xcc,0x9f,0xd1,0xd0,0xcb,0x9f,0xdd,0xda,0xda,0xd1,0x9f,0xd6,0xd2,0xcf,0xd3,0xda,0xd2,0xda,0xd1,0xcb,0xda,0xdb]

private func intervalNana(prevention num: UInt8) -> UInt8 {
    return num ^ 191
}

/*: "btn_intimate_send_bg_nor" :*/
fileprivate let appBedValue:[Character] = ["b","t","n","_","i","n","t","i","m","a","t","e","_","s","e"]
fileprivate let k_ermineFormat:String = "process ceiling limitednd_bg_nor"

/*: "Send" :*/
fileprivate let main_allTitle:[Character] = ["S","e","n","d"]
...
class ToleranceImpact: GestureLast {
    //: typealias SendBlock = () -> Void
    typealias SendBlock = () -> Void
    //: var sendBlock: SendBlock?
    var sendBlock: SendBlock?
    //: var giftData: VisitPrepare?
    var giftData: VisitPrepare?
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: data_enjoyStr.map{intervalNana(prevention: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var sendBtn: KeepPurple = {
        //: let btn = TalkingButton.init()
        let btn = KeepPurple()
        //: btn.setBackgroundImage(UIImage.BundleImageNamed(name: "btn_intimate_send_bg_nor"), for: .normal)
        btn.setBackgroundImage(UIImage.road(name: (String(appBedValue) + String(k_ermineFormat.suffix(9)))), for: .normal)
        //: btn.setTitle("Send".localized, for: .normal)
        btn.setTitle((String(main_allTitle)).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(frontContrast), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension ToleranceImpact {
    //: @objc func sendBtnClick() {
    @objc func frontContrast() {
        //: self.sendBlock?()
        self.sendBlock?()
    }
}
...
extension ToleranceImpact {
    ...
    override func grain(with data: ChartEmptyPop) {
        ...
        self.indicator.stopAnimating()
        //: self.giftData = data as? VisitPrepare
        self.giftData = data as? VisitPrepare
