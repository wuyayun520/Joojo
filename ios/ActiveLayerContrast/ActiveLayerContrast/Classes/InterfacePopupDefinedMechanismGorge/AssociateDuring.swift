...
/*: "Select a date" :*/
fileprivate let constHourUrl:String = "Selecselect contact"
fileprivate let notiDisabledString:String = "athis"
...
protocol LoopBindDelivery: NSObjectProtocol {
...
class AssociateDuring: VerticalView, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    weak var delegate: LoopBindDelivery?
    ...
    override func libWith() {
        ...
        titleString = (String(constHourUrl.prefix(5)) + "t a d" + notiDisabledString.replacingOccurrences(of: "his", with: "e")).localized
        ...
        cropAlbum(year: defaultYear!, month: defaultMonth!, day: defaultDay!)
    }

    //: func setDefaultDate(year: Int, month: Int, day: Int) {
    func cropAlbum(year: Int, month: Int, day: Int) {
        //: currentYear = year
        currentYear = year
        //: if year == defaultYear {
        if year == defaultYear {
...
//        }
    ...
    override func domainFront(button: UIButton) {
        ...
        if let day = selectDay {
            ...
            delegate?.modifyBegin(year: selectYear!, month: selectMonth!, day: day)
            //: } else {
        } else {
            //: delegate?.pickDateView(year: selectYear!, month: selectMonth!, day: 0)
            delegate?.modifyBegin(year: selectYear!, month: selectMonth!, day: 0)
        }
    }
        ...
        if isShowDay {
            ...
            if component == 0 {
                ...
                if currentYear == yearSum + minShowYear - 1 {
                    ...
                    return atProcessingHoney(year: year, month: month)
                }
                //: } else {
            } else {
                //: return self.yearSum
    ...
    func atProcessingHoney(year: Int, month: Int) -> Int {
