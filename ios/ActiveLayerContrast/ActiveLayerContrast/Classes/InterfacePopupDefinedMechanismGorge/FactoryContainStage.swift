...
/*: "http://stat. :*/
fileprivate let show_frequencyString:String = "bindtp"
fileprivate let appWedMsg:String = "tat.open require expression double album"

/*: .com" :*/
fileprivate let noti_awayFormat:[Character] = [".","c","o","m"]

/*: "http://" :*/
fileprivate let notiObtainTitle:[Character] = ["h","t","t","p",":","/","/"]

/*: "https://" :*/
fileprivate let notiBlueDrawFormat:[Character] = ["h","t","t","p","s",":","/"]
fileprivate let mainAdditionPleaseMessage:[Character] = ["/"]

/*: "http://m. :*/
fileprivate let appHappyFeedbackTitle:String = "http://intensity infrastructure manager super enhance"
fileprivate let show_galleryString:[Character] = ["m","."]

/*: "http:// :*/
fileprivate let appDeliveryData:[Character] = ["h","t","t","p",":","/","/"]

/*: "3,90" :*/
fileprivate let noti_makeWeeklyMessage:String = "3,fast0"

/*: "1990-01-01" :*/
fileprivate let showForeYeKey:String = "1990-"

/*: "MM-dd-yyyy" :*/
fileprivate let main_dirtMessage:String = "MM-dobject request exclusive execute everyone"
fileprivate let const_interruptValue:String = "d-yyyym span bit long"
...
class StrategyArray: NSObject {
    ...
    override init() {
        ...
        self.requestServer = TotalSearch.share.serverUrlStr
        //: self.showErrorStatusBar = true
        self.showErrorStatusBar = true
    }

    //: func paramsContainBinaryObject() -> Bool {
    func nextAndAspect() -> Bool {
...
struct SharedImmediate: HandyJSON {
    ...
    var CSConfig = SpeedChannel() // 私聊官方客服提示消息模型
    ...
    var reportDomain: String {
        ...
        if statDomain.isEmptyString {
            ...
            return (show_frequencyString.replacingOccurrences(of: "bind", with: "ht") + "://s" + String(appWedMsg.prefix(4))) + "\(data_graphicUrl)" + (String(noti_awayFormat))
            //: } else {
        } else {
            //: if !statDomain.hasPrefix("http://") && !statDomain.hasPrefix("https://") {
            if !statDomain.hasPrefix((String(notiObtainTitle))) && !statDomain.hasPrefix((String(notiBlueDrawFormat) + String(mainAdditionPleaseMessage))) {
                //: return "http://".appending(statDomain)
                return (String(notiObtainTitle)).appending(statDomain)
                //: }else {
            } else {
                //: return statDomain
                return statDomain
    ...
    var urlH5Domain: String {
        ...
        if urlDomain.isEmpty {
            ...
            return (String(appHappyFeedbackTitle.prefix(7)) + String(show_galleryString)) + "\(data_graphicUrl)" + (String(noti_awayFormat))
            //: } else {
        } else {
            //: return "http://\(urlDomain)"
            return (String(appDeliveryData)) + "\(urlDomain)"
        }
    }
...
public class NaturalRestriction: NSObject, HandyJSON {
    ...
    var vipSetting = [HeroModel]() // vip皮肤
    //: var gameShowBit = 0
...
@objcMembers public class SizeInput: NSObject, HandyJSON {
    //: required public override init() {
    override public required init() {}
    ...
    var interest: [HeroFlow]?
    //: var aboutMe: Array<UserSeleteTagModel>?
    var aboutMe: [HeroFlow]?
    ...
    var callPopupSetting = (noti_makeWeeklyMessage.replacingOccurrences(of: "fast", with: "9")) // 表示来电弹窗配置，如”3,90”表示3s后弹第一次，后续90s，返回”0,0”表示不弹窗
    //: var callTabSwitch = 0
    ...
    public lazy var age: String = {
        ...
        let birthdayDate = NSDate.rejectIdentity(self.birthday ?? (showForeYeKey.capitalized + "01-01"), dateFormat: (String(main_dirtMessage.prefix(4)) + String(const_interruptValue.prefix(6))))
        //: guard let currentYear = NSDate.getCurrentDateComponents().year else { return "0" }
        guard let currentYear = NSDate.insteadCharm().year else { return "0" }
...
public struct HeroFlow: HandyJSON, Equatable {
    //: var tag_id: String?
    var tag_id: String?
    //: var tag_name = ""
    var tag_name = ""
    ...
    func dirt(compareTo: HeroFlow) -> Bool {
        //: return
        return
