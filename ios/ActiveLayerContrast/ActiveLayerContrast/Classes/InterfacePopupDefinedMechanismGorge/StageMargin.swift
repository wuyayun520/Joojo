...
/*: "gifts" :*/
fileprivate let constPromptVeryData:String = "giflover"

/*: "vipGifts" :*/
fileprivate let kToAboveTitle:[Character] = ["v","i"]
fileprivate let userAwayOriginalValue:[Character] = ["p","G","i","f","t","s"]

/*: "packGifts" :*/
fileprivate let userVideoMessage:String = "beside notice limited dive significantpackG"

/*: "giftSendItem" :*/
fileprivate let showRemainCompleteValue:String = "package"
fileprivate let showThroughId:String = "iftSeexist professional conversation"
...
class StageMargin: NSObject {
    ...
    private var cacheDict = [String: [GreenAttachImage: [Any]]]()
    // 数据锁
    ...
    private var giftType = PointExecute.normal

    //: static let share = TalkingChatGiftManager()
    static let share = StageMargin()
...
extension StageMargin {
    ...
    func per(type: PointExecute = .normal, isRefresh: Bool = false, completion: @escaping () -> Void) {
        //: TalkingChatGiftManager.share.giftType = type
        StageMargin.share.giftType = type
        //: if isRefresh == false {
        if isRefresh == false {
            //: let dataDict = cacheDict[type.rawValue]
            let dataDict = cacheDict[type.rawValue]
            //: let hotArr = dataDict?[GiftDataType.Hot] ?? [Any]()
            let hotArr = dataDict?[GreenAttachImage.Hot] ?? [Any]()
        ...
        if type == .groupChat {
            ...
            StageMargin.share.location(completion: { isSucceed in
            ...
            StageMargin.share.nearDeploy(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            StageMargin.share.onSomeone(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
    ...
    private func onSomeone(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = StageMargin.share.giftType
        //: TalkingChatRequestTool.req_ChatGiftList(type: type.rawValue) { succeed, result, errorModel in
        NameProjection.launchTo(type: type.rawValue) { succeed, result, errorModel in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
                //: var dataDict = [GiftDataType: [Any]]()
                var dataDict = [GreenAttachImage: [Any]]()
                // hot礼物
                //: if let array: Array = data["gifts"] as? Array<Any> {
                if let array: Array = data[(constPromptVeryData.replacingOccurrences(of: "lover", with: "ts"))] as? [Any] {
                    //: var hotArr = [Any]()
                    var hotArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<TierBrush>.deserialize(from: array as? Array) {
                        //: hotArr.append(contentsOf: datas as [Any])
                        hotArr.append(contentsOf: datas as [Any])
                    }
                    //: dataDict[GiftDataType.Hot] = hotArr
                    dataDict[GreenAttachImage.Hot] = hotArr
                }

                // vip礼物
                //: if let array: Array = data["vipGifts"] as? Array<Any> {
                if let array: Array = data[(String(kToAboveTitle) + String(userAwayOriginalValue))] as? [Any] {
                    //: var vipArr = [Any]()
                    var vipArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<TierBrush>.deserialize(from: array as? Array) {
                    ...
                    dataDict[GreenAttachImage.Vip] = vipArr
                }

                // Bags礼物
                //: if let array: Array = data["packGifts"] as? Array<Any> {
                if let array: Array = data[(String(userVideoMessage.suffix(5)) + "ifts")] as? [Any] {
                    //: var bagsArr = [Any]()
                    var bagsArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<TierBrush>.deserialize(from: array as? Array) {
                    ...
                    dataDict[GreenAttachImage.Bags] = bagsArr
                }
                ...
                var firstModel = BackgroundPlatform()
                //: firstModel.num = 1
                firstModel.num = 1
                ...
                let sendItem: Array = data[(showRemainCompleteValue.replacingOccurrences(of: "package", with: "g") + String(showThroughId.prefix(5)) + "ndItem")] as! [Any]
                ...
                for i in 0 ..< sendItem.count {
                    ...
                    if let configModel = JSONDeserializer<BackgroundPlatform>.deserializeFrom(dict: sendItem[i] as? [String: Any], designatedPath: nil) {
    ...
    private func location(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = StageMargin.share.giftType
        //: TalkingChatRequestTool.req_ChatRoomGiftList(type: type.rawValue) { succeed, result, errorModel in
        NameProjection.mainOff(type: type.rawValue) { succeed, result, errorModel in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func architectureThroughCommand(params: [String: Any], completion: @escaping (_ data: [Any]?) -> Void) {
        //: TalkingChatRequestTool.req_getChatRoomMembers(params: params, completion: { succeed, result, errorModel in
        NameProjection.strategyAcrossCritical(params: params, completion: { succeed, result, errorModel in
            //: if succeed {
            if succeed {
                ...
                for dict in data {
                    ...
                    if let memberModel = JSONDeserializer<RomanMagnitudeerval>.deserializeFrom(dict: dict as? [String: Any], designatedPath: nil) {
    ...
    private func nearDeploy(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.live
        let type = PointExecute.live
        //: TalkingChatRequestTool.req_LiveGiftList(type: type.rawValue) { succeed, result, errorModel in
        NameProjection.everyReady(type: type.rawValue) { succeed, result, errorModel in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func finishEvery(perPageCount: Int, dataType: GreenAttachImage) -> Int {
    ...
    func actualFor(dataType: GreenAttachImage) -> [Any] {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
    ...
    func smartTo() -> [Any] {
        ...
        if StageMargin.share.giftType == .groupChat {
            //: return chatRoomSendItems
            return chatRoomSendItems
            //: } else if TalkingChatGiftManager.share.giftType == .live {
        } else if StageMargin.share.giftType == .live {
    ...
    func comprehensiveTar(dataType: GreenAttachImage, dataArr: [Any]) {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
...
extension StageMargin {
    ...
    private func rebuildCrush() {
        // 刷新礼物背包
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(receiveNewGiftPackage),
                                               selector: #selector(allow),
    ...
    @objc private func allow() {
        ...
        temporaryCounterrupt(showRed: true)
    }
}
...
extension StageMargin {
    ...
    func temporaryCounterrupt(showRed: Bool = false) {
        //: TalkingChatGiftManager.share.showBagsRed = showRed
        StageMargin.share.showBagsRed = showRed
        //: cacheDict.removeAll()
        cacheDict.removeAll()
    }
}
