...
/*: "bannerList" :*/
fileprivate let show_offerUniformMsg:String = "baverticale"
fileprivate let appPerceptStr:[Character] = ["r","L","i","s","t"]

/*: "icon_me_settings" :*/
fileprivate let mainMeetId:[Character] = ["i","c","o","n","_","m","e","_","s","e","t","t","i","n","g"]
fileprivate let k_globalMessage:[Character] = ["s"]

/*: "icon_me_tc" :*/
fileprivate let appBeyondCapData:[Character] = ["i","c","o","n","_","m","e","_"]
fileprivate let constArenaString:String = "tstep"
...
class LastCapacityNumber: TrainBrush {
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(partyResource),
    ...
    private lazy var tableView: UITableView = {
        ...
        table.register(ActionComponent.self, forCellReuseIdentifier: ActionComponent.className())
        //: table.register(TalkingMeBannerCell.self, forCellReuseIdentifier: TalkingMeBannerCell.className())
        table.register(BothFile.self, forCellReuseIdentifier: BothFile.className())
        //: table.register(TalkingMeTwoColumnsCell.self, forCellReuseIdentifier: TalkingMeTwoColumnsCell.className())
        table.register(UndertakeMenu.self, forCellReuseIdentifier: UndertakeMenu.className())
        //: table.register(TalkingMeThreeColumnsCell.self, forCellReuseIdentifier: TalkingMeThreeColumnsCell.className())
        table.register(BlocPresentView.self, forCellReuseIdentifier: BlocPresentView.className())
        //: table.register(TalkingUserCenterCell.self, forCellReuseIdentifier: TalkingUserCenterCell.className())
        table.register(ListenerView.self, forCellReuseIdentifier: ListenerView.className())
    ...
    lazy var bannerListData: [PushMaximum] = //: return Array<PushMaximum>()
        .init()
    //: }()
}
...
extension LastCapacityNumber {
    ...
    func birthdayValid() {
        ...
        AdvancedCountegrationSpell().sis(position: 2) { [weak self] succeed, result, errorModel in
            //: guard let self = self else { return }
            guard let self = self else { return }
            ...
            let array = JSON(result[(show_offerUniformMsg.replacingOccurrences(of: "vertical", with: "nn") + String(appPerceptStr))] ?? []).arrayValue
    ...
    private func marFromCap() {
        //: tupleData = [(.My_Settings, "icon_me_settings")]
        tupleData = [(.My_Settings, (String(mainMeetId) + String(k_globalMessage)))]
        //: if ChartShowSceneScientific.share.appUserConfigMode.showTaskCenter {
        if ChartShowSceneScientific.share.appUserConfigMode.showTaskCenter {
            //: tupleData.insert((.Task_Center, "icon_me_tc"), at: 0)
            tupleData.insert((.Task_Center, (String(appBeyondCapData) + constArenaString.replacingOccurrences(of: "step", with: "c"))), at: 0)
        }
    }
}
...
extension LastCapacityNumber {
    //: @objc func pushEdit() {
    @objc func partyResource() {
        //: let editvc = TalkingEditProfilesVC()
        let editvc = WouldGesture()
...
extension LastCapacityNumber: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        guard indexPath.section == 0 else {
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: ListenerView.className(), for: indexPath) as! ListenerView
            ...
            cell.acceptOf(title: data.0.rawValue, icon: data.1, row: indexPath.row, isLast: isLast)
        ...
        case 0:
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: ActionComponent.className(), for: indexPath) as! ActionComponent
            //: cell.setViewData()
            cell.serverConnect()
            //: return cell
            return cell
        ...
        case 1:
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: BothFile.className(), for: indexPath) as! BothFile
            //: cell.setViewData(bannerData: self.bannerListData)
            cell.nana(bannerData: self.bannerListData)
            //: return cell
            return cell
        //: case 2:
        case 2:
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: UndertakeMenu.className(), for: indexPath) as! UndertakeMenu
            //: cell.setViewData()
            cell.transferLarge()
            //: return cell
            return cell
        ...
        case 3:
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: BlocPresentView.className(), for: indexPath) as! BlocPresentView
            //: cell.setViewData()
            cell.notUpon()
            //: return cell
            return cell
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        //: case .Task_Center:
        case .Task_Center:
            //: BeforeAssociate.share.func__pushToWebVC(webViewType: .TaskCenter)
            BeforeAssociate.share.successfullyHi(webViewType: .TaskCenter)
