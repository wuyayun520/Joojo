...
fileprivate let data_bothData:[UInt8] = [0xc5,0xca,0xc5,0xd0,0x84,0xbf,0xcb,0xc0,0xc1,0xce,0x96,0x85,0x7c,0xc4,0xbd,0xcf,0x7c,0xca,0xcb,0xd0,0x7c,0xbe,0xc1,0xc1,0xca,0x7c,0xc5,0xc9,0xcc,0xc8,0xc1,0xc9,0xc1,0xca,0xd0,0xc1,0xc0]

fileprivate func lineCapacity(better num: UInt8) -> UInt8 {
...
/*: "%@.mp4" :*/
fileprivate let user_linkWantValue:String = "mystery"
fileprivate let kStrengthReasonName:String = "@.mp4popular sink receiver movement chemist"

/*: "http" :*/
fileprivate let const_sourceMessage:String = "HTTP"
...
class RapidView: PauseExtendView {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: data_bothData.map{lineCapacity(better: $0)}, encoding: .utf8)!)
    }
    ...
    override func tipAcross() {
        ...
        let path = CountTension.shared.paraSufficient(fileName: self.giftEffectModel?.mainFile ?? "", model: self.effectMsgModel ?? SecondActive())

        //: if  path.count > 0 {
        if path.count > 0 {
            //: let mp4Path = String.init(format: "%@.mp4", path)
            let mp4Path = String(format: (user_linkWantValue.replacingOccurrences(of: "mystery", with: "%") + String(kStrengthReasonName.prefix(5))), path)
            ...
            self.delegate?.zoneSmall(effectItemView: self)
        }
    }

    //: override func stopAnimating() {
    override func bitVisitor() {
        //: self.vapPlayerView?.stopHWDMP4()
        self.vapPlayerView?.stopHWDMP4()
    }

    //: override func cleanAnimating() {
    override func versusEnable() {
...
extension RapidView: HWDMP4PlayDelegate {
    ...
    func loadVapImage(withURL urlStr: String!, context: [AnyHashable: Any]!, completion completionBlock: VAPImageCompletionBlock!) {
        ...
        DispatchQueue.main.async {
            ...
            if urlStr.hasPrefix((const_sourceMessage.lowercased())) {
