...
fileprivate let notiDecideValue:[UInt8] = [0xf3,0xf4,0xf3,0xee,0xb2,0xf9,0xf5,0xfe,0xff,0xe8,0xa0,0xb3,0xba,0xf2,0xfb,0xe9,0xba,0xf4,0xf5,0xee,0xba,0xf8,0xff,0xff,0xf4,0xba,0xf3,0xf7,0xea,0xf6,0xff,0xf7,0xff,0xf4,0xee,0xff,0xfe]

private func rejectProsecution(find num: UInt8) -> UInt8 {
    return num ^ 154
}
...
typealias SlidingHandler = (_ direction: CountEdge) -> Void
...
class ChainSpace: UIView {
    //: var cropStart: CGFloat {
    var cropStart: CGFloat {
        //: let offset = overlay.startOffset - (LeftMargin - self.contentView.contentOffset.x)
        let offset = overlay.startOffset - (main_purchaseInformationString - self.contentView.contentOffset.x)
    ...
    init(frame: CGRect, url: URL, imgWidth: CGFloat, maxDuration: CGFloat, minVideoDuration: CGFloat) {
        ...
        //: setupSubviews()
        administrativeAcross()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: notiDecideValue.map{rejectProsecution(find: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIScrollView = {
        ...
        while time <= self.duration {
            ...
            let image = self.boaTension(point: time)
            //: imv.image = image
            imv.image = image
    ...
    lazy var overlay: IconFeatureDimension = {
        ...
        let overlay = IconFeatureDimension(frame: self.bounds, maxCropLen: maxWidth, minCropLen: minWidth)
...
extension ChainSpace {
    ...
    func largeUniform() {
        ...
        self.overlay.grassList(startX: main_purchaseInformationString - x)
    }
    ...
    private func boaTension(point: CGFloat) -> UIImage {
...
extension ChainSpace: UIScrollViewDelegate {
    //: func scrollViewDidScroll(_ scrollView: UIScrollView) {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        //: var offset = scrollView.contentOffset
        var offset = scrollView.contentOffset
        //: let minx = LeftMargin - overlay.startOffset
        let minx = main_purchaseInformationString - overlay.startOffset
    ...
    func scrollViewWillBeginDragging(_ scrollView: UIScrollView) {
        ...
        self.overlay.usedOf(isShow: false)
    }
    ...
    func scrollViewDidEndDragging(_ scrollView: UIScrollView, willDecelerate decelerate: Bool) {
        ...
        self.overlay.usedOf(isShow: true)
    }
...
extension ChainSpace {
    ...
    private func administrativeAcross() {
        ...
        self.contentView.snp.makeConstraints { make in
            ...
            make.leading.equalToSuperview().offset(main_purchaseInformationString)
