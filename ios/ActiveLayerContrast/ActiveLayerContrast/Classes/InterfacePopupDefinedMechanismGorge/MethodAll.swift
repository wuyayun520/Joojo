...
fileprivate let userActivityMsg:[UInt8] = [0x86,0x8b,0x86,0x91,0x45,0x80,0x8c,0x81,0x82,0x8f,0x57,0x46,0x3d,0x85,0x7e,0x90,0x3d,0x8b,0x8c,0x91,0x3d,0x7f,0x82,0x82,0x8b,0x3d,0x86,0x8a,0x8d,0x89,0x82,0x8a,0x82,0x8b,0x91,0x82,0x81]

fileprivate func processHalf(comment num: UInt8) -> UInt8 {
...
class MethodAll: UIView {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: userActivityMsg.map{processHalf(comment: $0)}, encoding: .utf8)!)
    }
...
extension MethodAll {
    ...
    override func hitTest(_ point: CGPoint, with event: UIEvent?) -> UIView? {
        ...
        for subview in self.subviews {
            ...
            let itemview: WhiteClip = subview as! WhiteClip
            ...
            if #available(iOS 11.0, *) {
                ...
                if itemview.avatarImage.frame.contains(point) {
                    //: BeforeAssociate.share.func__pushToUserDetailVC(uid: "\(itemview.giftMsgModel.fromUid ?? 0)")
                    BeforeAssociate.share.outsideTechnique(uid: "\(itemview.giftMsgModel.fromUid ?? 0)")
                    //: break
                    break
                }
                //: } else {
    ...
    func plainFlag(model: IndexInput) {
        //  1.相同礼物时则连击
        //: for  subview in self.subviews {
        for subview in self.subviews {
            //: let itemview: TalkingGiftTrackItemView = subview as! TalkingGiftTrackItemView
            ...
            if tensionProgram(giftMsgModel: itemview.giftMsgModel, withGiftMsgModel: model) {
                //: itemview.comboHitWith(giftMsgModel: model)
                itemview.tillBottom(giftMsgModel: model)
                //: itemview.setGiftMsgModel(model: model)
                itemview.enableicialNorm(model: model)
                //: return
                return
            }
        }
        ...
        for index in 0 ... 2 {
            ...
            let subView: WhiteClip? = self.viewWithTag(tag) as! WhiteClip?
            //: if subView == nil {
            if subView == nil {
                //: self.addNewGiftDanumWith(giftMsgModel: model, atIndex: index)
                self.amongTier(giftMsgModel: model, atIndex: index)
                //: return
                return
            }
        }
        ...
        for index in 0 ... 2 {
            ...
            if index == 1 || index == 2 {
                ...
                let subView1: WhiteClip = self.viewWithTag(tag) as! WhiteClip
                ...
                subView1.mechanismChain(frame: frame)
    ...
    func tensionProgram(giftMsgModel: IndexInput, withGiftMsgModel: IndexInput) -> (Bool) {
...
//        subView.showUserInfoViewBlock   = self.showUserInfoViewBlock
        ...
        subView.enableicialNorm(model: giftMsgModel)
        //: self.addSubview(subView)
        self.addSubview(subView)

        //: subView.showGiftAnimate(model: giftMsgModel, animate: true)
        subView.visit(model: giftMsgModel, animate: true)
    }
}
