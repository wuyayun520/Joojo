...
/*: "/TXCache" :*/
fileprivate let data_reasonDebateSunKey:String = "movie"
fileprivate let showOkString:String = "form recording temporary heTXCache"
...
protocol NativeCalculate: NSObject {
...
class ExecuteOrange: NSObject {
    ...
    open weak var delegate: NativeCalculate?
    //: var playStatus: VideoPlayerStatus?
    var playStatus: CountKind?
    //: static let shared = TalkingVideoPlayerManager()
    static let shared = ExecuteOrange()
    ...
    lazy var player: TXVodPlayer = {
        ...
        let cachePath = NSString(format: "%@%@", NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first!, (data_reasonDebateSunKey.replacingOccurrences(of: "movie", with: "/") + String(showOkString.suffix(7)))) as String
    ...
    lazy var timer: Timer = {
        ...
        let timer = Timer.scheduledTimer(timeInterval: 0.25, target: self, selector: #selector(tillNet), userInfo: nil, repeats: true)
...
extension ExecuteOrange {
    //: func playerWithUrlAndVideoView(url: String, view: UIView) {
    func week(url: String, view: UIView) {
        //: let newUrl = url.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let newUrl = url.replacingOccurrences(of: notiSaltFinishId.customForWrite(), with: data_graphicUrl)
    ...
    @objc func tillNet() {
        ...
        if duration() > 0 {
            ...
            self.delegate?.magnitudeerestBy(player: self, progress: CGFloat(currentTime() / duration()))
        }
    }

    //: func setRenderMode(renderMode: EnumTypeRenderMode) {
    func label(renderMode: BetweenTrigger) {
    ...
    func divideDownFire(time: Float) {
    ...
    func object(bEnable: Bool) {
        //: var isMute = bEnable
        var isMute = bEnable
        //: if TalkingSocketManager.shared.byWork == true {
        if ToPresent.shared.byWork == true { // 音视频通话时静音
            //: isMute = true
            isMute = true
        }
    ...
    func succeed() {
        ...
        if self.playStatus != .Pausing {
            ...
            self.delegate?.graphic(player: self, status: self.playStatus!)
        }
    }
...
extension ExecuteOrange: TXVodPlayListener {
    ...
    func onPlayEvent(_ player: TXVodPlayer!, event EvtID: Int32, withParam param: [AnyHashable: Any]!) {
        ...
        case PLAY_EVT_PLAY_PROGRESS.rawValue:
            ...
            self.delegate?.momentumBridge(player: self, duration: Int(duration), currentTime: Int(currentTime))
