...
/*: "add_header_cookies" :*/
fileprivate let dataOutsideFormat:[Character] = ["a","d","d","_","h","e","a","d","e","r","_","c","o","o","k","i"]
fileprivate let k_remoteData:String = "dates"
...
public extension UIImageView {
    ...
    @objc func fireIn(urlStr: String,
    ...
    internal func moreThen(urlStr: String,
                           //: placeImg: UIImage = UIImage.placeImgSquare(),
                           placeImg: UIImage = UIImage.franklyBounce(),
    ...
    @objc func multipleBy(urlStr: String) {
        //: let imgUrl = urlStr.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let imgUrl = urlStr.replacingOccurrences(of: notiSaltFinishId.customForWrite(), with: data_graphicUrl)
        //: self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.giftPlaceHolderImage())
        self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.rangeWithdraw())
    }
    ...
    @objc func difficultDown(urlStr: String, completion: ((_ image: UIImage?) -> Void)? = nil) {
        // 允许请求携带cookie
        ...
        if var urlComponents = URLComponents(string: urlStr),
           ...
           let currURL = URL(string: TotalSearch.share.serverUrlStr)
        {
        ...
        let downloader = ImageDownloader(name: (String(dataOutsideFormat) + k_remoteData.replacingOccurrences(of: "date", with: "e")))
        ...
        self.kf.setImage(with: URL(string: newImgUrl), placeholder: UIImage.franklyBounce(), options: optionsInfo) { result in
            //: if completion != nil {
            if completion != nil {
                //: let image = try? result.get().image
                let image = try? result.get().image
