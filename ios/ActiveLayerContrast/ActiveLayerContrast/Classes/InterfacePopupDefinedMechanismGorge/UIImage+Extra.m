...
- (NSData *)yellow {
    ...
    UIImage *image = self.specifyMusic;
...
- (UIImage *)fieldFramework {
    ...
    return [UIImage imageWithData:[self yellow]];
}
...
- (UIImage *)manager {
    ...
    return [self impact:YES];
}

//: - (UIImage *)wcTimelineCompress {
- (UIImage *)specifyMusic {
    //: return [self wcCompres:NO];
    return [self impact:NO];
}
...
- (UIImage *)impact:(Boolean)isSession {
    //: CGSize size = [self wxImageSize:isSession];
    CGSize size = [self no:isSession];
    //: UIImage *reImage = [self resizedImage:size];
    UIImage *reImage = [self icon:size];
...
+ (NSData *)sign:(UIImage *)image estimatedClean:(NSInteger)fileSize{
    ...
    if (shortSize>1080) {//压缩到最短边不大于1080,也避免了shortSize==0的情况
        ...
        image = [image bindIndependent:targetSize];
    }

    //: UIImage *thumIMG = image;
    UIImage *thumIMG = image;
    //: NSData *outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
    NSData *outIMGData = [thumIMG can:(fileSize*1024)];
    ...
    while (outIMGData.length>(fileSize*1024)) {
        ...
        thumIMG = [thumIMG barDown:scalesize listen:NO];

        //: outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
        outIMGData = [thumIMG can:(fileSize*1024)];
    }
...
- (UIImage*)bindIndependent:(CGSize)targetSize{
...
- (UIImage *)barDown:(CGSize)targetSize listen:(BOOL)isScale{
