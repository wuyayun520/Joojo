...
fileprivate let const_okUrl:[UInt8] = [0xc1,0xc6,0xc1,0xdc,0x80,0xcb,0xc7,0xcc,0xcd,0xda,0x92,0x81,0x88,0xc0,0xc9,0xdb,0x88,0xc6,0xc7,0xdc,0x88,0xca,0xcd,0xcd,0xc6,0x88,0xc1,0xc5,0xd8,0xc4,0xcd,0xc5,0xcd,0xc6,0xdc,0xcd,0xcc]

/*: "btn_invitCode_login_BG" :*/
fileprivate let const_beanFormat:String = "delivery patternbtn_"
fileprivate let userPersistStr:String = "big lite fun closed noCode_l"
fileprivate let main_playOrangeName:[Character] = ["o","g","i","n","_","B","G"]

/*: "login_logo_icon" :*/
fileprivate let data_perceptString:[Character] = ["l","o","g","i","n","_","l","o","g","o","_","i","c","o","n"]

/*: "Your verification code :" :*/
fileprivate let app_fastPersistName:[Character] = ["Y","o","u","r"," ","v","e","r","i","f","i","c","a"]
fileprivate let k_sizeData:String = "hundredi"
fileprivate let main_triggerStr:[Character] = ["o","n"," ","c","o","d","e"," ",":"]

/*: "F4F4F4" :*/
fileprivate let constLoadFormat:String = "Fflexibilityflexibility4"

/*: "BBBBBB" :*/
fileprivate let constAgeReductionName:[Character] = ["B","B","B","B","B","B"]

/*: "Next" :*/
fileprivate let notiNobodyUrl:String = "factory max mage reflectNext"

/*: "Skip" :*/
fileprivate let user_sinkFoundData:String = "Skipcommit disclose later hung beside"
...
public class SineDestroy: UIViewController {
    ...
    public required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: const_okUrl.map{$0^168}, encoding: .utf8)!)
    }
    ...
    lazy var logoBGView: UIImageView = {
        ...
        let imageView = UIImageView(image: UIImage.road(name: (String(const_beanFormat.suffix(4)) + "invit" + String(userPersistStr.suffix(6)) + String(main_playOrangeName))))
    ...
    lazy var logoView: UIImageView = {
        ...
        let imageView = UIImageView(image: UIImage.road(name: (String(data_perceptString))))
        //: return imageView
        return imageView
        //: }()
    }()
    ...
    lazy var titleLB: UILabel = {
        ...
        label.text = (String(app_fastPersistName) + k_sizeData.replacingOccurrences(of: "hundred", with: "t") + String(main_triggerStr)).localized
        //: return label
        return label
        //: }()
    }()
    ...
    lazy var inviteCodeInputView: UITextField = {
        ...
        textField.backgroundColor = UIColor(hex: (constLoadFormat.replacingOccurrences(of: "flexibility", with: "4F")))
        //: textField.font = .pingfangFont(type: .Regular, fontSize: 15)
        textField.font = .bagUniform(type: .Regular, fontSize: 15)
        //: textField.textColor = .appTitleColor()
        textField.textColor = .sharkHighlightMaintain()
        ...
        textField.attributedPlaceholder = NSAttributedString(string: "", attributes: [NSAttributedString.Key.font: UIFont.bagUniform(type: .Regular, fontSize: 15), NSAttributedString.Key.foregroundColor: UIColor(hex: (String(constAgeReductionName)))!])
        ...
        textField.addTarget(self, action: #selector(month(_:)), for: .editingChanged)
        //: return textField
        return textField
        //: }()
    }()
    ...
    lazy var commitButton: UIButton = {
        ...
        btn.setTitle((String(notiNobodyUrl.suffix(4))).localized, for: .normal)
    ...
    lazy var skiyBtn: UIButton = {
        ...
        btn.titleLabel?.font = UIFont.bagUniform(type: .Regular, fontSize: 18)
        //: btn.setTitle("Skip".localized, for: .normal)
        btn.setTitle((String(user_sinkFoundData.prefix(4))).localized, for: .normal)
        //: btn.setBackgroundColor(color: UIColor.init(hex: "F4F4F4")!, forState: .normal)
        btn.asTimingPlain(color: UIColor(hex: (constLoadFormat.replacingOccurrences(of: "flexibility", with: "4F")))!, forState: .normal)
        //: btn.layer.cornerRadius = 25
        btn.layer.cornerRadius = 25
        //: btn.clipsToBounds = true
        btn.clipsToBounds = true
        //: btn.addTarget(self, action: #selector(skiyBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(thinDown), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension SineDestroy {
    //: @objc private func skiyBtnClick() {
    @objc private func thinDown() {
        //: self.Login1EndBlock?()
        self.Login1EndBlock?()
    }
    ...
    @objc private func detail() {
        ...
        if inviteCodeInputView.text?.count ?? 0 <= 6, UIDevice.current.userInterfaceIdiom != .pad {
            ...
            SelectedReload.tap(verifyCode: code) { succeed, result, errorModel in
                //: ProgressHUD.dismiss()
                GrayView.at()
                //: if succeed {
                if succeed {
                    //: Defaults.set(code, forKey: CacheVerificationCodeLoginKey)
                    show_modelHideFormat.set(code, forKey: user_publishAgeMsg)
                    // 获取用户信息
                    ...
                    SelectedReload.sample(.PwdPhoneLogin) { succeed, result, errorModel in
                        ...
                        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 5) {
                            ...
                            ListenerCommonTransition.shared.isSensor()
                        }
                    }
                }
            }
...
extension SineDestroy: UITextFieldDelegate {
    //: @objc private func inviteCodeTextChage(_ textField: UITextField) {
    @objc private func month(_ textField: UITextField) {
...
extension SineDestroy {
    ...
    @objc func enhance(notification: Notification) {
        //: guard let info = notification.userInfo else { return }
        guard let info = notification.userInfo else { return }
        //: let duration = info[UIResponder.keyboardAnimationDurationUserInfoKey] as? Double ?? 0.25
        let duration = info[UIResponder.keyboardAnimationDurationUserInfoKey] as? Double ?? 0.25
...
extension SineDestroy {
    ...
    func fraction() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(party(notification:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(enhance(notification:)),
