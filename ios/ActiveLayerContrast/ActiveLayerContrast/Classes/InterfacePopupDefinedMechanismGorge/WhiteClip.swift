...
fileprivate let mainFrequencyData:[UInt8] = [0x3e,0x39,0x3e,0x23,0x7f,0x34,0x38,0x33,0x32,0x25,0x6d,0x7e,0x77,0x3f,0x36,0x24,0x77,0x39,0x38,0x23,0x77,0x35,0x32,0x32,0x39,0x77,0x3e,0x3a,0x27,0x3b,0x32,0x3a,0x32,0x39,0x23,0x32,0x33]

private func masterAsset(content num: UInt8) -> UInt8 {
    return num ^ 87
}

/*: "#FFD13A" :*/
fileprivate let appRelatedMsg:[Character] = ["#","F","F","D","1","3","A"]

/*: "gift_gb_umnad_1" :*/
fileprivate let kInsertKey:String = "gift_gboth license"

/*: "icon_circle_pic_default" :*/
fileprivate let data_nativeComposeMsg:[Character] = ["i","c","o","n","_","c","i","r"]
fileprivate let noti_confirmRankName:String = "cle_pic_margin string writing"
fileprivate let userSlideString:String = "DEFAULT"

/*: "Open lucky blind-box" :*/
fileprivate let user_spendStr:[Character] = ["O","p","e","n"," ","l","u","c"]
fileprivate let constPhotoCoverValue:[Character] = ["k","y"," ","b","l","i","n","d","-","b","o","x"]

/*: "Sent " :*/
fileprivate let main_searcherKey:[Character] = ["S","e","n","t"," "]

/*: "PingFangSC-Semibold" :*/
fileprivate let kHmmPackageString:String = "onto reachPingFang"
fileprivate let appRagTitle:[Character] = ["S","C","-","S","e","m","i","b","o","l","d"]

/*: "Bombard" :*/
fileprivate let notiCollectionKey:String = "Bombardpress bass"
...
class WhiteClip: UIView {
    //: var giftMsgModel = TalkingGiftMsgAnimatModel()
    var giftMsgModel = IndexInput()
    //: var giftSendItem = [TalkingGiftNumArrModel()]
    var giftSendItem = [BackgroundPlatform()]
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: mainFrequencyData.map{masterAsset(content: $0)}, encoding: .utf8)!)
    }
...
//        lb.numberOfLines = 1
        ...
        lb.textColor = UIColor(hex: (String(appRelatedMsg)))
    ...
    private lazy var pluralTip: ExtendSort = {
    ...
    private lazy var countLb: ExtendSort = {
        //: let lab = TalkingGiftTrackItemLabel()
        let lab = ExtendSort()
...
extension WhiteClip {
    ...
    func enableicialNorm(model: IndexInput) {
        ...
        self.bgImage.image = UIImage.road(name: (String(kInsertKey.prefix(6)) + "b_umnad_1"))

        //: let placeholder = UIImage.BundleImageNamed(name: "icon_circle_pic_default")
        let placeholder = UIImage.road(name: (String(data_nativeComposeMsg) + String(noti_confirmRankName.prefix(8)) + userSlideString.lowercased()))

        //: if (model.showType == ChatGiftAnimatType.myStery.rawValue ) {
        if model.showType == SinkContent.myStery.rawValue {
            //: self.nickLb.text = "Open lucky blind-box".localized
            self.nickLb.text = (String(user_spendStr) + String(constPhotoCoverValue)).localized
            //: }else {
        } else {
        ...
        self.avatarImage.moreThen(urlStr: giftMsgModel.fromHeadPic, placeImg: placeholder)
        //: self.giftImage.setUrlImage(urlStr: giftMsgModel.giftPic, placeImg: placeholder)
        self.giftImage.moreThen(urlStr: giftMsgModel.giftPic, placeImg: placeholder)

        //: self.giftName.text = "Sent ".localized + giftMsgModel.pname
        self.giftName.text = (String(main_searcherKey)).localized + giftMsgModel.pname
        ...
        if sendNumber > 1 {
            ...
            var font1 = UIFont(name: (String(kHmmPackageString.suffix(8)) + String(appRagTitle)), size: 22)
            //: font1 = italicFontWith(angle: 15, fontSize: 18, fontName: font1!.fontName)
            font1 = view(angle: 15, fontSize: 18, fontName: font1!.fontName)
            //: var font2 = UIFont(name: "PingFangSC-Semibold", size: 26)
            var font2 = UIFont(name: (String(kHmmPackageString.suffix(8)) + String(appRagTitle)), size: 26)
            //: font2 = italicFontWith(angle: 15, fontSize: 22, fontName: font2!.fontName)
            font2 = view(angle: 15, fontSize: 22, fontName: font2!.fontName)
            ...
            numText = numText.imagePion(texts: strs, colors: colors, fonts: fonts)
        ...
        packageOff(giftCount: giftMsgModel.comboNum)
    }

    //: func refreshGiftCount(giftCount: Int) {
    func packageOff(giftCount: Int) {
        //: var font1 = UIFont(name: "PingFangSC-Semibold", size: 45)
        var font1 = UIFont(name: (String(kHmmPackageString.suffix(8)) + String(appRagTitle)), size: 45)
        //: font1 = italicFontWith(angle: 15, fontSize: 26, fontName: font1!.fontName)
        font1 = view(angle: 15, fontSize: 26, fontName: font1!.fontName)
        //: var font2 = UIFont(name: "PingFangSC-Semibold", size: 45)
        var font2 = UIFont(name: (String(kHmmPackageString.suffix(8)) + String(appRagTitle)), size: 45)
        //: font2 = italicFontWith(angle: 15, fontSize: 38, fontName: font2!.fontName)
        font2 = view(angle: 15, fontSize: 38, fontName: font2!.fontName)
        ...
        let colors = [UIColor.doingShelter(150, 77, 20, 1), UIColor.white, UIColor.doingShelter(150, 77, 20, 1)]
        ...
        attText = attText.imagePion(texts: strs, colors: colors, fonts: fonts)
        ...
        countLb.processing(attributedText: attText)
        ...
        countLb.doingceWindow(frame: CGRect(x: x, y: countLb.origin.y, width: width, height: countLb.size.height))
    }

    /// 连击
    //: func comboHitWith(giftMsgModel: TalkingGiftMsgAnimatModel) {
    func tillBottom(giftMsgModel: IndexInput) {
        ...
        NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(example), object: nil)
        ...
        self.perform(#selector(example), afterDelay: 3.5)
    }

    /// 展示
    //: func showGiftAnimate(model: TalkingGiftMsgAnimatModel, animate: Bool) {
    func visit(model: IndexInput, animate: Bool) {
    ...
    func mechanismChain(frame: CGRect) {
        //: UIView.animate(withDuration: 0.2, delay: 0.1, options: [.layoutSubviews]) {
        UIView.animate(withDuration: 0.2, delay: 0.1, options: [.layoutSubviews]) {
            //: self.frame = frame
            self.frame = frame
...
extension WhiteClip {
    ...
    func view(angle: CGFloat, fontSize: CGFloat, fontName: String) -> (UIFont) {
...
extension WhiteClip {
    ...
    private func stageBy() {
        ...
        font = view(angle: 15, fontSize: 20, fontName: font.fontName)
        ...
        let attText = attributedString.imagePion(texts: [(String(notiCollectionKey.prefix(7))).localized], colors: [UIColor.doingShelter(255, 232, 72, 1)], fonts: [font])
        //: pluralTip.setFrame(frame: CGRect.init(x: giftImage.right, y: 14, width: attText.size().width + 2, height: height-20))
        pluralTip.doingceWindow(frame: CGRect(x: giftImage.right, y: 14, width: attText.size().width + 2, height: height - 20))

        //: pluralTip.setAttributedText(attributedText: attText)
        pluralTip.processing(attributedText: attText)
        //: contentView.addSubview(pluralTip)
        contentView.addSubview(pluralTip)

        //: countLb.setFrame(frame: CGRect.init(x: giftImage.right, y: 2, width: 70, height: height))
        countLb.doingceWindow(frame: CGRect(x: giftImage.right, y: 2, width: 70, height: height))
