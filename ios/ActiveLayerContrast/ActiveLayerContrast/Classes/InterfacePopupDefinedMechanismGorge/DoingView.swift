...
fileprivate let showTapPath:[UInt8] = [0xd2,0xd7,0xd2,0xdd,0x91,0xcc,0xd8,0xcd,0xce,0xdb,0xa3,0x92,0x89,0xd1,0xca,0xdc,0x89,0xd7,0xd8,0xdd,0x89,0xcb,0xce,0xce,0xd7,0x89,0xd2,0xd6,0xd9,0xd5,0xce,0xd6,0xce,0xd7,0xdd,0xce,0xcd]

fileprivate func sendMix(ermine num: UInt8) -> UInt8 {
...
/*: "img_loading_lose" :*/
fileprivate let constAirportString:[Character] = ["i","m","g","_","l","o","a","d","i","n","g","_","l","o","s"]
fileprivate let dataSecondUrl:String = "ported"
...
class DoingView: UICollectionViewCell {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: showTapPath.map{sendMix(ermine: $0)}, encoding: .utf8)!)
    }
...
extension DoingView {
    ...
    private func platform() {
        ...
        let singleTap = UITapGestureRecognizer(target: self, action: #selector(bounce))

        //: doubleTap = UITapGestureRecognizer(target: self, action: #selector(doubleTapAction)).then {
        doubleTap = UITapGestureRecognizer(target: self, action: #selector(fill)).then {
...
extension DoingView {
    ...
    func hide(imgModel: VerticalStorage) {
        ...
        if imgModel.imagePic != nil {
            ...
            imgView.kf.setImage(with: URL(string: imgModel.url), placeholder: UIImage(named: (String(constAirportString) + dataSecondUrl.replacingOccurrences(of: "ported", with: "e"))), options: [.keepCurrentImageWhileLoading]) { result in
                //: self.recoverSubviews()
                self.fast()
            }
        }
    ...
    func age(imgUrl: String) {
        ...
        imgView.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage(named: (String(constAirportString) + dataSecondUrl.replacingOccurrences(of: "ported", with: "e"))), options: [.keepCurrentImageWhileLoading]) { result in
            //: self.recoverSubviews()
            self.fast()
        }
    }
...
extension DoingView: UIScrollViewDelegate, UIGestureRecognizerDelegate {
...
extension DoingView {
    ...
    func fast() {
        ...
        var newW = show_licenseTitle
        //: var newH = ScreenWidth * tempImg!.size.height / tempImg!.size.width
        var newH = show_licenseTitle * tempImg!.size.height / tempImg!.size.width

        //: if newH > ScreenHeight {
        if newH > dataChainValue {
            //: newH = ScreenHeight
            newH = dataChainValue
            //: newW = tempImg!.size.width * ScreenHeight / tempImg!.size.height
            newW = tempImg!.size.width * dataChainValue / tempImg!.size.height
        }
        ...
        self.imgContainerView.origin = CGPoint(x: (show_licenseTitle - newW) / 2.0, y: (dataChainValue - newH) / 2)
