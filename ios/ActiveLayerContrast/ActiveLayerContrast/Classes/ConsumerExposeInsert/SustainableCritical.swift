...
fileprivate let constReserveValue:[UInt8] = [0x9b,0xa0,0x9b,0xa6,0x5a,0x95,0xa1,0x96,0x97,0xa4,0x6c,0x5b,0x52,0x9a,0x93,0xa5,0x52,0xa0,0xa1,0xa6,0x52,0x94,0x97,0x97,0xa0,0x52,0x9b,0x9f,0xa2,0x9e,0x97,0x9f,0x97,0xa0,0xa6,0x97,0x96]

fileprivate func observerSumryPanel(attach num: UInt8) -> UInt8 {
...
@objc protocol SensorMarker: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func gift()

    //: @objc optional func endScroll()
    @objc optional func every()
}

//: class TalkingVideoDanmuMsgListTableView: UIView {
class SustainableCritical: UIView {
    ...
    override init(frame: CGRect) {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(difficultCommand))
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: constReserveValue.map{observerSumryPanel(attach: $0)}, encoding: .utf8)!)
    }
...
extension SustainableCritical {
    ...
    func trainBy() {
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(effect), userInfo: nil, repeats: true)
    }
...
extension SustainableCritical {
    ...
    func resign(msgModel: OvalTrain?) {
    ...
    func checked() {
        ...
        if AllHeight > kShadePriceFormat {
            //: if self.msgTableView.height < VideoMsgTableViewHeight {
            if self.msgTableView.height < kShadePriceFormat {
                ...
                self.msgTableView.height = kShadePriceFormat
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = kShadePriceFormat - AllHeight
        ...
        fillCurrent(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func fillCurrent(animated: Bool) {
    ...
    func postEnablece(inPending: Bool) {
...
extension SustainableCritical: UITableViewDelegate, UITableViewDataSource {
    ...
    func condition(_ scrollView: UIScrollView) {
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(noti_orientationTitle)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = ComposeLabel.functional(tableView: tableView, msg: msgModel, indexPath: indexPath)
...
extension SustainableCritical {
    ...
    func price() {
        ...
        self.msgTableView.frame = CGRect(x: 0, y: 0, width: k_easyObserveKey, height: kShadePriceFormat)
    }
}
