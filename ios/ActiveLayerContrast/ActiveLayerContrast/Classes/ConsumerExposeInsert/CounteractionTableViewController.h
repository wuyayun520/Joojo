 ...
 *
 *  CouldExitSource 协议和 CounteractionTableViewController 类。
 *  CounteractionTableViewController 是聊天界面的核心类，负责聊天界面大部分核心业务逻辑的处理与运算。
 *  该类的详细属性与功能，请参展本文件中 CounteractionTableViewController 的具体声明。
 *
 ******************************************************************************/
...
@class CounteractionTableViewController;
...
//: typedef NS_ENUM(NSInteger, UserChatType) {
typedef NS_ENUM(NSInteger, UserChatType) {
    //: UserChatType_unDefined = 0,
    UserChatType_unDefined = 0,
    //: UserChatType_private = 1,
    UserChatType_private = 1,
    //: UserChatType_group = 2,
    UserChatType_group = 2,
    //: UserChatType_system = 3,
    UserChatType_system = 3,
    //: UserChatType_service = 4, 
    UserChatType_service = 4, // 官方客服
    //: UserChatType_meeting = 5, 
    UserChatType_meeting = 5, // 公共聊天室
    //: UserChatType_moreMsg = 6, 
    UserChatType_moreMsg = 6, // 更多消息（超过5天未回复）
//: };
};
...
@protocol CouldExitSource <NSObject>
...
- (void)attaches:(CounteractionTableViewController *)controller;
...
- (void)blocced:(CounteractionTableViewController *)controller;
...
- (BOOL)permanentSignificant:(CounteractionTableViewController *)controller add:(UIView *)view;
 ...
 */
//: - (EmbraceShadow *)messageController:(TUIMessageController *)controller onNewMessage:(V2TIMMessage *)data;
- (EmbraceShadow *)message:(CounteractionTableViewController *)controller rounding:(V2TIMMessage *)data;
 ...
 */
//: - (ImplementProperty *)messageController:(TUIMessageController *)controller onShowMessageData:(EmbraceShadow *)data;
- (ImplementProperty *)reload:(CounteractionTableViewController *)controller embraceDown:(EmbraceShadow *)data;
 ...
 *  @param cell 所点击的消息单元
 */
//: - (void)messageController:(TUIMessageController *)controller onSelectMessageAvatar:(ImplementProperty *)cell;
- (void)pic:(CounteractionTableViewController *)controller immediateDrawing:(ImplementProperty *)cell;
 ...
 *  @param cell 所点击的消息单元
 */
//: - (void)messageController:(TUIMessageController *)controller onSelectMessageContent:(ImplementProperty *)cell;
- (void)over:(CounteractionTableViewController *)controller tagUponDataOf:(ImplementProperty *)cell;
 ...
 */
//: - (void)messageController:(TUIMessageController *)controller onSelectMessageMenu:(NSInteger)menuType withData:(EmbraceShadow *)data;
- (void)obtain:(CounteractionTableViewController *)controller appear:(NSInteger)menuType temporary:(EmbraceShadow *)data;
/**
...
/*重发*/
//: - (void)messageController:(TUIMessageController *)controller onRetryMessageContent:(ImplementProperty *)cell;
- (void)lastSkirt:(CounteractionTableViewController *)controller information:(ImplementProperty *)cell;

/*引用*/
//: - (void)messageController:(TUIMessageController *)controller onReplyMessage:(EmbraceShadow *)cellData;
- (void)roman:(CounteractionTableViewController *)controller countegration:(EmbraceShadow *)cellData;

/*点击引用*/

//: - (void)messageController:(TUIMessageController *)controller onClickReplyMessage:(EmbraceShadow *)cellData;
- (void)attach:(CounteractionTableViewController *)controller pro:(EmbraceShadow *)cellData;
...
/////////////////////////////////////////////////////////////////////////////////
//
//                         CounteractionTableViewController
//
/////////////////////////////////////////////////////////////////////////////////

/**
 * 腾讯云 TUIKit
 *  【模块名称】CounteractionTableViewController
...
@interface CounteractionTableViewController : UITableViewController

/**
 *  执行 CouldExitSource 协议的委托
 */
//: @property (nonatomic, weak) id<TMessageControllerDelegate> delegate;
@property (nonatomic, weak) id<CouldExitSource> delegate;
 ...
 * @param uiMsgs 待删除的数据
 */
//- (void)deleteMessages:(NSArray<EmbraceShadow *> *)uiMsgs;
...
/**
 * 批量将V2TIMessage转换成EmbraceShadow
 */
...
///当前对话类型
//: @property (nonatomic, assign) UserChatType currentChatType;
@property (nonatomic, assign) UserChatType currentChatType;
