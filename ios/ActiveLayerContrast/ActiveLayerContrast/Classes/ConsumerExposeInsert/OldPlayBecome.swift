...
/*: "%@ reply %@：%@" :*/
fileprivate let userBagKey:String = "%@ repan interrupt"
...
struct OldPlayBecome: HandyJSON {
    //: var cid: String?
...
extension OldPlayBecome {
    ...
    mutating func sub() {
        ...
        if replyUser.count > 0 {
            ...
            tempContent = String(format: (String(userBagKey.prefix(5)) + "ply %@\u{ff1a}%@"), nickname, replyUser, content)
            //: } else {
        } else {
        ...
        let sizeheight = Int(ScientificView.everyNative(text: tempContent, size: CGSize(width: show_licenseTitle - 82, height: CGFloat(MAXFLOAT)), font: UIFont.bagUniform(type: .Regular, fontSize: 14), lineNumber: 0).height)
