...
class MediaAfter: UINavigationController {

    //: var tabBarType: TabBarItemType?
    var tabBarType: NormDocumentEager?
    //: var pushing = false                             
    ...
    override func pushViewController(_ viewController: UIViewController, animated: Bool) {
        ...
        if viewControllers.count >= 1 {
            ...
            if viewController.navigationItem.leftBarButtonItem == nil {
                ...
                viewController.navigationItem.leftBarButtonItem = viewController.borderStatus()
            }
        }
    ...
    func may() {
        ...
        if #available(iOS 15.0, *) {
            ...
            barApp.backgroundImage = UIImage.trainActual(color: UIColor.white)
            //: barApp.shadowImage = UIImage.init()
            barApp.shadowImage = UIImage.init()
            //: barApp.shadowColor = UIColor.appBgColor()
            barApp.shadowColor = UIColor.livingRank()
            ...
            barApp.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.sharkHighlightMaintain(), NSAttributedString.Key.font: UIFont.bagUniform(type: .Medium, fontSize: 17)]
        ...
        //: } else {
            ...
            navigationBar.setBackgroundImage(UIImage.trainActual(color: UIColor.white), for: .any, barMetrics: .default)
            ...
            navigationBar.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.sharkHighlightMaintain(), NSAttributedString.Key.font: UIFont.bagUniform(type: .Medium, fontSize: 17)]
        }
        //: UINavigationBar.appearance().tintColor = UIColor.appTitleColor()
        UINavigationBar.appearance().tintColor = UIColor.sharkHighlightMaintain()
...
extension MediaAfter: UINavigationControllerDelegate {
    ...
    func navigationController(_ navigationController: UINavigationController, willShow viewController: UIViewController, animated: Bool) {
        ...
        if (viewController.hidesBottomBarWhenPushed) {
            ...
            SpecifyMusic.shared.advSuperView?.isHidden = true
        //: } else {
        } else {
...
extension MediaAfter: UIGestureRecognizerDelegate {
