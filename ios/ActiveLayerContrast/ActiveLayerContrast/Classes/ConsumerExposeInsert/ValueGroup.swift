...
fileprivate let constAreaString:[UInt8] = [0xa5,0xaa,0xa5,0xb0,0x64,0x9f,0xab,0xa0,0xa1,0xae,0x76,0x65,0x5c,0xa4,0x9d,0xaf,0x5c,0xaa,0xab,0xb0,0x5c,0x9e,0xa1,0xa1,0xaa,0x5c,0xa5,0xa9,0xac,0xa8,0xa1,0xa9,0xa1,0xaa,0xb0,0xa1,0xa0]

fileprivate func statReserve(spec num: UInt8) -> UInt8 {
...
/*: "icon_chat_start" :*/
fileprivate let notiPracticalKey:String = "istar"
fileprivate let app_grabName:[Character] = ["o","n","_","c","h","a","t","_","s","t","a","r","t"]

/*: "icon_chat_lock" :*/
fileprivate let dataEnoughStretchSignFormat:String = "icon_chresign mini"
fileprivate let const_keepTransferStr:String = "at_lockwhite response await may gesture"

/*: "icon_chat_burned" :*/
fileprivate let dataSaveTitle:String = "assert existingicon_ch"
fileprivate let user_orangTechniqueStr:[Character] = ["a","t","_","b","u","r","n","e","d"]

/*: "icon_chat_burned_shan" :*/
fileprivate let noti_earlyValue:[Character] = ["i","c","o","n","_","c","h","a","t","_"]
fileprivate let show_signatureLaunchData:String = "burapid"

/*: "Already burned" :*/
fileprivate let user_commonSortMsg:[Character] = ["A","l","r","e","a","d","y"," ","b","u"]
fileprivate let app_pageEfficiencyFormat:[Character] = ["r","n","e","d"]
...
class ValueGroup: GestureLast {
    //: var videoData: RoundingAsset?
    var videoData: RoundingAsset?
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: constAreaString.map{statReserve(spec: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var playImgView: UIImageView = {
        ...
        imgV.image = UIImage.road(name: (notiPracticalKey.replacingOccurrences(of: "star", with: "c") + String(app_grabName)))
        //: return imgV
        return imgV
        //: }()
    }()
    ...
    private lazy var lockImgView: UIImageView = {
        ...
        imgV.image = UIImage.road(name: (String(dataEnoughStretchSignFormat.prefix(7)) + String(const_keepTransferStr.prefix(7))))
        //: return imgV
        return imgV
        //: }()
    }()
    ...
    private lazy var burnedImgView: KeepPurple = {
        ...
        imgV.setBackgroundImage(UIImage.road(name: (String(dataSaveTitle.suffix(7)) + String(user_orangTechniqueStr))), for: .normal)
        //: imgV.setImage(UIImage.BundleImageNamed(name: "icon_chat_burned_shan"), for: .normal)
        imgV.setImage(UIImage.road(name: (String(noti_earlyValue) + show_signatureLaunchData.replacingOccurrences(of: "rapid", with: "rn") + "ed_shan")), for: .normal)
        //: imgV.imageAlignment = .top
        imgV.imageAlignment = .top
        //: imgV.setTitle("Already burned".localized, for: .normal)
        imgV.setTitle((String(user_commonSortMsg) + String(app_pageEfficiencyFormat)).localized, for: .normal)
...
extension ValueGroup {
    ...
    override func grain(with data: ChartEmptyPop) {
        ...
        self.dutationLab.text = NSDate.mountEnable(totalTime: duration)
        ...
        self.unlessWithin(urlStr: videoModel.coverImg, isMosaic: imgIsMosaic)
    }
    ...
    func unlessWithin(urlStr: String, isMosaic: Bool) {
        //: guard let newData = self.videoData else { return }
        guard let newData = self.videoData else { return }
        ...
        if !coverImgPath.isEmptyString, isExit { // 加载本地
            ...
            //: if isMosaic {
            if isMosaic {
                //: self.picImgView.image = UIImage.stop(image, logIn: 90)
                self.picImgView.image = UIImage.stop(image, logIn: 90)
                //: } else {
            } else {
            ...
            self.picImgView.difficultDown(urlStr: urlStr) { [weak self] image in
