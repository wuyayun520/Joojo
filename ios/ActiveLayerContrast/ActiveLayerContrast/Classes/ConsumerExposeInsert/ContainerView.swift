...
fileprivate let notiExpressionUrl:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "btn_dynamic_stop_nor" :*/
fileprivate let notiRestoreUrl:String = "voice can chart clearly dragbtn_dy"
fileprivate let notiOxygenString:String = "c_stopanel important missing found canvas"
fileprivate let main_difficultTitle:String = "p_norcanvas personal after"

/*: "btnwOblEh_tdj_oediv_stop" :*/
fileprivate let dataCapacityValue:String = "btnwOalbum key what currently constraint"
fileprivate let userAttachData:String = "blEhfollowing out"
fileprivate let noti_decisionUrl:String = "_tdj_oraw hi ya pull response"
fileprivate let noti_ermineId:String = "stosophisticated"

/*: "btnCmjoS7_tdj_oediv_play" :*/
fileprivate let k_pictureContainMessage:[Character] = ["b","t","n","C","m","j","o","S","7","_","t","d","j","_","o","e","d","i","v","_","p"]
fileprivate let appDrinkFormat:String = "debatey"
...
class ContainerView: UIView {
    ...
    open weak var delegate: PutTheme?
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: notiExpressionUrl.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var centerPlayButton: UIButton = {
        ...
        btn.setImage(UIImage.road(name: (String(notiRestoreUrl.suffix(6)) + "nami" + String(notiOxygenString.prefix(5)) + String(main_difficultTitle.prefix(5)))), for: .normal)
        //: btn.addTarget(self, action: #selector(centerButtonClickAction(sender:)), for: .touchUpInside)
        btn.addTarget(self, action: #selector(skin(sender:)), for: .touchUpInside)
        //: btn.isHidden = true
        btn.isHidden = true
        //: return btn
        return btn
    ...
    private lazy var playButton: UIButton = {
        ...
        btn.setImage(UIImage.road(name: (String(dataCapacityValue.prefix(5)) + String(userAttachData.prefix(4)) + String(noti_decisionUrl.prefix(6)) + "ediv_" + noti_ermineId.replacingOccurrences(of: "sophisticated", with: "p"))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "btnCmjoS7_tdj_oediv_play"), for: .selected)
        btn.setImage(UIImage.road(name: (String(k_pictureContainMessage) + appDrinkFormat.replacingOccurrences(of: "debate", with: "la"))), for: .selected)
        //: btn.addTarget(self, action: #selector(playButtonClickAction(sender:)), for: .touchUpInside)
        btn.addTarget(self, action: #selector(square(sender:)), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var slider: ActiveView = {
        ...
        let slid = ActiveView(frame: CGRect(x: 100, y: 0, width: show_licenseTitle - 167, height: 40), sliderWidth: 12, sliderColor: UIColor.white, progressHeight: 2, progressBgColor: UIColor(red: 255 / 255.0, green: 255 / 255.0, blue: 255 / 255.0, alpha: 0.3), progressPlayedColor: UIColor.white, progressCachedColor: UIColor.clear, isShowCorner: true)
...
extension ContainerView {
    ...
    private func air() {
        ...
        self.slider.getValueBlock = { [weak self] value in
            ...
            self.delegate?.key(view: self, value: value)
        }
