...
fileprivate let noti_oldMsg:[UInt8] = [0x0,0x5,0x0,0xb,0xbf,0xfa,0x6,0xfb,0xfc,0x9,0xd1,0xc0,0xb7,0xff,0xf8,0xa,0xb7,0x5,0x6,0xb,0xb7,0xf9,0xfc,0xfc,0x5,0xb7,0x0,0x4,0x7,0x3,0xfc,0x4,0xfc,0x5,0xb,0xfc,0xfb]

fileprivate func fatalBe(two num: UInt8) -> UInt8 {
...
/*: "OverMaterial" :*/
fileprivate let mainBubbleName:String = "have via security areaDropMe"
fileprivate let const_dentedTitle:String = "sort quit efficiency all draftmberMe"
fileprivate let app_oddGateJustName:[Character] = ["n","u","C","e","l","l"]

/*: "roomId" :*/
fileprivate let app_assertData:String = "roomIfragment genetic sophisticated"
fileprivate let main_confidentMessage:[Character] = ["d"]

/*: "type" :*/
fileprivate let k_flagMessage:String = "harassmentype"

/*: "page" :*/
fileprivate let dataPaintName:String = "PAGE"

/*: "icon_chats_pcr" :*/
fileprivate let showNetConsentFormat:String = "safe ban related locationicon_"
fileprivate let app_frameId:[Character] = ["c","h","a","t","s","_","p","c","r"]

/*: "All Numbers" :*/
fileprivate let user_veryKey:[Character] = ["A","l","l"," ","N","u","m","b","e","r","s"]

/*: "all" :*/
fileprivate let k_processingUrl:[Character] = ["a","l","l"]

/*: "Recent Gifts" :*/
fileprivate let user_unlessId:String = "read opportunityRece"
...
class ImplementCharacter: UIView {
    //: var pageIndex = 0
    var pageIndex = 0
    //: var roomID = ""
    var roomID = ""
    //: var menuDelegate: DropDownMemberMenuViewDelegate?
    var menuDelegate: TakeOrange?
    ...
    fileprivate lazy var contentView: UIView = self.gemOffThumb()
    //: fileprivate lazy var mTable: UITableView = self.makeTableView()
    fileprivate lazy var mTable: UITableView = self.onlyCustomer()
    //: fileprivate var anchorRect = CGRect.zero
    fileprivate var anchorRect = CGRect.zero

    //: static let shared = DropDownMemberMenuView(frame: .zero)
    static let shared = ImplementCharacter(frame: .zero)
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: noti_oldMsg.map{fatalBe(two: $0)}, encoding: .utf8)!)
    }

    //: private func makeContentView() -> UIView {
    private func gemOffThumb() -> UIView {
        //: let view  = UIView()
        let view = UIView()
        //: return view
        return view
    ...
    private func onlyCustomer() -> UITableView {
    ...
    private func curDisable() {
        ...
        mTable.register(OverMaterial.self, forCellReuseIdentifier: (String(mainBubbleName.suffix(6)) + String(const_dentedTitle.suffix(6)) + String(app_oddGateJustName)))
    ...
    func stripDown() {
        ...
        var dict = [String: Any]()
        //: dict["roomId"] = roomID
        dict[(String(app_assertData.prefix(5)) + String(main_confidentMessage))] = roomID
        //: dict["type"] = "3"
        dict[(k_flagMessage.replacingOccurrences(of: "harassment", with: "t"))] = "3"
        //: dict["page"] = String(pageIndex)
        dict[(dataPaintName.lowercased())] = String(pageIndex)
        ...
        StageMargin.share.architectureThroughCommand(params: dict, completion: { array in
            ...
            if self.pageIndex == 0 {
                ...
                let model = RomanMagnitudeerval()
                //: model.headPic = "icon_chats_pcr"
                model.headPic = (String(showNetConsentFormat.suffix(5)) + String(app_frameId))
                //: model.nickname = "All Numbers".localized
                model.nickname = (String(user_veryKey)).localized
                //: model.uid = "all"
                model.uid = (String(k_processingUrl))
...
extension ImplementCharacter: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: (String(mainBubbleName.suffix(6)) + String(const_dentedTitle.suffix(6)) + String(app_oddGateJustName)), for: indexPath) as! OverMaterial
        ...
        if recentmenuArray.count > 0 && indexPath.section == 0 {
            ...
            cellModel = recentmenuArray[indexPath.row] as! RomanMagnitudeerval
            //: }else {
        } else {
            //: cellModel = menuArray[indexPath.row] as! TalkingChatRoomMemberModel
            cellModel = menuArray[indexPath.row] as! RomanMagnitudeerval
        }
        //: cell.setDropMemberMenuCell(model: cellModel)
        cell.canShadeValidVolumeCountegrityOpen(model: cellModel)
        //: return cell
        return cell
    }
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        menuDelegate?.liveCalculate(model: cellModel)
    }

    //: func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        //: let headeView = UILabel.init(frame: CGRect(x: 0, y: 0, width: ScreenWidth, height: 23))
        let headeView = UILabel(frame: CGRect(x: 0, y: 0, width: show_licenseTitle, height: 23))
        //: headeView.backgroundColor = UIColor.appBgColor()
        headeView.backgroundColor = UIColor.livingRank()
        //: let titleLB = UILabel.init()
        let titleLB = UILabel()
        //: titleLB.frame = CGRect(x: 3, y: 5, width: 100, height: 14)
        titleLB.frame = CGRect(x: 3, y: 5, width: 100, height: 14)
        //: titleLB.font = .pingfangFont(type: .Medium, fontSize: 12)
        titleLB.font = .bagUniform(type: .Medium, fontSize: 12)
        //: titleLB.textColor = .appValueDetailColor()
        titleLB.textColor = .beforePainter()
        //: if recentmenuArray.count > 0 && section == 0 {
        if recentmenuArray.count > 0 && section == 0 {
            //: titleLB.text = "Recent Gifts".localized
            titleLB.text = (String(user_unlessId.suffix(4)) + "nt Gifts").localized
            //: }else {
        } else {
            //: titleLB.text = "All Numbers".localized
            titleLB.text = (String(user_veryKey)).localized
        }
