...
/*: "#AAAAAA" :*/
fileprivate let app_trainData:String = "#AAAAAAjudge why"

/*: " No more than 10 characters" :*/
fileprivate let const_helpData:[Character] = [" ","N","o"," ","m","o","r","e"," ","t","h","a","n"," ","1","0"," ","c","h","a","r","a","c"]
fileprivate let k_increasinglyString:String = "tromanrs"

/*: "init(coder:) has not been implemented" :*/
fileprivate let appNeverOwnData:[UInt8] = [0x4,0x3,0x4,0x19,0x45,0xe,0x2,0x9,0x8,0x1f,0x57,0x44,0x4d,0x5,0xc,0x1e,0x4d,0x3,0x2,0x19,0x4d,0xf,0x8,0x8,0x3,0x4d,0x4,0x0,0x1d,0x1,0x8,0x0,0x8,0x3,0x19,0x8,0x9]

private func rollMForward(snap num: UInt8) -> UInt8 {
    return num ^ 109
}
...
class BetweenView: UIView {
    ...
    private var alertConfig: MissionField
    ...
    lazy var titleLabel: UILabel = {
        ...
        lab.font = UIFont.bagUniform(type: .Semibold, fontSize: 20)
        //: lab.numberOfLines = 0
        lab.numberOfLines = 0
    ...
    private lazy var inputTView: TitleView = {
        //: let input = InputTextView()
        let input = TitleView()
        //: input.font = UIFont.pingfangFont(type: .Regular, fontSize: 16)
        input.font = UIFont.bagUniform(type: .Regular, fontSize: 16)
        //: input.placeholderColor = .init(hex: "#AAAAAA") ?? UIColor.gray
        input.placeholderColor = .init(hex: (String(app_trainData.prefix(7)))) ?? UIColor.gray
        //: input.placeholder = " No more than 10 characters".localized
        input.placeholder = (String(const_helpData) + k_increasinglyString.replacingOccurrences(of: "roman", with: "e")).localized
        ...
        input.backgroundColor = .livingRank()
        ...
        let attributes = [NSAttributedString.Key.foregroundColor: UIColor.sharkHighlightMaintain(), .font: UIFont.bagUniform(type: .Regular, fontSize: 16)]
    ...
    private lazy var leftBtn: UIButton = {
        ...
        leftBtn.layer.borderColor = UIColor.special().cgColor
        //: leftBtn.layer.borderWidth = 1
        leftBtn.layer.borderWidth = 1
        //: leftBtn.addTarget(self, action: #selector(leftClick), for: .touchUpInside)
        leftBtn.addTarget(self, action: #selector(bodyDifficult), for: .touchUpInside)
        //: return leftBtn
        return leftBtn
        //: }()
    }()
    ...
    private lazy var rightBtn: UIButton = {
        ...
        rightBtn.setBackgroundImage(UIImage.managerMouse(colors: UIColor.raceGold(), size: CGSize(width: 267, height: 50)), for: .normal)
        ...
        rightBtn.setBackgroundImage(UIImage.managerMouse(colors: UIColor.ermineAcross(), size: CGSize(width: 267, height: 50)), for: .disabled)
        ...
        rightBtn.addTarget(self, action: #selector(singleTo), for: .touchUpInside)
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: appNeverOwnData.map{rollMForward(snap: $0)}, encoding: .utf8)!)
    }

    //: func changeRightBtn(title: String, isTouch: Bool) {
    func factorOn(title: String, isTouch: Bool) {
        ...
        if !isTouch {
            ...
            rightBtn.setBackgroundImage(UIImage.road(name: alertConfig.rightBackgroundImagefalse), for: .normal)
            //: } else {
        } else {
            //: let image = UIImage.BundleImageNamed(name: alertConfig.rightBackgroundImage).withTintColor(.appThemeColor())
            let image = UIImage.road(name: alertConfig.rightBackgroundImage).withTintColor(.special())
...
extension BetweenView: UITextViewDelegate {
    //: func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
    func factorIntervalegrity(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
    ...
    func aboveDetect(_ textField: UITextField) -> Bool {
    ...
    func textViewDidChange(_ textView: UITextView) {
        ...
        if textView.text.count > alertConfig.lengthLimit {
            ...
            if textNum > alertConfig.lengthLimit && alertConfig.lengthLimit > 0 {
                ...
                textView.text = first(index: alertConfig.lengthLimit, text: textContent)
            }
        }
    }

    //: private func string_prefix(index: Int, text: String) -> String {
    private func first(index: Int, text: String) -> String {
        //: if text.count <= index {
        if text.count <= index {
            //: return text
            return text
