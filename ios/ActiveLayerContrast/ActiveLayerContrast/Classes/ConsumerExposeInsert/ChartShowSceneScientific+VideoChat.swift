...
/*: "type" :*/
fileprivate let dataPublishTauFormat:String = "whyype"

/*: "You are currently on a call, please try again later" :*/
fileprivate let show_publisherPiString:[UInt8] = [0x9d,0xab,0xb1,0xe4,0xa5,0xb6,0xa1,0xe4,0xa7,0xb1,0xb6,0xb6,0xa1,0xaa,0xb0,0xa8,0xbd,0xe4,0xab,0xaa,0xe4,0xa5,0xe4,0xa7,0xa5,0xa8,0xa8,0xe8,0xe4,0xb4,0xa8,0xa1,0xa5,0xb7,0xa1,0xe4,0xb0,0xb6,0xbd,0xe4,0xa5,0xa3,0xa5,0xad,0xaa,0xe4,0xa8,0xa5,0xb0,0xa1,0xb6]

private func attentionWant(automatically num: UInt8) -> UInt8 {
    return num ^ 196
}

/*: "onCallResume" :*/
fileprivate let show_alongsideMereStr:[Character] = ["o","n","C","a","l"]
fileprivate let mainCountryYellowId:String = "lResumehighlight special original context request"
...
// __CLOSE_PRINT__
//
//  ChartShowSceneScientific+VideoChat.swift
//  AbroadTalking
//
...
extension ChartShowSceneScientific {
    ...
    func scaleContext(info: [String: Any], situation: ProjectVideo = .Start) {
        //: let json = JSON(info)
        let json = JSON(info)
        //: let type = json["type"].intValue
        let type = json[(dataPublishTauFormat.replacingOccurrences(of: "why", with: "t"))].intValue
        ...
        if type == 0 { // 视频
            ...
            (vc as! PerformPoint).chatModel = GreatEnhanceTimingAgileModel.atSubstantialMinimum(info: info, situation: situation)
            //: } else {
        } else { // 音频
            ...
            (vc as! TechniqueOld).chatModel = GreatEnhanceTimingAgileModel.atSubstantialMinimum(info: info, situation: situation)
        }

        //: self.tryToPushTalkRoom(roomVC: vc!, isVideoTalk: (type == 0 ? true : false), resumeTalk: false, resumeData: [:])
        self.abortMotive(roomVC: vc!, isVideoTalk: type == 0 ? true : false, resumeTalk: false, resumeData: [:])
    }

    /// 接收到视频通话
    /// - Parameter info: 数据
    //: func recieve1v1TalkRequest(info: [String: Any]) {
    func coca(info: [String: Any]) {
        //: handle1v1AudioRequest(info: info, resumeTalk: false)
        notRecord(info: info, resumeTalk: false)
    }
    ...
    func sink(info: [String: Any]) {
        ...
        notRecord(info: info, resumeTalk: true)
    }
}
...
extension ChartShowSceneScientific {
    ...
    private func notRecord(info: [String: Any], resumeTalk: Bool) {
        //: if !resumeTalk && TalkingSocketManager.shared.byWork {
        if !resumeTalk && ToPresent.shared.byWork {
            //: self.func__showStatusBarErrorMsg(showMsg: "You are currently on a call, please try again later".localized)
            self.bury(showMsg: String(bytes: show_publisherPiString.map{attentionWant(automatically: $0)}, encoding: .utf8)!.localized)
            //: return
            return
        }
        ...
        if (ObserverQuitEfficiency.shared as! ObserverQuitEfficiency).inviteCallView != nil {
            //: (TalkingApplication.shared as! TalkingApplication).inviteCallView?.closeView()
            (ObserverQuitEfficiency.shared as! ObserverQuitEfficiency).inviteCallView?.behindSecondWeight()
        }
        ...
        if type == 0 { // 视频通话
            ...
            (vc as! PerformPoint).chatModel = GreatEnhanceTimingAgileModel.atSubstantialMinimum(info: info, situation: resumeTalk ? .Resume : .Recieved)
            //: } else {
        } else { // 语音通话
            //: vc = TalkingVoiceChatViewController()
            vc = TechniqueOld()
            ...
            (vc as! TechniqueOld).chatModel = GreatEnhanceTimingAgileModel.atSubstantialMinimum(info: info, situation: resumeTalk ? .Resume : .Recieved)
        }

        //: let callResume = json["onCallResume"].stringValue
        let callResume = json[(String(show_alongsideMereStr) + String(mainCountryYellowId.prefix(7)))].stringValue
        ...
        if resumeTalk, callResume.count > 0, callResume == "1" { // 恢复通话
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 1) {
                ...
                self.abortMotive(roomVC: vc!, isVideoTalk: type == 0 ? true : false, resumeTalk: resumeTalk, resumeData: info)
            }
            //: } else {
        } else { // 构建音视频通话
            //: self.tryToPushTalkRoom(roomVC: vc!, isVideoTalk: (type == 0 ? true : false), resumeTalk: resumeTalk, resumeData: info)
    ...
    private func abortMotive(roomVC: UIViewController, isVideoTalk: Bool, resumeTalk: Bool, resumeData: [String: Any]) {
        //: guard var currentVc = currentViewController() else { return }
        guard var currentVc = chemist() else { return }
        ...
        if currentVc.isKind(of: UINavigationController.self) {
            ...
            let rootVc = CharacteristicSine.asCap().rootViewController as! UITabBarController
        ...
        if !isVideoTalk {
            ...
            talkClass = NSStringFromClass(TechniqueOld.self)
            //: } else {
        } else {
            //: talkClass = NSStringFromClass(TalkingVideoChatViewController.self)
            talkClass = NSStringFromClass(PerformPoint.self)
        }
        //: var chatIndex = -1
        var chatIndex = -1
        //: var index = -1
        ...
        if isVideoTalk {
            ...
            if currentChatVc != nil, currentChatVc?.chatModel?.pairUid == pushChatVc.chatModel?.pairUid {
                ...
                if resumeTalk {
                    ...
                    currentChatVc?.enabled(resumeData: resumeData)
                }
                //: return
                return
            }
            ...
            if navVc!.isKind(of: MediaAfter.self) {
                //: let nav = navVc as! TalkingNavigationController
                let nav = navVc as! MediaAfter
            ...
            if currentChatVc != nil, currentChatVc?.chatModel?.pairUid == pushChatVc.chatModel?.pairUid {
                ...
                if resumeTalk {
                    //: currentChatVc?.updateInfoViaResumeTalkData(resumeData: resumeData)
                    currentChatVc?.remove(resumeData: resumeData)
                }
                //: return
                return
            }
