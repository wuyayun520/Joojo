...
fileprivate let const_ventAlbumValue:String = "TSysreliability control top"
fileprivate let const_appAdvertisingString:String = "eastern tradessageC"
fileprivate let data_homeId:String = "euseIclick country body attribute mar"
fileprivate let constChoiceData:[Character] = ["d"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let app_validDetailedData:[UInt8] = [0x26,0x2b,0x26,0x31,0xe5,0x20,0x2c,0x21,0x22,0x2f,0xf7,0xe6,0xdd,0x25,0x1e,0x30,0xdd,0x2b,0x2c,0x31,0xdd,0x1f,0x22,0x22,0x2b,0xdd,0x26,0x2a,0x2d,0x29,0x22,0x2a,0x22,0x2b,0x31,0x22,0x21]

fileprivate func spaceAir(uniform num: UInt8) -> UInt8 {
...
/*: "icon_talk_notic" :*/
fileprivate let k_senData:String = "icon_talist could"
fileprivate let dataEnableeMsg:String = "lk_noticlast replacement"
...
@objc public protocol TotaleractionExecute: NSObjectProtocol {
    /**
     *  长按消息回调
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onLongPressMessage(cell: ImplementProperty)
    func endPrivacy(cell: ImplementProperty)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onRetryMessage(cell: ImplementProperty)
    func roundSlide(cell: ImplementProperty)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onSelectMessage(cell: ImplementProperty)
    func gapWith(cell: ImplementProperty)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onSelectMessageAvatar(cell: ImplementProperty)
    func performBy(cell: ImplementProperty)

    /**
     *  点击消息单元中引用消息
     *  @param cell 委托者，消息单元
     */
    //: func onReplyMessage(cell: ImplementProperty)
    func pinAnswer(cell: ImplementProperty)
}

//: @objcMembers public class ImplementProperty: TCommonTableViewCell {
@objcMembers public class ImplementProperty: ErrorBound {
    /**
     *  信息数据类。
     *  存储了该massageCell中所需的信息。包括发送者 ID，发送者头像、信息发送状态、信息气泡图标等。
     *  messageData 详细信息请参考：Section\Chat\CellData\EmbraceShadow.h
     */
    //: public var messageData: EmbraceShadow?
    public var messageData: EmbraceShadow?

    /// 复用cell标识符
    //: @objc public static let TSystemMessageCell_ReuseId = "TSystemMessageCell_ReuseId"
    public static let TSystemMessageCell_ReuseId = (String(const_ventAlbumValue.prefix(4)) + "temMe" + String(const_appAdvertisingString.suffix(6)) + "ell_R" + String(data_homeId.prefix(5)) + String(constChoiceData))

    /**
     *  协议委托
     *  负责实现 TotaleractionExecute 协议中的功能。
     */
    //: public weak var delegate: TMessageCellDelegate?
    public weak var delegate: TotaleractionExecute?
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: app_validDetailedData.map{spaceAir(uniform: $0)}, encoding: .utf8)!)
    }
    ...
    override public func grain(with data: ChartEmptyPop) {
        //: super.fill(with: data)
        super.grain(with: data)
        //: let newData: EmbraceShadow = data as! EmbraceShadow
        let newData: EmbraceShadow = data as! EmbraceShadow
    ...
    override public func updateConstraints() {
        ...
        let csize = self.messageData?.success()
    ...
    lazy var avatarView: UIImageView = {
        ...
        let tap1 = UITapGestureRecognizer(target: self, action: #selector(resultEmbrace(recognizer:)))
    ...
    lazy var nameLabel: UILabel = {
        ...
        lb.font = UIFont.pass(fontSize: 13)
        //: lb.textColor = .gray
        lb.textColor = .gray
    ...
    public lazy var container: UIView = {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(multipleSumegrity(recognizer:)))
        ...
        let longPress = UILongPressGestureRecognizer(target: self, action: #selector(clipAcrossAlive(recognizer:)))
    ...
    lazy var retryView: UIImageView = {
        ...
        imagV.image = UIImage.road(name: (String(k_senData.prefix(7)) + String(dataEnableeMsg.prefix(8))))
        ...
        let resendTap = UITapGestureRecognizer(target: self, action: #selector(goOf(recognizer:)))
...
//: extension ImplementProperty {
extension ImplementProperty {
    //: @objc func onSelectMessageAvatar(recognizer: UIGestureRecognizer) {
    @objc func resultEmbrace(recognizer: UIGestureRecognizer) {
        //: self.delegate?.onSelectMessageAvatar(cell: self)
        self.delegate?.performBy(cell: self)
    }

    //: @objc func onLongPress(recognizer: UIGestureRecognizer) {
    @objc func clipAcrossAlive(recognizer: UIGestureRecognizer) {
