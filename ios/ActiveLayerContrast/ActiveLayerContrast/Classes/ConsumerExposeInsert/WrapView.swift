...
fileprivate let app_endMessage:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Reply" :*/
fileprivate let user_borderKey:[Character] = ["R","e","p","l","y"]

/*: "#F95151" :*/
fileprivate let data_calendarName:String = "#F951year tab required associate zz"
fileprivate let constShadowTowardMessage:[Character] = ["5","1"]
...
let main_grayPath = (show_orientationKey + 80.0)
...
class WrapView: UIView {
    ...
    var startDismissBlock: ((_ model: DistinctionLocation) -> Void)? // 开始移除视图Block
    //: var finishDismissBlock: ((_ view: TalkingMessageNotificationView) -> Void)?
    var finishDismissBlock: ((_ view: WrapView) -> Void)? // 完成移除视图Block
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: app_endMessage.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var nickNameLabel: UILabel = {
        ...
        label.font = .bagUniform(type: .Medium, fontSize: 15)
    ...
    private lazy var replyBtn: UIButton = {
        ...
        btn.titleLabel?.font = .bagUniform(type: .Medium, fontSize: 15)
        //: btn.setTitle("Reply".localized, for: .normal)
        btn.setTitle((String(user_borderKey)).localized, for: .normal)
        //: btn.setTitleColor(UIColor(hex: "#F95151"), for: .normal)
        btn.setTitleColor(UIColor(hex: (String(data_calendarName.prefix(5)) + String(constShadowTowardMessage))), for: .normal)
        //: btn.isEnabled = false
        btn.isEnabled = false
        //: return btn
        return btn
...
extension WrapView {
    ...
    func systemOf(_ model: DistinctionLocation?) {
    ...
    func privacy(needAlpha: Bool = true) {
        ...
        DispatchQueue.main.async {
            ...
            UIView.animate(withDuration: 0.5) {
                ...
                if needAlpha {
                    ...
                    self.frame.origin.y = -main_grayPath
                }
                //: } completion: { _ in
            } completion: { _ in
    ...
    @objc private func length() {
        ...
        if currModel.chatType == .service {
            ...
            //: BeforeAssociate.share.func__pushToPriveteChatVC(chatID: currModel.targetId)
            BeforeAssociate.share.precis(chatID: currModel.targetId)
        }
    }

    /// 滑动手势
    //: @objc private func swipeGestureEvent(gesture: UISwipeGestureRecognizer) {
    @objc private func source(gesture: UISwipeGestureRecognizer) {
...
extension WrapView {
    ...
    func recover() {
        ...
        if let userInfo = currModel.gj_userInfo {
            ...
            headImgView.moreThen(urlStr: userInfo.headPic, placeImg: UIImage.holiday(sex: userInfo.sex))
            //: if !userInfo.headPicFrame.isEmptyString {
            if !userInfo.headPicFrame.isEmptyString {
                //: iconBorder.isHidden = false
                iconBorder.isHidden = false
                //: iconBorder.setHeadFrameUrlImage(urlStr: userInfo.headPicFrame)
                iconBorder.lightByMar(urlStr: userInfo.headPicFrame)
                //: } else {
            } else {
            ...
            headImgView.image = UIImage.alongPatter()
            //: iconBorder.isHidden = true
            iconBorder.isHidden = true
        }

        //: msgShowLabel.attributedText = TalkingChatListTableCell.handleShowC2CDetailTextOfMessage(model: currModel)
        msgShowLabel.attributedText = GlobalOdd.greenAcross(model: currModel)
    }
    ...
    private func numbererrupt() {
        ...
        self.frame = CGRect(x: 0, y: -main_grayPath, width: show_licenseTitle, height: main_grayPath)
    ...
    private func afterRebuild() {
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(tapGestureEvent))
        let tap = UITapGestureRecognizer(target: self, action: #selector(length))
        //: self.addGestureRecognizer(tap)
        self.addGestureRecognizer(tap)

        //: let swipe = UISwipeGestureRecognizer(target: self, action: #selector(swipeGestureEvent))
        let swipe = UISwipeGestureRecognizer(target: self, action: #selector(source))
        //: swipe.direction = .up
        swipe.direction = .up
