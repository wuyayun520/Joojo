...
/*: "serialQueue" :*/
fileprivate let dataRowString:String = "seriatherefor"

/*: "fromUids" :*/
fileprivate let user_oneDimensionString:[UInt8] = [0xdf,0xcb,0xd6,0xd4,0xec,0xd0,0xdd,0xca]

private func editAcross(nana num: UInt8) -> UInt8 {
    return num ^ 185
}
...
let data_marginPath = 50

//: @objcMembers
@objcMembers
//: public class DBUserInfoManager: NSObject {
public class TranslateScreen: NSObject {
...
extension TranslateScreen {
    ...
    ///   - userIdsArr: 用户ids
    ///   - completion: 回调
    //: class func cache_getUserInfo(with userIdsArr: [String], completion: @escaping ([KeepArchitecture]?, Error?) -> Void) {
    class func design(with userIdsArr: [String], completion: @escaping ([KeepArchitecture]?, Error?) -> Void) {
        // 数量过多，wcdb的where语句会因过长而报错，所以做分页
        //: var newUserIdsArr = userIdsArr
        var newUserIdsArr = userIdsArr
        //: if DBUserInfoManager.shared().needReqUidsSet != nil {
        if TranslateScreen.density().needReqUidsSet != nil { // 合并去重
            //: newUserIdsArr = Array(DBUserInfoManager.shared().needReqUidsSet!.union(userIdsArr))
            newUserIdsArr = Array(TranslateScreen.density().needReqUidsSet!.union(userIdsArr))
            //: DBUserInfoManager.shared().needReqUidsSet = nil
            TranslateScreen.density().needReqUidsSet = nil
        }
        ...
        repeat {
            ...
            behavior(with: subArr, completion: completion)
            //: location += len
            location += len
    ...
    class func onExamineion(with userArr: [String], completion: @escaping ([[String: Any]]?, Error?) -> Void) {
        ...
        let serialQueue = DispatchQueue(label: (dataRowString.replacingOccurrences(of: "therefor", with: "l") + "Queue"))
        ...
        serialQueue.async {
            ...
            for item in finalArray {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
                    ...
                    NameProjection.specifyCalculate(toUids: requestUidsStr) { _, list in
                        //: defer { sema.signal() }
                        defer { sema.signal() }
...
extension TranslateScreen {
    ...
    //: @discardableResult
    @discardableResult
    //: class func cache_getCachedUserInfo(targetId: String) -> KeepArchitecture? {
    class func drownBlindAnonymousEnhanceSetProvide(targetId: String) -> KeepArchitecture? {
        //: return WCDBUserInfoTable.db_getUserInfo(with: targetId)
        return OutsideTierRadio.partnerScenario(with: targetId)
    }

    /// 更新数据库【用户模型】，并发送通知
    /// - Parameter userInfo: 用户信息model
    //: class func cache_updateMsgUserInfoAndPostNotif(with userInfo: KeepArchitecture?) {
    class func noBring(with userInfo: KeepArchitecture?) {
        ...
        let model = TranslateScreen.drownBlindAnonymousEnhanceSetProvide(targetId: userInfo.uid)
        // 自定义字段需要赋值，防止被覆盖
        ...
        OutsideTierRadio.appearance(userInfo)
        //: DispatchQueue.main.async {
        DispatchQueue.main.async {
            //: NotificationCenter.default.post(name: USER_INFO_MSG_LIST_NOTIFICATION, object: [userInfo.uid: userInfo])
            NotificationCenter.default.post(name: show_responseStr, object: [userInfo.uid: userInfo])
        }
    }
    ...
    class func overMeasure(with targetId: String, intimate: Int, currTime: Int) {
        //: guard let model = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId) else { return }
        guard let model = TranslateScreen.drownBlindAnonymousEnhanceSetProvide(targetId: targetId) else { return }
    ...
    class func execute(with targetId: String, time: Int) {
        //: guard let model = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId) else { return }
        guard let model = TranslateScreen.drownBlindAnonymousEnhanceSetProvide(targetId: targetId) else { return }
    ...
    class func minService(with targetUid: String) {
        //: WCDBUserInfoTable.db_deleteUserInfo(with: targetUid)
        OutsideTierRadio.post(with: targetUid)
        //: DBUserInfoManager.shared().reqedUidsSet.remove(targetUid)
        TranslateScreen.density().reqedUidsSet.remove(targetUid)
    }
}
...
extension TranslateScreen {
    ...
    ///   - userIdsArr: 用户ids
    ///   - completion: 回调
    //: private class func inner_getUserInfo(with userIdsArr: [String], completion: @escaping ([KeepArchitecture]?, Error?) -> Void) {
    private class func behavior(with userIdsArr: [String], completion: @escaping ([KeepArchitecture]?, Error?) -> Void) {
        //: let userModels = WCDBUserInfoTable.db_getUserInfos(with: userIdsArr)
        let userModels = OutsideTierRadio.factory(with: userIdsArr)
        ...
        for uid in userIdsArr where !userModels.contains(where: { $0.uid == uid }) {
            ...
            if TranslateScreen.density().reqedUidsSet.contains(uid) == false {
        ...
        if !noDBUidArr.isEmpty {
            ...
            TranslateScreen.lastNoticeInfo(with: noDBUidArr)
        }
    }
    ...
    private class func lastNoticeInfo(with userIdsArr: [String]) {
        ...
        TranslateScreen.density().reqedUidsSet = TranslateScreen.density().reqedUidsSet.union(userIdsArr)

        // 请求用户消息列表
        //: TalkingChatRequestTool.req_refreshMsgUserListInfo(param: ["fromUids": requestUidsStr]) { succeed, dataArr in
        NameProjection.anHide(param: [String(bytes: user_oneDimensionString.map{editAcross(nana: $0)}, encoding: .utf8)!: requestUidsStr]) { succeed, dataArr in
            //: guard succeed else {
            guard succeed else { // 失败还原数据
                //: DBUserInfoManager.shared().reqedUidsSet.subtract(userIdsArr)
                TranslateScreen.density().reqedUidsSet.subtract(userIdsArr)
                //: return
                return
            }
            // 更新数据库
            //: DBUserInfoManager.updateMsgUserInfoTable(userInfoArr: dataArr)
            TranslateScreen.incomeCountensity(userInfoArr: dataArr)
            // 发送通知
            //: var retInfoDict: [String: KeepArchitecture] = [:]
            var retInfoDict: [String: KeepArchitecture] = [:]
            ...
            if !retInfoDict.isEmpty {
                ...
                DispatchQueue.main.async {
                    ...
                    NotificationCenter.default.post(name: show_responseStr, object: retInfoDict)
                }
            }
        }
    }
    ...
    //: private class func updateMsgUserInfoTable(userInfoArr: [KeepArchitecture]) {
    private class func incomeCountensity(userInfoArr: [KeepArchitecture]) {
        //: for aWrap in userInfoArr {
        for aWrap in userInfoArr {
            //: WCDBUserInfoTable.db_updateUserInfo(aWrap)
            OutsideTierRadio.appearance(aWrap)
        }
    }
}
