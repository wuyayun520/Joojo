...
/*: "Speaker On" :*/
fileprivate let dataCombinedStr:String = "slow past error distributionSpeaker"
fileprivate let appRepublicName:[Character] = [" ","O","n"]

/*: "Speaker Off" :*/
fileprivate let showStreamString:String = "Speakspit depth up put check"
fileprivate let k_equipmentKey:[Character] = ["e","r"," ","O","f","f"]

/*: "TRTC 音频通话销毁. logId： :*/
fileprivate let main_flexibleMsg:String = "TRTC tool weight"
fileprivate let user_environmentMessage:[Character] = ["."," ","l","o","g","I","d","："]

/*: "TRTC 音频通话错误. errCode: :*/
fileprivate let constTeeMessage:[Character] = ["T","R","T","C"," ","\u{97f3}","频","通","话","错","误","."," ","e","r","r","C","o"]
fileprivate let const_promiseMessage:String = "just it giving ariade:"

/*: , errMsg: :*/
fileprivate let showYourselfKey:String = "called instance, err"

/*: "TRTC 音频通话SDK收到警告. warningCode: :*/
fileprivate let main_statueString:[Character] = ["T","R","T","C"," ","音"]
fileprivate let noti_adviceKey:String = "\u{9891}通\u{8bdd}SD"
fileprivate let k_streamUrl:String = "condemn consumer add pick multiple告. w"
fileprivate let userPushId:String = "gCode:sort behind key"

/*: , warningMsg: :*/
fileprivate let k_statueGraphName:String = ", warnyou agree"
fileprivate let show_reasonStr:String = "migration"
fileprivate let k_tingSuccessfullyMsg:[Character] = ["n","g","M","s","g",":"]

/*: "TRTC 音频通话进入房间. result: :*/
fileprivate let show_twoGraphId:String = "honeyRhoneyC"
fileprivate let show_quotePressStr:String = "\u{5165}房间. "
fileprivate let show_fileData:String = "click divide instruction more nort:"

/*: , logId： :*/
fileprivate let data_additionString:[Character] = [","," ","l","o","g","I","d","："]

/*:  加入了房间" :*/
fileprivate let notiTitleData:[Character] = [" ","加","入","了","房","间"]

/*:  离开了房间" :*/
fileprivate let user_mereDentedTitle:[Character] = [" ","离","开","了","房","\u{95f4}"]
...
private class SquareBullet: NSObject {
...
class MenuArc: NSObject {
    ...
    private var config = SquareBullet() // 音频渲染配置类
    //: private var logId = 0
...
extension MenuArc {
    ...
    func include(roomId: Int) {
        //: self.handsfreeBtn?.isEnabled = !hasHeadSet()
        self.handsfreeBtn?.isEnabled = !doSelectFade()
    ...
    func architectureFrame(isMute: Bool) {
    ...
    func discount(isHandsfree: Bool) {
        //: guard hasHeadSet() == false else { return }
        guard doSelectFade() == false else { return }
        ...
        self.handsfreeDesLab?.text = isHandsfree ? (String(dataCombinedStr.suffix(7)) + String(appRepublicName)).localized : (String(showStreamString.prefix(5)) + String(k_equipmentKey)).localized
        //: updateWhenPlayDeviceChanged(isHandsfree: isHandsfree)
        basicAcross(isHandsfree: isHandsfree)
    }
}
...
extension MenuArc {
    ...
    private func doSelectFade() -> Bool {
...
extension MenuArc {
    ...
    func reveal() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(alongRe(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(mission(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(constraintOf(notif:)),
    ...
    @objc private func alongRe(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
    ...
    @objc private func mission(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
    ...
    @objc private func time(notif: Notification) {
    ...
    @objc func constraintOf(notif: Notification) {
        ...
        case .oldDeviceUnavailable: // 免提
            ...
            self.handsfreeDesLab?.text = (String(dataCombinedStr.suffix(7)) + String(appRepublicName)).localized
            //: updateWhenPlayDeviceChanged(isHandsfree: self.config.isHandsfree)
            basicAcross(isHandsfree: self.config.isHandsfree)
...
extension MenuArc {
    ...
    private func basicAcross(isHandsfree: Bool) {
    ...
    private func primary() {
        ...
        KitTolerance.evenIndex(msg: (String(main_flexibleMsg.prefix(5)) + "音频\u{901a}话\u{9500}毁" + String(user_environmentMessage)) + "\(logId).")
        //: guard self.config.byWork else { return }
        guard self.config.byWork else { return }

        //: if self.config.isHandsfree == false {
        if self.config.isHandsfree == false { // 恢复系统扬声器播放声音
            //: updateWhenPlayDeviceChanged(isHandsfree: true)
            basicAcross(isHandsfree: true)
        }
...
extension MenuArc: TRTCCloudDelegate {
    //: func onError(_ errCode: TXLiteAVError, errMsg: String?, extInfo: [AnyHashable: Any]?) {
    func onError(_ errCode: TXLiteAVError, errMsg: String?, extInfo: [AnyHashable: Any]?) {
        //: UploadLogTool.writeLog(msg: "TRTC 音频通话错误. errCode:\(errCode), errMsg:\(String(describing: errMsg)).")
        KitTolerance.evenIndex(msg: (String(constTeeMessage) + String(const_promiseMessage.suffix(3))) + "\(errCode)" + (String(showYourselfKey.suffix(5)) + "Msg:") + "\(String(describing: errMsg)).")
        //: if errMsg != nil {
        if errMsg != nil {
    ...
    func onWarning(_ warningCode: TXLiteAVWarning, warningMsg: String?, extInfo: [AnyHashable: Any]?) {
        ...
        KitTolerance.evenIndex(msg: (String(main_statueString) + noti_adviceKey + "K收到警" + String(k_streamUrl.suffix(4)) + "arnin" + String(userPushId.prefix(6))) + "\(warningCode)" + (String(k_statueGraphName.prefix(6)) + show_reasonStr.replacingOccurrences(of: "migration", with: "i") + String(k_tingSuccessfullyMsg)) + "\(String(describing: warningMsg)).")
    }

    //: func onEnterRoom(_ result: Int) {
    func onEnterRoom(_ result: Int) {
        //: UploadLogTool.writeLog(msg: "TRTC 音频通话进入房间. result:\(result), logId：\(logId).")
        KitTolerance.evenIndex(msg: (show_twoGraphId.replacingOccurrences(of: "honey", with: "T") + " 音\u{9891}\u{901a}话进" + show_quotePressStr + "resul" + String(show_fileData.suffix(2))) + "\(result)" + (String(data_additionString)) + "\(logId).")
        //: if result < 0 {
        if result < 0 { // 加入失败
    ...
    func onRemoteUserEnterRoom(_ userId: String) {
        ...
        printLog(message: "\(userId)" + (String(notiTitleData)))
    }

    //: func onRemoteUserLeaveRoom(_ userId: String, reason: Int) {
    func onRemoteUserLeaveRoom(_ userId: String, reason: Int) {
        //: printLog(message: "\(userId) 离开了房间")
        printLog(message: "\(userId)" + (String(user_mereDentedTitle)))
    }
}
