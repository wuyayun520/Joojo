...
/*: "#BAFBFF" :*/
fileprivate let data_delayFormat:String = "roman hello drag#BAFBFF"
...
class CellSize: NSObject {
    ...
    func infrastructurePrivacy(model: OvalTrain) -> OvalTrain {
        //: self.model = model
        self.model = model
        /// 普通文本消息
    ...
    func assetMaintain(model: OvalTrain) -> OvalTrain {
        //: self.model = model
        self.model = model
        /// 普通文本消息
        //: if self.model.messageType == 1 {
    ...
    func daily() {
        ...
        let name = underSquare(text: firstStr, font: UIFont.pass(fontSize: 14), color: UIColor(hex: (String(data_delayFormat.suffix(7))))!)
        // 内容
        //: let content = getAttributed(text: self.model.content, font: UIFont.pingfangMediumFont(fontSize: 14), color: UIColor.white)
        let content = underSquare(text: self.model.content, font: UIFont.pass(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        ...
        beforeQuick(attribText: self.model.msgAttribText)
    }
    ...
    func cover() {
        ...
        let content = underSquare(text: self.model.transContent, font: UIFont.pass(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        //: name.yy_paragraphStyle = paraStyle
        name.yy_paragraphStyle = paraStyle
    ...
    func timeSwitchto() -> NSMutableParagraphStyle {
    ...
    func underSquare(text: String, font: UIFont, color: UIColor) -> NSMutableAttributedString {
...
//            attribute.yy_writingDirection = [3]
        ...
        attribute.yy_paragraphStyle = timeSwitchto()
        //: return attribute
        return attribute
    }
    ...
    func beforeQuick(attribText: NSMutableAttributedString) {
        // 距离左边8  距离右边也为8
        //: let maxWidth = VideoMsgTableViewWidth - 36
        let maxWidth = k_easyObserveKey - 36
