...
fileprivate let user_airFormat:[Character] = ["i","c","o","n","_","k","o","n","g","_","k","o","n","g"]
fileprivate let notiAvailableHiddenValue:[Character] = ["_","d","e","f","a","u","l","t"]

/*: "page" :*/
fileprivate let show_whiteMergeLightMessage:[UInt8] = [0xc0,0xb1,0xb7,0xb5]

fileprivate func unknownRecover(withdraw num: UInt8) -> UInt8 {
...
/*: "uid" :*/
fileprivate let k_sentenceFailMessage:[UInt8] = [0xcd,0xd1,0xdc]

private func disableClot(direction num: UInt8) -> UInt8 {
    return num ^ 184
}

/*: "list" :*/
fileprivate let show_multiId:String = "lspeakert"
...
class MainViewController: TrainBrush {
    //: private var pageIndex = 0
    private var pageIndex = 0
    //: private var postDataArr = [TalkingUserInfoMomentItemModel]()
    private var postDataArr = [DespiteFrequency]()
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        collectionView.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: notiTotalervalFormat, right: 0)
        //: collectionView.register(TalkingPostItemCell.self, forCellWithReuseIdentifier: TalkingPostItemCell.className())
        collectionView.register(ReceiverPromiseDownWould.self, forCellWithReuseIdentifier: ReceiverPromiseDownWould.className())
    ...
    private lazy var emptyView: UIView = {
        ...
        img.image = UIImage.road(name: (String(user_airFormat) + String(notiAvailableHiddenValue)))
...
extension MainViewController {
    ...
    private func during() {
        ...
        let params = [String(bytes: show_whiteMergeLightMessage.map{unknownRecover(withdraw: $0)}, encoding: .utf8)!: pageIndex, String(bytes: k_sentenceFailMessage.map{disableClot(direction: $0)}, encoding: .utf8)!: userInfoModel.uid] as [String: Any]
        //: TalkingMomentRequestTool.req_UserMomentList(params: params) { [weak self] succeed, result, errorModel in
        BelowPer.combineOld(params: params) { [weak self] succeed, result, errorModel in
            //: guard let self = self else { return }
            guard let self = self else { return }
            ...
            if let resultArr = resultDict[(show_multiId.replacingOccurrences(of: "speaker", with: "is"))] as? [Any] {
                ...
                for dict in resultArr {
                    ...
                    if let model = FinishEdge.deserialize(from: dict as? Dictionary) {
...
extension MainViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: ReceiverPromiseDownWould.className(), for: indexPath) as! ReceiverPromiseDownWould
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let vc = FutureCriticalSimilar(model: detailModel)
...
extension MainViewController {
    ...
    func bridge(_ userModel: ThresholdBeyond) {
