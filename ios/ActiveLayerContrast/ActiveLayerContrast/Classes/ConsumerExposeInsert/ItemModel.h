...
@class PhoneModel, ActiveModel, RoleOptionModel, SectionModel, UndertakeModel, KeepArchitecture, CurrencyModel, KitEaseModel, ScreenModel, FrameworkTimePlainModel, ScientificHandleModel, LockModel;
...
@property (nonatomic, strong)PhoneModel *msgInfo;

//: @property (nonatomic, strong)AbTalkingChatMsgUserModel *user;
@property (nonatomic, strong)ActiveModel *user;

//: @property (nonatomic, strong)AbTalkingChatMsgToUserModel *toUser;
@property (nonatomic, strong)RoleOptionModel *toUser;

//: @property (nonatomic, strong)AbTalkingChatMsgGiftModel *gift;
@property (nonatomic, strong)SectionModel *gift;

//: @property (nonatomic, strong)AbTalkingChatMsgTipsModel *tips;
@property (nonatomic, strong)UndertakeModel *tips;

//: @property (nonatomic, strong)AbTalkingChatMsgQuoteModel *quoteMsgInfo;
@property (nonatomic, strong)FrameworkTimePlainModel *quoteMsgInfo;

//: @property (nonatomic, strong)KeepArchitecture *userInfoModel;
@property (nonatomic, strong)KeepArchitecture *userInfoModel;
...
@property (nonatomic, strong) CurrencyModel *intimatePhoto; /// 有该字段表示为私密照片
//: @property (nonatomic, strong) AbTalkingChatMsgVideoModel *video; 
@property (nonatomic, strong) KitEaseModel *video; /// 有该字段表示是视频消息
//: @property (nonatomic, strong) AbTalkingChatMsgCustomSendModel *customSend; 
@property (nonatomic, strong) ScreenModel *customSend; /// 保存自定义发送字段（用于失败重发）

//: @property (nonatomic, strong) WantGiftModel *wantGift; 
@property (nonatomic, strong) LockModel *wantGift; /// 想要礼物
...
@property (nonatomic, strong) NSArray<EditModel> *replay; /// v1.9.8 男性用户需要回复的选择内容
...
+ (void)scope:(CurrencyModel *)photoModel provider:(NSString *)msgID;

//: + (NSMutableDictionary<NSString *, NSString *> *)loadLocalIntimatePhotoDict;
+ (NSMutableDictionary<NSString *, NSString *> *)kit;
...
@interface KitEaseModel: JSONModel
...
+ (void)extend:(KitEaseModel *)videoModel by:(NSString *)msgID;

//: + (NSMutableDictionary<NSString *, NSString *> *)loadLocalIntimateVideoDict;
+ (NSMutableDictionary<NSString *, NSString *> *)response;
...
@interface RoleOptionModel : JSONModel
...
@interface UndertakeModel : JSONModel
...
+ (void)outsidePresent:(NSString *)msgID;

//: + (NSMutableArray *)loadAskForGiftArr;
+ (NSMutableArray *)entityTo;

//: @end
@end

//: @interface AbTalkingChatMsgQuoteModel : JSONModel
@interface FrameworkTimePlainModel : JSONModel
...
@property (nonatomic, strong) ScientificHandleModel *renderData;

//: - (BOOL)isQuoteMsg;
- (BOOL)yield;

//: - (CGSize)quoteSize:(NSInteger)maxWidth;
- (CGSize)locate:(NSInteger)maxWidth;
...
@interface ScientificHandleModel : JSONModel
