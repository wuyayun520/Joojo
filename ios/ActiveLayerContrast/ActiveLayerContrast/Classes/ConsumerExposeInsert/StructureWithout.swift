...
fileprivate let dataYouValue:[UInt8] = [0x59,0x5e,0x59,0x64,0x18,0x53,0x5f,0x54,0x55,0x62,0x2a,0x19,0x10,0x58,0x51,0x63,0x10,0x5e,0x5f,0x64,0x10,0x52,0x55,0x55,0x5e,0x10,0x59,0x5d,0x60,0x5c,0x55,0x5d,0x55,0x5e,0x64,0x55,0x54]

fileprivate func darkHave(battery num: UInt8) -> UInt8 {
...
/*: "img_home_shadow" :*/
fileprivate let noti_myFormat:String = "img_homefreeze stat"
fileprivate let mainCellPath:[Character] = ["_","s","h","a","d","o","w"]

/*: "#9610FF" :*/
fileprivate let appWritTitle:[Character] = ["#","9","6","1","0","F","F"]

/*: "#8566FF" :*/
fileprivate let show_romanRawSituationString:[Character] = ["#","8","5","6","6","F","F"]

/*: "Random Video" :*/
fileprivate let userSplitRootStr:String = "its super server bodyRand"

/*: "icon_match_call_free" :*/
fileprivate let k_decisionTeeKey:String = "wait construct why field ontoicon_m"
fileprivate let dataCharacteristicTowardName:String = "_call_reward way ask"

/*: "Free" :*/
fileprivate let kRouteCollectionUrl:String = "smart medalFree"

/*: "Scroll_upto_discove" :*/
fileprivate let mainTagAfterData:[Character] = ["S","c","r","o","l","l","_","u","p","t"]
fileprivate let user_platformPanString:String = "o_discoveintensity disk extra begin dented"

/*: "icon_VectorStroke" :*/
fileprivate let appHolderTitle:String = "icon_variable freeze operate there pass"
fileprivate let constSingleMsg:[Character] = ["V","e","c","t","o","r","S"]
fileprivate let main_balanceScenarioName:String = "assistoke"

/*: "get json error" :*/
fileprivate let notiFeeStr:[Character] = ["g","e","t"," ","j","s","o","n"," ","e","r","r","o","r"]
...
class StructureWithout: UIView {
    ...
    var maxRemoveDistance = show_licenseTitle / 4.0 /// 最大移除距离(默认屏幕的1/4)
    //: var reusableCells = [TalkingClubCardViewCell]()
    var reusableCells = [BlocFraction]() /// 重用卡片数组
    //: var isAlpha = true
    ...
    var dataSource: EffectGift? /// 数据源
    //: var delegate: TalkingClubCardViewDelegate?
    var delegate: FormatCharacteristic?
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: dataYouValue.map{darkHave(battery: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var leftbackImageView: UIImageView = {
        ...
        imag.image = UIImage.road(name: (String(noti_myFormat.prefix(8)) + String(mainCellPath)))
    ...
    private lazy var randomBtn: UIButton = {
        ...
        btn.setBackgroundImage(UIImage.managerMouse(colors: [UIColor(hex: (String(appWritTitle)))!.cgColor, UIColor(hex: (String(show_romanRawSituationString)))!.cgColor], size: CGSize(width: show_licenseTitle - 30, height: 54)), for: .normal)
        //: btn.layer.cornerRadius = 27
        btn.layer.cornerRadius = 27
        //: btn.layer.masksToBounds = true
        btn.layer.masksToBounds = true
        //: btn.setTitle("Random Video".localized, for: .normal)
        btn.setTitle((String(userSplitRootStr.suffix(4)) + "om Video").localized, for: .normal)
        //: btn.addTarget(self, action: #selector(randomBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(tick), for: .touchUpInside)
        //: self.addSubview(btn)
        self.addSubview(btn)
    ...
    lazy var freeImag: UIButton = {
        ...
        img.setBackgroundImage(UIImage.road(name: (String(k_decisionTeeKey.suffix(6)) + "atch" + String(dataCharacteristicTowardName.prefix(6)) + "free")), for: .normal)
        //: img.setTitle("Free".localized, for: .normal)
        img.setTitle((String(kRouteCollectionUrl.suffix(4))).localized, for: .normal)
...
extension StructureWithout {
    ...
    //: func setFirstGuide() {
    func coordinatorGraph() {
        //: showGuidePage()
        tillGlobal()
    }

    //: private func showGuidePage() {
    private func tillGlobal() {
        ...
        image.image = UIImage.road(name: (String(mainTagAfterData) + String(user_platformPanString.prefix(9))))
        ...
        image2.image = UIImage.road(name: (String(appHolderTitle.prefix(5)) + String(constSingleMsg) + main_balanceScenarioName.replacingOccurrences(of: "assist", with: "tr")))
        ...
        drinkToSay(player: player)
    }

    //: private func likeplayer(player: SVGAPlayer) {
    private func drinkToSay(player: SVGAPlayer) {
        //: do {
        do {
            //: let url = SVGAEffectTool.default.getZipEffectPath(type: .Club_guidePage)
            let url = DocumentWhite.default.countensity(type: .Club_guidePage)
            ...
            printLog(message: (String(notiFeeStr)))
        }
    }
    ...
    @objc private func tick() {
        ...
        self.delegate?.retainHiSumoLink?()
    }

    //: func reloadDataAnimated(anim: Bool) {
    func noneBubbleSize(anim: Bool) {
        //: moveCount = 0
        moveCount = 0
        //: self.currentIndex = 0
        self.currentIndex = 0
        ...
        let maxCount = dataSource?.appropriateSingle(cardView: self) ?? 0
    ...
    func robotOrOn(index: Int) {
        //: let cell = dataSource?.cardViewCellForRowAtIndex(cardView: self, index: index)
        let cell = dataSource?.sTexture(cardView: self, index: index)
...
//        let showCount = Double(self.visibleCount - 1)
        ...
        containerView.insertSubview(cell ?? BlocFraction(), at: 0)
    ...
    func mustGate(anim: Bool) {
        ...
        for i in 0 ... count - 1 {
            ...
            let cell: BlocFraction = self.visibleCells[i] as! BlocFraction
            ...
            if isVisbleLast {
                ...
                self.delegate?.mobile?(cardViewCell: cell, forRowAtIndex: self.currentIndex - i)
            }

            //: if anim {
            if anim {
                //: updateConstraintsCell(cell: cell, transform: transform)
                bling(cell: cell, transform: transform)
                //: } else {
            } else {
    ...
    func bling(cell: BlocFraction, transform: CGAffineTransform) {
    ...
    func cropIn(cellClass: AnyClass, forellIdentifier: String) {
    ...
    func clearClick(identifier: String) -> BlocFraction? {
        ...
        if self.cellClass != nil {
            ...
            let cellclass = self.cellClass as! HeapRounding.Type
            //: let cell: TalkingClubCardItemCell = cellclass.init(reuseIdentifier: identifier )
            let cell: HeapRounding = cellclass.init(reuseIdentifier: identifier)
...
extension StructureWithout: InvisibleMax {
    ...
    func beforeSwitchicial(cell: BlocFraction) {
        ...
        self.delegate?.lover?(cardView: self, cardViewCell: cell, forRowAtIndex: self.currentFirstIndex - 1)

        //: let count = self.dataSource?.numberOfCountInCardView(cardView: self) ?? 0
        let count = self.dataSource?.appropriateSingle(cardView: self) ?? 0
        // 移除后的卡片是最后一张(没有更多)
        ...
        if self.visibleCells.count == 0 { // 只有最后一张卡片的时候
            ...
            self.delegate?.someMere?(cardViewCell: cell, forRowAtIndex: self.currentIndex)
            //: return
            return
        }
        // 当前数据源还有数据 继续创建cell
        ...
        if self.currentIndex < count - 1 {
            ...
            robotOrOn(index: self.currentIndex + 1)
        }
        // 更新布局
    ...
    func anyOld(cell: BlocFraction, forMovePoint: CGPoint) {
        //: self.delegate?.didMoveCell?(cardView: self, cardViewCell: cell, forMovePoint: forMovePoint)
        self.delegate?.bill?(cardView: self, cardViewCell: cell, forMovePoint: forMovePoint)
    }
}
